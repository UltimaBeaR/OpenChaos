...
**Предположение по причине:** где-то в последних коммитах (ветка `main`, коммиты `c650f3c`–`86e8ecb`) при удалении «мёртвого» кода была случайно задета живая логика. Кандидаты: функции или переменные, которые казались unused по предупреждениям компилятора, но реально использовались через косвенные вызовы, указатели на функции, или были side-effect-ными (например, глобальные конструкторы/деструкторы, инициализации).

**Что нужно сделать:**
1. Прогнать `git diff` по коммитам `c650f3c`..`86e8ecb` (или по каждому коммиту отдельно) и вручную перепроверить все удалённые сущности — особенно в файлах с глобальным состоянием, инициализацией, или с указателями на функции.
