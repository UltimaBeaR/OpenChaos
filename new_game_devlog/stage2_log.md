...
- `Game.cpp`: `psx_camera()` — всегда `return 0`, вызов закомментирован (`// psx_camera()`). 
- `cam.h`: объявление `CAM_get_psx()` — нигде не определена и не вызывается.
- `Controls.cpp`: `dodgy_psx_mode` — объявлена и инициализирована, нигде не читается.
...
**Результат:** 0 ошибок. Debug: 130 предупреждений, Release: 255 предупреждений (без изменений).
...
## ⚠️ Рантайм-регрессия — обнаружена (2026-03-18)

В ходе итераций Этапа 2 была сломана игра в рантайме. Компиляция проходит чисто, но при запуске что-то идёт не так.

**Статус:** не исследовано, нужно фиксить.

**Предположение по причине:** где-то в последних коммитах (ветка `main`, начиная с `3e699d613880af4ec36bf89b9a1afd3a2fe07c56` включительно до последнего коммита) при удалении «мёртвого» кода была случайно задета живая логика. Кандидаты: функции или переменные, которые казались unused по предупреждениям компилятора, но реально использовались через косвенные вызовы, указатели на функции, или были side-effect-ными (например, глобальные конструкторы/деструкторы, инициализации).

**Что нужно сделать:**
1. Прогнать `git log 3e699d6..HEAD` и пройтись по диффам каждого коммита — вручную перепроверить все удалённые сущности, особенно в файлах с глобальным состоянием, инициализацией, или с указателями на функции.
2. Сузить коммит-виновник: `git bisect` между последним рабочим состоянием и текущим.
3. Восстановить удалённое или заменить заглушкой с сохранением поведения.

**Примечание:** при следующей сессии Claude должен пройтись по диффам коммитов самостоятельно — ошибку внёс именно ИИ в рамках автоматического удаления мёртвого кода.
