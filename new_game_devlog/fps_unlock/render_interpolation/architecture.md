...
Физика тикает 20 Hz (`g_physics_hz`, дизайновая частота оригинала, `NORMAL_TICK_TOCK=50ms`). Рендер бежит на любой частоте (60+ Hz). Без интерполяции render-кадры между physics-тиками показывают **идентичные позиции** — визуально это рывки, мир кажется тормозным даже при высоком FPS.
