...
2. **[fps_unlock_pitfalls.md](fps_unlock_pitfalls.md)** — чеклист типичных граблей при отвязке эффектов; читать ПЕРЕД работой над любым новым FPS-bound эффектом, **дописывать** при нахождении новых классов бага
3. Этот файл — понять статус
4. [fps_unlock.md](fps_unlock.md) — почему нельзя просто поднять Hz, какой путь правильный
5. [fps_unlock_issues.md](fps_unlock_issues.md) — конкретные баги если нужно что-то чинить
6. [debug_physics_render_rate.md](debug_physics_render_rate.md) — детали реализации если нужно трогать debug-инструмент
