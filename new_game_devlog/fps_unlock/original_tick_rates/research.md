...
2. Прочитать [`../review/findings.md`](../review/findings.md) — особенно §2, §3, §4,
   §5, §15, §17 — это уже частичный анализ герцовок которые надо углубить.
3. Прочитать `original_game_knowledge_base/` — поиск по "tick", "rate", "frame",
   "Timer", "GAME_TURN" — может быть уже есть полезный анализ.
4. Запустить грепы из секции "Полезные грепы" и проанализировать каждый матч.
5. Прочитать `original_game/fallen/Source/Game.cpp` целиком — особенно main
...
- **НЕ trust комментариям** в текущем new_game/ коде безоговорочно. Многие
  ...
  предположений (см. §5 в `../review/findings.md`). Проверять по
  оригинальному коду.
