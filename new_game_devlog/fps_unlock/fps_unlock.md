...
### Debug-инструмент (клавиши 1/2/9/0)

Runtime смена physics/render Hz. Оверлей в левом верхнем углу.
...
## Почему render > 30 Hz без интерполяции не даёт плавности

При `physics_fps=30`, `render_fps=60`:
- Физика обновляет позиции объектов раз в 33 мс
- Рендер рисует кадр каждые 16 мс, но объекты не двигались
...
### А. Render interpolation (рекомендуется, пост-1.0)

- Физика остаётся на 30 Hz — геймплей корректный
- Рендер на 60/120 Hz
...
- Требует: сохранять `WorldPos` / ротацию на конец каждого тика (снапшоты t-1 и t)
- Затронет: все draw-функции персонажей, объектов, частиц
- Игровая логика не меняется совсем

**Это правильный путь.** Геймплей точный (30 Hz физика), визуал плавный (60+ Hz рендер).
...
### В. Гибрид: render 60 Hz, physics 30 Hz, без интерполяции

- Только уменьшение input latency
- Визуально неотличимо от 30 FPS
- Реализовано инфраструктурно прямо сейчас — просто поставить `g_render_fps_cap=60`

---

## Physics Hz в разных версиях

Замеры (2026-04-29). NORMAL_TICK_TOCK = 50ms → дизайновая частота = 20 Hz.
...
**Вывод:** оригинал проектировался под 20 Hz (NORMAL_TICK_TOCK=50ms). При 20 Hz таймер
миссий идёт 1:1 с реальным временем. Ни PS1 ни PC-релиз этого не достигали из-за
