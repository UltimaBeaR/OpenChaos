...
**Дополнение 2026-05-05** (архитектурный фикс multi-toggle + lost-press):

Помимо изначальной "реакция на короткое нажатие при низкой физике" — был ещё класс
багов от зажатия кнопки сирены. `apply_button_input_car` уровнево копировал
`INPUT_MASK_KICK`/`INPUT_MASK_JUMP` в `veh->DControl |= VEH_SIREN` каждый physics-тик
пока кнопка зажата. `do_car_input` per-tick делал `siren(veh, !veh->Siren)` — siren
переключался N раз за зажатие, конечное состояние зависело от чётности (physics_hz ×
hold_duration). Например на 20 Hz физике зажатие на 200 ms = 4 toggle = обратно в
исходное.

**Фикс через input_frame sticky press_pending:**

1. Добавлен `input_btn_press_pending` / `input_btn_consume` для гейпада в input_frame
   (mirror keyboard'у). Latched на rising edge каждого render-frame snapshot'а,
   cleared только consume'ом.

2. В [`apply_button_input_car`](../../new_game/src/game/input_actions.cpp) — siren-bit
   ставится только если `press_pending` true для соответствующей кнопки. Consume
   вызывается безусловно каждый physics-тик (drain pattern).

3. В call-site `do_packets` (else-ветка не-в-машине) — те же кнопки drain'ятся каждый
   physics-тик чтобы накопленный pending от outside-car-press'ов (Triangle для menu
   cancel, SPACE для jump) не leaked в первый car-entry tick.

**Почему press_pending, а не local edge:**

Изначально была попытка с local static edge-detect (was_kb_siren / was_pad_siren) —
работала для multi-toggle, но **не ловила tap'ы целиком между двумя physics-тиками**.
На 1 Hz physics (debug) tap 150ms полностью между двумя 1-секундными тиками → level
signal на обоих тиках = "не зажата" → tap пропадает. Пользователь поймал это в
тестировании.

`press_pending` решает: render-frame snapshot ловит rising edge → flag latched →
выживает до physics-тика → consume и toggle. Любой rate физики, любой tap.

**Drain rule** (зафиксирован в `input_frame.h`): consumer должен вызывать `consume()`
**безусловно** в каждом relevant физтике (не только когда action'ится), и **дренировать
вне active state'а** если кнопка dual-use (Triangle = menu cancel + car siren). Без
этого stale flag будет accumulated и фаерит spurious toggle при первом entry.

Подтверждено пользователем 2026-05-05 (multi-toggle + lost-press на 1 Hz physics + leak от outside-car press'ов).
...
**Симптом (был):** брошенная граната во время полёта рендерилась не интерполировано — ступенчатое движение по physics-тикам (~20 Hz). Банка пепси (аналогичный механизм) интерполировалась корректно.

**Закрыто** 2026-05-05: граната вписана в render_interp-пайплайн аналогично банке. Подтверждено пользователем.
