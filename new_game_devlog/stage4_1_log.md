...
## Шаг 4 — Закомментированные тела функций (2026-03-24)

Изменение плана: первоначально планировалось просто удалять `/* */` и оставлять пустые тела.
На практике решили иначе: однострочный комментарий-пояснение **нужен** (иначе непонятно —
это баг или осознанный stub). Поэтому:
- Многострочные `/* */` блоки с закомментированным кодом → удалены, заменены на `// Stub — <причина>.`
- Однострочные пояснительные комментарии → оставлены/приведены к формату `// Stub — ...`
- Мёртвые функции (не вызываются) → не удалялись (было только 2 кандидата: `level_won`/`level_lost` —
  оставлены как stub'ы с пояснением)

Обработано **33 stub-функции** в 19 файлах.
Закомментированный код удалён из: `aeng.cpp` (`AENG_draw_power` ~75 строк, `AENG_demo_attract` ~26 строк),
`building.cpp` (`add_tri_to_walkable_list`, `fn_building_normal`), `outro_os.cpp` (`OS_texture_finished_creating`).
