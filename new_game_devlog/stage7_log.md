...
### Прямой D3D доступ: SetRenderState, SetTransform ✅
...
Оставшийся прямой lp_D3D_Device доступ (16 вхождений):
- 4× DrawIndPrimMM (fastprim, figure×3, aeng) — нужна ge_ обёртка для multi-matrix
- 7× PolyPage::Render(device) — Render принимает D3D device как аргумент
- 2× DrawIndexedPrimitive в aeng.cpp (LVERTEX path)
- 1× attract.cpp хранит device в локальной переменной
- 2× FORCE_SET_* в poly.cpp debug path
