...
**Результат**: hardware Weapon25 не успевает перевзводиться при быстром тапе.
- **≥ 270 мс между press'ами** — клик срабатывает стабильно.
- **200-270 мс** — серая зона, клик может быть а может и не быть.
...
DualSense. Любая стрельба с интервалом быстрее ~270 мс принципиально не
...
- Для пистолета Timer1=140 (internal units) — надо сопоставить с реальным
  временем. Если натуральный темп < 270 мс, I1 проявляется на быстрой
  стрельбе одиночными. Если ≥ 270 — вообще не проявляется.
...
- ✅ **I2 сохранён как и ожидалось** (R4-допустимо): нажатие во время
  Timer1 cooldown даёт щелчок без выстрела — hardware эмитит клик
  при пересечении зоны, `actually_fire_gun` молча отказывает по
  Timer1. Поведение консистентно keyboard-like refusal.
