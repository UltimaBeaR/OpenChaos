...
## ⚠️ Update (2026-05-23) — stage12_parity workstream закрыт как тупиковая ветвь

Работа из этого батча и весь связанный stage12_parity workstream исходили из
сравнения с кодовой базой, которую мы считали РЕЛИЗНОЙ версией исходников
MuckyFoot. Позже выяснилось что это **не релизная**, а ещё более **старая**
ветка исходников (по сути ещё более ранняя чем наша пре-релизная). То есть
"находки парности" были сравнением с более старым кодом, не с тем что
реально пошло в финальный PC-релиз.

В коммите [`9721e71eed5df1bad7ce342698216d1ef12524f5`](https://github.com/) был
откатан коммит `8628ea1358ae9ba48ff028c5018ac91f81ed4fa9` — там были снесены
правки сделанные на основании того ошибочного сравнения (95 insertions /
84 deletions в 10 файлах: `pcom.cpp`, `aeng.cpp`, `aeng_globals.{cpp,h}`,
`poly.h`, `poly_globals.cpp`, `ob.cpp`, `wmove.{cpp,h}`, `bat.cpp`).
**Возможно среди отката оказались и правки описанные в этом батче** —
точную таблицу "что именно из batch1.md осталось vs откатилось" специально
не сверяли. Описание ниже оставлено как есть для исторической записи.

Сам план `new_game_planning/stage12_parity/` (findings.md / handoff.md /
plan.md) удалён 2026-05-23 — workstream закрыт. Папка
`original_game_compare/` (которая лежала вне git) больше не нужна, ссылка на
неё в `.gitignore` тоже убрана.
