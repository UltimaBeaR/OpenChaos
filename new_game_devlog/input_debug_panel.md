...
**Требуется** `bangunsnotgames` — гейт на `allow_debug_keys`. Сначала
F9 → вводишь `bangunsnotgames` → Enter, только после этого F11
открывает панель.
...
**Keyboard page не показывает "перехваченные" клавиши.**
Репро (2026-04-17): открыть F11-панель, вкладка Keyboard, нажать
`1` / `2` / `3` — клавиши не появляются в списке "currently-held
keys". То же для всего что консьюмится другими обработчиками —
ESC (закрытие панели), TAB (переключение sub-page), F11 (toggle
panel), arrow keys (навигация).

Корень: `Keys[]` — **глобальный массив состояния клавиатуры**, а не
raw-данные с драйвера. Консьюмеры (input_debug_tick, game.cpp F11
handler, etc.) пишут `Keys[sc] = 0` чтобы "съесть" нажатие, и после
этого страница Keyboard (которая читает именно `Keys[]` через
`input_debug_key_held`) видит его как не-нажатое.

Для fix'а нужно либо (а) отдельный slot'ик "raw keys held" который
пишется только SDL-хендлерами KEYDOWN/KEYUP и панель читает его
вместо `Keys[]`, либо (б) в input_debug_tick снапшотить `Keys[]`
в local копию **до** консьюма и показывать её. Не критично,
**отложено** — пользователь это видел и знает почему так.
...
### Почему d-pad → stick override в gamepad.cpp остаётся

Первоначально была идея "убрать d-pad → stick эмуляцию совсем" —
казалось что это некорректно смешивает tank controls и аналог.
При разборе **2026-04-17** выяснилось что это не смешивание, а
штатная трансляция d-pad в движение:

- `gamepad.cpp` при нажатии d-pad кладёт в `lX/lY` экстремальные
  значения (0 / 65535) — **и** выставляет `dpad_active = true`.
- `input_actions.cpp::get_hardware_input` смотрит на `dpad_active`:
  если `true` — режим `analogue=0` (full speed / бег), если `false` —
  `analogue=1` (proportional speed от отклонения стика).
- Direction flags (`INPUT_MASK_LEFT/RIGHT/FORWARDS/BACKWARDS`)
  читаются **из осей** независимо от режима — без override'а lX/lY
  из d-pad перс бы вообще не двигался по d-pad (lX=32768 = центр).

Пользователь наблюдает что перс на d-pad и стике "по-разному бегает" —
это и есть разница `analogue=0` vs `analogue=1`. Штатное поведение,
убирать нельзя.

Единственная правка которая всё-таки нужна была для панели — скип
override'а когда панель активна, иначе точка стика прыгала в угол
при нажатии d-pad. Сделано в Iteration 8.
