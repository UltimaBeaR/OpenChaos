...
**Статус (2026-04-25):** Этапы 1-3 + частично Этап 5 сделаны и
отвалидированы (кошачий fade / F1 / F11 / консоль / pause меню /
FONT_buffer_draw сейчас рисуются в post-composition, не блюрятся
FXAA/bilinear). Этапы 4 / 6 / 7 в работе. Обнаружен бонусный баг
(фикс описан в разделе "Сделано"). Добавлен debug-флаг
`OC_DEBUG_HIGHLIGHT_NON_UI` чтобы визуально видеть границу между
UI и scene (scene тинтится магентовым + блюрится).
...
**Бонусный фикс латентного бага:** после abandon миссии
...
### Этап 4: Перенос HUD (PANEL_*) — TODO

1. В `OVERLAY_handle` оставить только `OVERLAY_draw_gun_sights`,
   `OVERLAY_draw_deferred_view_lines`, `OVERLAY_draw_enemy_health` и
   нужный setup (POLY_flush_local_rot, ge_set_viewport).
2. `PANEL_start`, `PANEL_draw_buffered`, `PANEL_last`, `PANEL_inventory`,
   `PANEL_finish` — переезжают в `ui_render_post_composition`.
3. Условия (типа `if (!EWAY_stop_player_moving())`, `if (panel)`)
   переезжают вместе.
4. Билд. Проверить весь HUD на 4:3 и 16:9:
...
### Этап 7 (опционально, после релиза): Sharp UI на render scale < 1.0

Только если нужен sharp UI при downsampling.

1. В `ui_render_post_composition`: на момент перед UI-вызовами
   временно перенастроить `PolyPage::s_XScale/s_YScale` и
   `ui_coords::g_screen_w_px/h_px` под dst rect размер (= window
   physical pixels), а не FBO размер.
2. После UI-пасса — восстановить FBO-based значения.
3. Возможно потребуется отдельный набор `ui_coords::recompute_native()`
   и `PolyPage::set_scaling_native()`.
4. Тестировать `OC_RENDER_SCALE = 0.5` + `OC_AA_ENABLE = true` — UI
   должен оставаться pixel-perfect независимо от scale.
...
| `OC_RENDER_SCALE = 0.5` + `OC_AA_ENABLE = true` | HUD блюрный (ожидаемо до Этапа 7) |
