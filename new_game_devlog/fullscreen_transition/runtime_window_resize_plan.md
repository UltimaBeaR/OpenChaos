# План: runtime-ресайз окна — ✅ DONE (исторический документ)

> **Статус: реализовано.** См. коммиты `47746276` ("larger screen resolutions:
> window is now user-resizeable") и `9b945001` ("fixes for manual user window
> resize"). Файл оставлен как историческая справка о ходе работ; для текущего
> кода консультироваться напрямую с источниками.

Задача (исходная): сделать окно игры ресайзабельным руками (юзер тянет за край OS
...
Этот файл был **handoff plan для свежего агента** (контекст разговора, где
он родился, был недоступен). Сейчас читать как архив.
