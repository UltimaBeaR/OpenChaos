...
### Отложено на следующие итерации (всё в рамках этой же задачи `camera_improvements`)

- **Indoor wall-collision** (warehouse'ы + PAP_FLAG_HIDDEN-клетки): сейчас работает плохо (zoom-in, прижатие к focus). Нужен другой sampler (WARE_inside для warehouse'ов, отдельный источник пола для HIDDEN-клеток). Гейтить по флагам нельзя — `PAP_FLAG_HIDDEN` ставится и на outdoor raised cells. Отдельная итерация.
- **Машины (CLASS_VEHICLE)** — отдельный класс геометрии, не покрыт.
- **DFacet-only объекты** (заборы, мебель — то что не представлено в MAV-сетке): возможный подход — параллельный `there_is_a_los` probe. На текущей итерации пользователь решил забить (ступеньки стало ловить как побочный эффект через MAV).
- **Скрытие модели Дарси / машины при близкой камере** — когда `d_new ≤ 0` (overshoot), камера внутри модели, near plane неизбежно режет. Отдельная фича.
...
1. ~~**Outro: чёрный экран вместо видеоролика.**~~ **Отозвано из этой задачи 2026-05-15** — пользователь перепроверил, outro работает. Однократно репродьюсилось чёрным экраном, может быть отдельный плавающий баг не от near plane. Записан как наблюдаемый post-1.0 в общих issues («Outro: чёрный экран вместо видеоролика (плавающий)»).
...
3. ~~**Тёмный полупрозрачный силуэт вокруг светящихся вывесок.**~~ **Отозвано из этой задачи 2026-05-15** — вывески висят в воздухе, не у стен, поэтому первоначальная гипотеза про sprite z-bias / блендинг со стеной не подходит. Скорее pre-existing баг рендера прозрачного спрайта не связанный с near plane. Записан как наблюдаемый post-1.0 в общих issues («Тёмный полупрозрачный силуэт вокруг светящихся вывесок»).

### Текущее состояние

- Открытые пункты Записи 13/14 закрыты на 3 из 4 (DFacet-only объекты — забили в этой итерации; остальные пункты живут как открытые итерации задачи, см. раздел «Отложено» выше).
- Indoor — wall-collision внутри зданий работает плохо; отдельная итерация в рамках этой же задачи.
- Регрессии от near plane: 1 закрыта (провода), 2 убраны из задачи (outro — плавающий, силуэты вокруг вывесок — не от near plane; обе в общие post-1.0). **Все регрессии этой итерации разобраны.**
- Задача `camera_improvements` целиком **не закончена** — остаются открытые итерации (indoor, машины, скрытие модели). Эта итерация коммитится отдельно.
