...
- [x] **Финальный аудит** — пройден. Покрыто всё основное (gameplay/menu/cinematic/dev-console/bangunsnotgames). Известные исключения задокументированы в `rules.md` секция 8: engine-level modifier sync, compile-time perf-diag keys, узкие контексты (GS_PLAYBACK, deadcivs warning) и батч ~30 остающихся легаси `bangunsnotgames` debug-keys в `game_tick.cpp:1833-2508` (отложено как отдельная мини-итерация).
