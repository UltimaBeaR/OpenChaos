...
> ## 🔧 Текущая работа: упрощённая модель `action_map/`
>
> Вместо полноценного runtime-ремапа (отложенного в `full_plan_deferred.md`)
> сейчас идёт **упрощённая** версия — все обращения к клавишам/кнопкам/осям
> в коде выносятся в семантические compile-time константы `ACT_*` в файлах
> `new_game/src/game/action_map/`. Параллельно убирается старая
> env-rebind система и gamepad→keyboard bridge.
>
> Документация и план → [`action_map/README.md`](action_map/README.md),
> [`action_map/plan.md`](action_map/plan.md), [`action_map/rules.md`](action_map/rules.md).
...
Открытые input-related в [`known_issues_and_bugs/known_issues_and_bugs.md`](../../known_issues_and_bugs/known_issues_and_bugs.md) секция "Управление" — отдельные UX/калибровочные баги, не относятся к архитектуре input pipeline (deadzone в меню для Xbox drift, sensitivity вождения, R3 inventory snapshot, и т.п.).
