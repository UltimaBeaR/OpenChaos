...
**Уточнение (пользователь):** то же зажатие анимации воспроизводится
и на **релизной PC-версии**, где на Shift **ничего не назначено**. Значит
дело НЕ в нашем биндинге Shift=ACTION — где-то есть отдельный обработчик
Shift (модификатор?), который ловит зажатый Shift во время открытого
inventory wheel и замораживает анимацию. Искать этот Shift-хендлер
(не только наш ACTION-путь): возможно legacy-модификатор (ShiftFlag) или
обработка в wheel/inventory коде, читающая Shift напрямую.
