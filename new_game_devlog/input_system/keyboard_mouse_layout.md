...
### BUG-CAM-VERT-LIMIT-DRIFT: на упоре вертикального лимита кручение мышью даёт смещение
**Воспроизведение:** на бегу подними камеру мышью максимально вверх (упор
по pitch). Продолжай двигать мышь в том же направлении — видно небольшое
смещение камеры (или подёргивание). При остановке мыши камера плавно
возвращается к центру.
**Гипотеза:** в `apply_pitch_y_delta` после clamp по pitch-углу
`eff_dy` становится 0, но что-то ещё применяется — возможно XZ-scale от
старого `old_dx/dz`, либо position smoothing подтягивает к стейлу
расхождение между want и fc. Или дельта по мыши накапливается во
внутренних state'ах между тиками.
**Где смотреть:** `fc.cpp` ~line 778+ `apply_pitch_y_delta`, проверить
что при `new_py == *py` (clamp полностью съел дельту) функция выходит
БЕЗ модификации XZ. Возможно return слишком поздно.
