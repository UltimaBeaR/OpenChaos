...
| Действие / интеракции (сесть-выйти из машины, поднять, обыск, арест, рычаги, грэпплинг) | ▢ (Square) | **F** |
...
| Действие — выйти из машины | ▢ (Square) | **F** |
...
### BUG-2 ✅ FIXED: Tab + Shift замораживает Дарси в странной позе
**Было:** Tab открывает inventory wheel; пока wheel на экране, зажатый Shift →
Дарси застывает в странной позе.
**Корень:** оригинальная фича в `process_controls` (`game_tick.cpp`, uc_orig
`Controls.cpp`): пока колесо открыто, `if (ShiftFlag)` → `set_person_gun_away()`
+ `drop_current_gun()` («Shift+Return = drop weapon»). Тройная поломка:
1. **Ground-drop мёртв:** `set_person_gun_away()` обнуляет GUN_OUT/SpecialUse
   ДО `drop_current_gun()`, поэтому тот всегда промахивается мимо обеих веток
   и ничего не спавнит — оружие только убирается в пустые руки (это же видно
   и на релизе: пустые руки, на землю ничего не падает).
2. **Фриз:** блок крутится каждый кадр пока Shift зажат → `set_person_idle()`
   каждый кадр → поза «убирания» застывает.
3. **Полупрозрачное колесо:** Shift-ветка пропускает `CONTROLS_new_inventory`
   (fade-in) → `PopupFade` не набирается → колесо еле видно.
Тот самый «отдельный Shift-хендлер» = прямое чтение `ShiftFlag` в wheel-коде.
В нашей раскладке Shift = ACTION (спринт), Tab = колесо → «спринт + Tab» бил
постоянно (в оригинале инвентарь был на Enter, Shift — strafe-модификатор,
коллизия редкая).
**Стало:** весь Shift-блок удалён (вместе с мёртвым `drop_current_gun` и его
теперь-неиспользуемым extern в game_tick.cpp). Колесо всегда делает обычный
fade-in. Ушли фриз, конфликт со спринтом, глюк прозрачного колеса и мёртвый код.
**Holster/убрать оружие** на клаве пока нет — TODO ниже (повторить геймпад:
«вниз» = убрать/переключить, вместо текущих 1-8).
...
### BUG-4 ✅ FIXED: лёгкая подкрутка модели после остановки
**Было:** разогнался WASD, навёл камеру сбоку, отпустил WASD — на остановке
моделька Дарси чуть-чуть докручивалась (еле-еле).
**Стало:** не воспроизводится. Закрыто как не-репро — видимо побочно
починилось правками по камере (перевод KBM в manual-режим / изменения
слежения камеры за персонажем).
...
### BUG-6 ✅ FIXED: в машине W + Space не работают одновременно
**Было:** в машине зажат W (газ); Space (тормоз) не реагирует пока газ зажат
(else-if в `apply_button_input_car` — газ маскировал тормоз).
**Стало:** исправлено переработкой управления машиной. Газ/тормоз/реверс
разнесены на отдельные кнопки (W=газ, Space=тормоз, S=реверс), читаются
напрямую, приоритет **тормоз > реверс > газ** (`vehicle.cpp::pedals`). else-if
убран — кнопки больше не маскируют друг друга.
...
### BUG-CAM-HANG-NEAR-CLIP → перенесён в post-1.0 (см. `../../new_game_planning/known_issues_and_bugs_post_1_0.md` → Камера)

### BUG-CAM-MMB-AIM-JERK ✅ FIXED: приближение вида (MMB) дёрганое и со смещением вбок
**Было:** на клаве вход в aim (средняя кнопка мыши) шёл резко, с одиночным
дёргом вбок; а если крутить мышью в зуме и отпустить — камеру кидало.
**Две причины:**
1. **Дёрг вбок на входе** — позиция камеры подтягивалась к OTS-позе через
   сглаживание (`>>2`), а угол на KBM снапался → рассинхрон → персонаж уезжал
   из центра на пару кадров.
2. **Кидок на выходе** — во время aim орбитальный mouse-блок в `fc.cpp`
   выключен (`!toonear`), мышь не потреблялась → дельта копилась и
   выплёскивалась рывком на выходе.
**Стало (`fc.cpp` + `input_actions.cpp`):**
- **Mouse-look в aim:** движение мыши теперь крутит персонажа (yaw) и pitch —
  как правый стик. Это попутно **потребляет** дельту мыши каждый тик → нет
  накопления → нет кидка на выходе.
- **Вход в aim = плавный разгон скорости:** камера подтягивается к OTS-позе со
  скоростью, нарастающей от `0.25/тик` (плавный зум как на геймпаде) до снапа,
  для позиции и угла в lockstep. Нет точки разрыва ease→snap → нет дёрга в
  конце; dead-zone убран → нет квантования при медленной мыши. В установившемся
  aim — снап (отзывчивый mouse-look). AUTO/геймпад не тронуты.
- Вертикаль (pitch) — не инвертирована (мышь вверх → смотрит вверх).
...
**⚠️ Позже доработано** (см. BUG-CAM-PLATFORM-MANUAL-FOLLOW): голый пропуск
ломал слежение при беге по крыше, поэтому гейт заменён на вычитание вклада
платформы (относительное слежение).

### BUG-CAM-PLATFORM-MANUAL-FOLLOW ✅ FIXED: рывки камеры при беге по крыше машины (KBM/manual)
**Было:** на клавомыши (manual) при беге по крыше машины плавное слежение
камеры за персонажем ломается — камера двигается рывками. На полу — норм.
На геймпаде (auto) — плавно и на полу, и на машине.
**Причина:** фикс BUG-CAM-PLATFORM-AUTO-ROT выше отключал 1:1 translation
tracking на платформе (`if (!fc->platform_thing)`). В manual это **единственный**
механизм слежения (get-behind в manual выключен принципиально). Гейт убивал
слежение не только за движением машины (что и хотели), но и за движением
персонажа **относительно** машины (бег по крыше) → камера не следует за бегом.
auto не страдал: там за бегом следит get-behind, который на платформе работает.
**Фикс (`fc.cpp`):** вместо пропуска — вычитаем собственный вклад платформы из
дельты фокуса. В блоке platform inheritance считается `plat_focus_dx/dz` —
насколько фокус сместился чисто из-за движения машины, **тем же `INHERIT_PT`
трансформом, что и для камеры** (тот же pivot, тот же `>>4`-prescale + `>>12`).
⚠️ `>>4` здесь — не «потеря точности», а часть Q16-масштаба SIN/COS: без него
результат раздувается ×16 (была такая ошибка в первой версии фикса — камеру
кидало на едущей машине). В manual: `want += focus_dx - plat_focus_dx`.
Стоишь на едущей машине → остаётся
≈0 (нет двойного учёта → нет авто-поворота, старый фикс сохранён); бежишь по
крыше → остаётся твой бег (камера следует). Вне платформы `plat_focus_*=0` —
обычное 1:1 слежение. auto-ветка и `s_prev_focus` не тронуты → auto «как было»,
переключение режимов корректно. Сглаживание позиции (>>2/>>3) работает всегда.

### BUG-CAM-CAR-ENTRY-NO-LOCK ✅ FIXED: при посадке/высадке камера не доворачивается
**Было:** на клаве (manual-режим) при посадке камера не доворачивалась
программно — get-behind и angle smoothing заморожены в manual.
**Стало (сценический оверрайд для входа И выхода):** введён флаг
`entering_vehicle_scene` (focus = person && SubState ENTERING/EXITING,
под общим компайл-тайм переключателем `CAR_ENTER_EXIT_CAM_LOCK`). Во
время посадки/высадки **независимо от режима камеры**:
- работает программный доворот за машину (get-behind), с обходом
  `nobehind`/fight-гейтов (чтобы орбита мышью перед посадкой не блокировала);
- угол сглаживается (rubber), а не снапается;
- включён min-clamp дистанции → камера орбитит вокруг персонажа (а не
  лерпит по прямой через центр);
- ручной ввод (мышь/стик) заблокирован, накопленное движение мыши
  дренируется (нет дёрга на снятии лока);
- кадрирование (`FC_calc_focus` yaw-офсет ±712) применяется и к выходу.
Выход доворачивается быстрее входа (~37.5%/тик vs 25%) — у выхода нет
паузы на открытие двери, камера должна успеть встать за машину раньше.
Реализовано в `fc.cpp`. Снимается автоматически на конце анимации.

### BUG-CAM-CAR-STUCK-CLOSE → перенесён в post-1.0 (см. `../../new_game_planning/known_issues_and_bugs_post_1_0.md` → Камера)
...
### BUG-CAM-SMALL-ROT-SHIFT → перенесён в post-1.0 (см. `../../new_game_planning/known_issues_and_bugs_post_1_0.md` → Камера)
...
### TODO-5: оружие на клаве — повторить геймпад вместо 1-8

Сейчас на клаве выбор оружия — цифры 1-8 (прямой выбор). План: повторить
геймпадную D-pad-схему (`controls_dpad_select_*` в `game_tick.cpp`):
- ↑ = пистолет, ← = AK47, → = дробовик, ↓ = убрать/переключить ближний бой
  (`controls_dpad_select_melee` — fist ↔ bat/knife, и «убрать оружие»).
Подобрать клавиши под эти 4 направления и переключить с 1-8 на них
(`ACT_FOOT_WEAPON_*_KKEY`). Это же закроет «убрать оружие на клаве»
(сейчас его нет — было на глючном Shift-в-колесе, удалено в BUG-2).
