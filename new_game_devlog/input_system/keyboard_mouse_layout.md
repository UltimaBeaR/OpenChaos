...
### BUG-CAM-CAR-STUCK-CLOSE: камера «залипает» близко к машине и не отъезжает
**Воспроизведение:** во время езды в какой-то момент камера оказывается
близко к машине и **больше не отъезжает** на нормальную дистанцию — едешь
с зажатой/близкой камерой. Лечится только выходом и повторной посадкой
в машину (тогда дистанция сбрасывается).
**Статус:** не расследовано. Похоже на застрявшее состояние камеры
(дистанция/clamp/wall-collision не восстанавливается), не связано напрямую
с рулём/демпфером. Возможные направления: остаточное состояние от
сценического оверрайда посадки/высадки (min-clamp дистанции не снят?), либо
wall-collision/near-clip залип на близкой дистанции. Проверить `fc.cpp`
distance clamp + снятие `entering_vehicle_scene`.
