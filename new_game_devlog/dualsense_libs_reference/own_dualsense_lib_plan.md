...
- **macOS USB вибрация не работает** (обнаружено 2026-04-17). На маке
  через BT всё ок, через USB вибрация еле чувствуется / не работает.
  Проверено и с libDualsense и со старой vendored `dualsense-multiplatform`
  — одинаковое поведение → дело не в нашем слое, скорее в output report
  или в транспорте (hidapi на macOS USB). На **Windows USB** всё ок.
  Сам контроллер не ломан — в онлайн-тестере **ds.daidr.me** (web-HID)
  на том же маке+USB вибрация нормально работает.
  **План:** посмотреть JS код ds.daidr.me (открытый проект?
  найти репо, либо отревёрсить build на сайте) — сравнить layout
  output report'ов с нашими. Подозрение что на macOS через SDL hidapi
  мы шлём либо другой report ID, либо transport padding отличается
  от того что ждёт контроллер через ОС-специфичный USB HID стек.
  Также проверить:
  - Что `device_open_first` подхватывает правильный interface number
    (macOS может репортить DualSense с отличными от Windows параметрами).
  - Что `device_write` не режет данные (полный размер 74 байт?).
  - Что output report первого пакета после connect корректный (вдруг
    на macOS USB нужен свой init packet как на BT).
  Запись в known_issues: `known_issues_and_bugs.md` раздел "Управление".
  Записан там же вопрос про Xbox USB (возможно общесистемная macOS
  проблема, не наша).
