...
- **Click rate limit ≈ 300 мс** (measured 2026-04-15, уточнено 2026-04-17).
  Если жать R2 чаще чем раз в ~300 мс, hardware Weapon25 моторчик
  ...
  визуальным контролем `last gap`. В диапазоне 200-300мс клики идут
  нерегулярно (иногда есть, иногда нет), стабильно только ≥300мс.
...
- **Эффект автоматически "рассинхронизируется" при быстрой стрельбе даже
  ...
  hardware проблема**, не наш HID race.

- **UPDATE (2026-04-17): rate limit ~300мс был артефактом vendored
  либы, а не самого железа.** Предыдущие измерения (2026-04-15) делались
  когда мы использовали `dualsense-multiplatform` с кривым Weapon25
  packing'ом. Железо получало сломанные параметры и частично
  глючило — отсюда нестабильные клики при быстрых тапах. После миграции
  на свой `libDualsense` с правильным packing'ом **клики стабильно
  срабатывают на каждом нажатии даже при максимально быстром тапе
  игрока**, без каких-либо программных трюков (mode=NONE pulse'ов,
  таймаутов и т.п.). Мотор не имеет заметного внутреннего rate limit
  когда получает валидные HID пакеты.

  **Следствие для дизайна**: PC-side логика OpenChaos не делает
  motor-reset трюков. Mode=AIM_GUN держится непрерывно пока оружие
  готово. Cooldown-gate (`fire_cooldown_ms` в `WeaponFeelProfile`)
  нужен только для синхронизации с game-side Timer1 (чтобы не было
  клика без выстрела во время cooldown'а оружия), не для мотора.
...
## Feedback-репортинг зависит от семейства эффекта

- **"Полные" эффекты репортят `act` bit и position nybble, "simple" /
  "limited" варианты — нет** (measured 2026-04-17, Windows 11 Pro через
  F11 → DualSense → TRIGGERS тестер панели отладки ввода).

  Группа A — **репортят** feedback status (обе части — act bit 4
  и nybble bits 0-3):
  - `Feedback` (0x21)
  - `Weapon` (0x25)
  - `Vibration` (0x26)
  ...
  Группа B — **НЕ репортят** feedback status (ни act, ни nybble):
  ...
  Эффекты из группы B физически отрабатывают на моторе (сопротивление
  / клик / вибрация чувствуются), но контроллер **не посылает
  диагностику** — байты 0x29/0x2A остаются в idle-состоянии как будто
  эффект выключен.

- **Функционально в группах A/B нет заметной тактильной разницы** для
  однотипных эффектов — Weapon / Simple_Weapon / Limited_Weapon на
  руках ощущаются одинаково при сравнимых параметрах. Различие
  фактически только в наличии/отсутствии `act`+nybble в input report.

  **Практическое следствие**: для любой логики которая хочет знать
  момент срабатывания эффекта (fire detection через rising edge
  `act 1→0`, прогресс-индикаторы, etc.) — нужно использовать
  full-name эффекты из группы A. Simple/Limited варианты подходят
  только если effect-active сигнал не нужен.
