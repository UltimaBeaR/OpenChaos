...
> **Обновление (позже):** путь изменён на `OpenChaos.config.json` — прямо рядом с exe, без подпапки. Папка `open_chaos/` осталась зарезервирована под будущие кастомные ресурсы, но конфиг туда больше не кладётся.
