...
// Triangle vertex index tables for each vehicle type (which prim_points build each face).
// Grouped as per-face triples of point indices within the vehicle body prim.
// uc_orig: WMOVE_get_pos (fallen/Source/wmove.cpp)
static void WMOVE_get_pos(Thing* p_thing, WMOVE_Point pos[3], SLONG number)
{
    SLONG i;
    SLONG x;
    SLONG y;
    ...
    case CLASS_VEHICLE:
    {
        ...
        */
        ...
        // Cache the rotation matrix for this vehicle on this tick.
        if (WMOVE_matrix_thing != THING_NUMBER(p_thing) || WMOVE_matrix_turn != GAME_TURN) {
...
// uc_orig: WMOVE_init (fallen/Source/wmove.cpp)
void WMOVE_init()
{
...
// uc_orig: WMOVE_create (fallen/Source/wmove.cpp)
void WMOVE_create(Thing* p_thing)
{
    ...
    for (i = 0; i < number; i++) {
        ...
        WMOVE_get_pos(p_thing, wf->last, i);
        ...
        f4->FaceFlags = FACE_FLAG_WALKABLE | FACE_FLAG_WMOVE;

        // Add slide flags for things characters can grab (only vans and platforms, not large cars).
        if (p_thing->Class != CLASS_VEHICLE || (p_thing->Class == CLASS_VEHICLE && (p_thing->Genus.Vehicle->Type == VEH_TYPE_VAN || p_thing->Genus.Vehicle->Type == VEH_TYPE_WILDCATVAN || p_thing->Genus.Vehicle->Type == VEH_TYPE_AMBULANCE))) {
            f4->FaceFlags |= FACE_FLAG_SLIDE_EDGE_0 | FACE_FLAG_SLIDE_EDGE_1 | FACE_FLAG_SLIDE_EDGE_2 | FACE_FLAG_SLIDE_EDGE_3;
        }
...
void WMOVE_process()
{
    ...
    for (i = 1; i < WMOVE_face_upto; i++) {
        ...
        // Save previous frame corners for relative_pos interpolation.
        ...
        if (p_thing->Class == CLASS_VEHICLE) {
            if (p_thing->State == STATE_DEAD) {
                if (!(p_thing->Flags & FLAGS_ON_MAPWHO)) {
                    // Car has disappeared from the map; release the face.
            ...
            // Skip update for stationary vehicles (optimization).
        ...
        WMOVE_get_pos(p_thing, now, wf->number);
    ...
    SLONG* now_dangle)
{
    ...
    */
    ...
    // Express last-frame position in the face's local (a, b) coordinate frame.
    xo = wf->last[0].x;
    yo = wf->last[0].y;
    zo = wf->last[0].z;
    ...
    SLONG rx = (last_x >> 8) - xo;
    SLONG rz = (last_z >> 8) - zo;
    ...
    // Suppress tiny vertical offsets so the character doesn't float off.
    if (abs(dy) < 0x200) {
        dy = 0;
    }

    // Apply the same (along_a, along_b) barycentric weights to the new face position.
    xo = pp0->X;
    yo = pp0->Y;
    zo = pp0->Z;
