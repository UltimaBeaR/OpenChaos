...
{
    return (p_person->Genus.Person->PersonType == PERSON_MIB1 || p_person->Genus.Person->PersonType == PERSON_MIB2 || p_person->Genus.Person->PersonType == PERSON_MIB3);
}
...
void set_person_dying(Thing* p_person, UBYTE substate)
{
    ...
    ASSERT(
        substate == SUB_STATE_DYING_ACTUALLY_DIE || substate == SUB_STATE_DYING_INITIAL_ANI || substate == SUB_STATE_DYING_FINAL_ANI || substate == SUB_STATE_DYING_KNOCK_DOWN || substate == SUB_STATE_DYING_KNOCK_DOWN_WAIT);
...
UBYTE VehicleBelongsToMIB(Thing* p_target)
{
    ...
    if ((p_target->Class != CLASS_VEHICLE) || !veh)
        return 0; // it's not even a vehicle, so it can't possibly be...

    if (veh->Driver) {
        thing = TO_THING(veh->Driver);
        if (PersonIsMIB(thing))
            return 1; // it's being driven by a MIB
    }
    passenger = veh->Passenger;
    while (passenger) {
        thing = TO_THING(passenger);
        if (PersonIsMIB(thing))
            return 1; // a MIB is riding shotgun
...
#define VAULT_DA (p_person->Genus.Person->PlayerID ? 128 : 256)
...

    {
        ...
        if (fl_height - (p_person->WorldPos.Y >> 8) > 0x50 || fr_height - (p_person->WorldPos.Y >> 8) > 0x50) {
            //
    ...
    case SUB_STATE_DYING_KNOCK_DOWN_WAIT:
        ...
        if (p_person->Genus.Person->Timer1++ > 200 - (GET_SKILL(p_person) * 8)) {
            ...
            case PERSON_ON_HIS_BACK:
                ...
                } else if (p_person->Genus.Person->PersonType == PERSON_DARCI || //&& count_gang(p_person))||
...
void fn_person_dead(Thing* p_person)
{
    ...
    */
    ...
    } else if (try_respawn) // MIB's dont respawn
    {
        SLONG vanish_time;
...
SLONG person_is_on(Thing* p_person)
{
    ...
    //	else
    {
        ...
        if (WITHIN(mx, 0, MAP_WIDTH - 1) && WITHIN(mz, 0, MAP_HEIGHT - 1)) {
            ...
            if (page == 0)
                return 255; // god this SUCKS SO HARD! sorry
