// Vehicle.cpp — redirect stub. All content migrated to src/new/actors/vehicles/vehicle.cpp
// (iterations 90-91). This file is kept only to satisfy the build system.

#include "Game.h"   // Required before vehicle.h: provides DrawTween, THING_INDEX, Thing, etc.
            ...
            for (SLONG mz = mz1; mz <= mz2; mz++) {
                for (oi = OB_find(mx, mz); oi->prim; oi++) {
                    if (VEH_col_upto >= VEH_MAX_COL)
                        return; // erk - out of room in array!

                    if (oi->y >= y + 0x180)
                        continue; // it's above us
...
static void do_car_input(Thing* p_thing)
{
    ...
    if (veh->Flags & FLAG_VEH_IN_AIR) {
        ...
        if ((veh->Dir == +2) || (veh->Dir == -2)) {
            if (!accel && (newmag < 10))
                stopped = true; // coast to a halt
        } else if ((veh->Dir == +1) || (veh->Dir == -1)) {
            if ((newmag < 10) || (newmag > oldmag))
                stopped = true; // brake to a halt
        } else {
            ASSERT(accel == 0);
            if (newmag < 10)
                stopped = true; // coast to a halt
