...
// (OpenChaos-owned minimal DualSense library).
//
// This file provides the same ds_bridge.h public API as the legacy
// ds_bridge.cpp that wraps the vendored rafaelvaloto library. At
// build time exactly one of the two .cpp files is compiled, selected
// by the OC_DUALSENSE_OWN_LIB CMake option.
...
// Game API: (start_zone=position, strength_raw, hand). Legacy code
// passes raw bytes (e.g. 20/200) that matched the rafaelvaloto
// SetResistance call, which is internally DualSense mode 0x01
// (Simple_Feedback). Map 1:1 to trigger_simple_feedback so existing
// call sites keep working identically.
...
// Game API: (start_zone, end_zone, strength, trigger_unused, hand).
// The 4th parameter exists for compatibility with the old
// SetWeapon25(start, amp, behavior, trigger, hand) signature used by
// the vendored liba — we ignore it here because the correct
// DualSense Weapon (0x25) packing only takes start/end/strength.
...
// Game API: (start, behavior_flag_unused, force_unused, amplitude,
// period, frequency, hand). The old vendored library's Machine27
// took a wider argument set; our trigger_machine follows duaLib and
// only uses start, end, amplitude_a, amplitude_b, frequency, period.
// Map amplitude→both amplitudes for a uniform pattern.
