...
// GPU character transform vertex shader — Iteration 1, Milestone 1A.
// Plan: new_game_devlog/skeletal_skinning_plan.md
//
// Replaces the per-vertex CPU transform in ge_draw_multi_matrix
// (DrawIndPrimMM, the D3D MultiMatrix extension) for the figure path.
// Pairs with the SAME fragment shader as the TL path (common_frag.glsl) —
// outputs match tl_vert.glsl exactly so the lit/fog/texture/alpha pipeline
// is reused unchanged.
//
...
//
// KEY 1:1: the math is a byte-exact replica of the CPU block in
...
// (incl. the −0.5 D3D6 pixel-center offset). Same numbers in → same pixels
// and same z-buffer value out; BUGFIX-OC-ZBUF-MISMATCH reproduced, not
// re-derived. D3D color 0xAARRGGBB is BGRA in memory → swizzle .zyxw.
...
layout(location = 1) in vec4  a_color;      // BGRA (CPU per-vertex lighting)
