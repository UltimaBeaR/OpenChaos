...
void destroy_attachments()
{
    ...
    if (s_color_tex) {
        glDeleteTextures(1, &s_color_tex);
        s_color_tex = 0;
    }
    if (s_depth_tex) {
        glDeleteTextures(1, &s_depth_tex);
        s_depth_tex = 0;
    }
...
void composition_shutdown()
{
    ...
    if (s_snap_fbo) {
        glDeleteFramebuffers(1, &s_snap_fbo);
        s_snap_fbo = 0;
    }
    if (s_snap_tex) {
        glDeleteTextures(1, &s_snap_tex);
        s_snap_tex = 0;
    }
    s_snap_w = s_snap_h = 0;
    s_u_scene = s_u_inv_scene_sz = s_u_dst_sz = s_u_dst_offset = s_u_fxaa_enabled = s_u_debug_highlight = -1;
}

bool composition_resize(int32_t scene_w, int32_t scene_h)
{
    if (scene_w == s_scene_w && scene_h == s_scene_h && s_fbo)
        return true;
    return create_attachments(scene_w, scene_h);
}
...
bool composition_window_to_fbo(int win_x, int win_y, int* fbo_x, int* fbo_y)
{
    ...
    if (win_x < s_last_dst_x || win_x >= s_last_dst_x + s_last_dst_w)
        return false;
    if (win_y < s_last_dst_y || win_y >= s_last_dst_y + s_last_dst_h)
        return false;
...
void composition_capture_last_frame(int32_t window_w, int32_t window_h)
{
    ...
    if (!s_snap_fbo)
        glGenFramebuffers(1, &s_snap_fbo);
    if (!s_snap_tex)
        glGenTextures(1, &s_snap_tex);
...
{
    if (s_snap_w <= 0 || s_snap_h <= 0 || !s_snap_fbo)
        return;
    if (window_w <= 0 || window_h <= 0)
        return;
