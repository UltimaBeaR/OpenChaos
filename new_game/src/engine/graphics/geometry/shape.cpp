...
// uc_orig: SHAPE_colour_mult (fallen/DDEngine/Source/shape.cpp)
// Component-wise multiply of two ARGB colours. Each channel = (c1[ch] * c2[ch]) >> 8.
static ULONG SHAPE_colour_mult(ULONG c1, ULONG c2)
{
    SLONG r1;
    SLONG g1;
    SLONG b1;
...
// uc_orig: SHAPE_semisphere (fallen/DDEngine/Source/shape.cpp)
    ...
    UBYTE blue)
{
    ...
    // Build two vectors orthogonal to the direction (dx,dy,dz).
    px = dy;
    py = dz;
    pz = dx;

    // (d x p) is orthogonal to d
    ax = dy * pz - dz * py >> 8;
    ay = dz * px - dx * pz >> 8;
    az = dx * py - dy * px >> 8;

    // (d x a) is orthogonal to both a and d
    bx = dy * az - dz * ay >> 8;
    by = dz * ax - dx * az >> 8;
    bz = dx * ay - dy * ax >> 8;

    // Scale all three basis vectors to 'radius' length
    bx = bx * radius >> 8;
    by = by * radius >> 8;
    bz = bz * radius >> 8;
    ...
    for (i = 0; i < c_num - 1; i++) {
        if (i == c_num - 2) {
            // Fan of triangles to the apex
            tri[2] = &POLY_buffer[c_num * c_points];
            ...
            // Quads between adjacent latitude rings
            for (j = 0; j < c_points; j++) {
                p1 = j + 0;
                p2 = j + 1;
...
// uc_orig: SHAPE_semisphere_textured (fallen/DDEngine/Source/shape.cpp)
    ...
    UBYTE blue)
{
    ...
    // Build orthogonal basis from direction
    px = dy;
    py = dz;
    pz = dx;
    ...
    for (i = 0; i < c_num; i++) {
        ...
        // UV radius at this latitude ring
...
// uc_orig: SHAPE_sphere (fallen/DDEngine/Source/shape.cpp)
void SHAPE_sphere(
    SLONG ix,
    SLONG iy,
    SLONG iz,
...
#define SHAPE_SPHERE_NUM_AROUND 8
    ...
    // Fan triangles at top and bottom poles
    for (i = 0; i < SHAPE_SPHERE_NUM_AROUND; i++) {
        p1 = i;
        p2 = i + 1;

        if (p2 == SHAPE_SPHERE_NUM_AROUND) {
            p2 = 0;
        }
        ...
        if (POLY_valid_triangle(tri)) {
            POLY_add_triangle(tri, POLY_PAGE_COLOUR, UC_FALSE);
        }
...
// uc_orig: SHAPE_sparky_line (fallen/DDEngine/Source/shape.cpp)
    ...
    float width)
{
    ...
    // Transform all spine points to screen space
    ...
    // Compute per-point screen-space normals for ribbon width
    for (i = 0; i < num_points; i++) {
        if (pp[i].IsValid()) {
            if (i == 0 && width == 20.0F) {
            ...
            if (p1 > 0 && pp[p1].IsValid()) {
                ...
                // Chebyshev length approximation (faster than sqrt)
    ...
    for (i = 0; i < num_points - 1; i++) {
        if (pp[i + 0].IsValid() && pp[i + 1].IsValid()) {
            ...
            POLY_add_quad(quad, POLY_PAGE_LITE_BOLT, UC_FALSE, true);
        }
    }
}

// uc_orig: SHAPE_glitter (fallen/DDEngine/Source/shape.cpp)
void SHAPE_glitter(
    SLONG x1,
    SLONG y1,
    SLONG z1,
    ...
    ULONG colour)
{
    ...
    // The tip extends one step past the glitter's direction
    POLY_transform(
        float(x2 + dx),
        float(y2 + dy),
        float(z2 + dz),
    ...
    if (POLY_valid_triangle(tri)) {
        ...
        POLY_add_triangle(tri, POLY_PAGE_SPARKLE, UC_FALSE, true);
    }
}

// Sets UV coordinates for a single animated frame of a tripwire.
// Selects one of two horizontal strips based on the high bit of 'counter',
// and animates vertically using the low bits.
// uc_orig: SHAPE_tripwire_uvs (fallen/DDEngine/Source/shape.cpp)
static void SHAPE_tripwire_uvs(
    UWORD counter,
    float* u1,
    float* v1,
    float* u2,
...
// uc_orig: SHAPE_tripwire (fallen/DDEngine/Source/shape.cpp)
void SHAPE_tripwire(
    SLONG ix1,
    SLONG iy1,
    SLONG iz1,
    ...
    UBYTE along)
{
    ...
    if (POLY_valid_quad(quad)) {
        ...
        // Second overlapping layer with reversed counter for a shimmer effect
        SHAPE_tripwire_uvs(
            0x2345 - counter,
            &pp[0].u, &pp[0].v,
            &pp[1].u, &pp[1].v,
...
// uc_orig: SHAPE_waterfall (fallen/DDEngine/Source/shape.cpp)
    ...
    SLONG bot)
{
    ...
    if (POLY_valid_quad(quad)) {
        POLY_add_quad(quad, POLY_PAGE_SEWATER, UC_TRUE);
    }
...
// uc_orig: SHAPE_droplet (fallen/DDEngine/Source/shape.cpp)
void SHAPE_droplet(
    SLONG x,
    SLONG y,
    SLONG z,
...
// Draws a cylindrical shadow: one screen-aligned quad fading from opaque at the base
// to transparent at the tip (length * 0.707 offset in the shadow direction).
// uc_orig: SHADOW_cylindrical_shadow (fallen/DDEngine/Source/shape.cpp)
static void SHADOW_cylindrical_shadow(float px, float py, float pz, float radius, float length)
{
    POLY_Point pp[6];
    POLY_Point* quad[4];
...
// uc_orig: SHAPE_prim_shadow (fallen/DDEngine/Source/shape.cpp)
...
#define SHAPE_PRIM_SHADOW_LENGTH (128.0F)
    ...
    case PRIM_SHADOW_FULLBOX:
        ...
        // Rotated bounding-box shadow quad
        ...
        // FALLTHROUGH to PRIM_SHADOW_BOXEDGE for the edge quads

    case PRIM_SHADOW_BOXEDGE:

        if (prim_get_shadow_type(oi->prim) == PRIM_SHADOW_FULLBOX) {
            // Fallen through from above; world[] already computed
        } else {
            pi = get_prim_info(oi->prim);
        ...
        // Choose which three corners to draw edge shadows from, based on yaw quadrant
    ...
    case PRIM_SHADOW_FOURLEGS:
        ...
        // Four cylindrical shadows at each bounding-box corner
        pi = get_prim_info(oi->prim);
...
// uc_orig: SHAPE_alpha_sphere (fallen/DDEngine/Source/shape.cpp)
    ...
    ULONG alpha)
{
    ...
    for (i = 0; i < SHAPE_SPHERE_NUM_AROUND; i++) {
        ...
        if (POLY_valid_triangle(tri)) {
            POLY_add_triangle(tri, POLY_PAGE_ALPHA, UC_FALSE);
        }
...
// uc_orig: SHAPE_draw_balloon (fallen/DDEngine/Source/shape.cpp)
void SHAPE_draw_balloon(SLONG balloon)
{
    ...
    // Draw the spring links as thin world lines
