...
CRINKLE_Handle CRINKLE_load(CBYTE* asc_filename)
{
    ...
    if (sizey < sizex && sizey < sizez) {
        // Ground crinkle: Y is the smallest dimension (flat on the ground).
        for (i = 0; i < cc->num_points; i++) {
            x = cc->point[i].vec1;
            y = cc->point[i].vec2;
            z = cc->point[i].vec3;
            ...
            cc->point[i].vec1 = x;
            cc->point[i].vec2 = z;
            cc->point[i].vec3 = y; // extrusion coordinate
        }
    } else {
        // Wall crinkle.
        for (i = 0; i < cc->num_points; i++) {
            ...
            cc->point[i].vec1 = 1.0F - x; // flipped in X and Y
            cc->point[i].vec2 = 1.0F - y;
            cc->point[i].vec3 = z;
        }
    }

    // Compute bilinear colour interpolation weights for each point.
    float v[4];
