...
static void AA_span_rhs(SLONG dydx, SLONG x1, SLONG y1, SLONG x2, SLONG y2) {
    ...
    if (x2 > x1) {
        xleft = x1; ytop = y1;
        while (1) {
            xright = xleft + AA_PIX; xright &= ~AA_PMN;
            if (xright > x2) xright = x2;
            dx = xright - xleft; dy = dx * dydx >> AA_FIX; ybot = ytop + dy;
            frac = dx * dy >> (AA_FIX + 1);
            frac += dx * (y2 - ybot) >> AA_FIX;
            if (xleft & AA_PMN) { frac += dy * (xleft & AA_PMN) >> AA_FIX; frac += (y2 - ybot) * (xleft & AA_PMN) >> AA_FIX; }
        ...
        xright = x1; ytop = y1;
        while (1) {
            xleft = xright - 1; xleft &= ~AA_PMN;
            if (xleft < x2) xleft = x2;
            dx = xright - xleft; dy = -dx * dydx >> AA_FIX; ybot = ytop + dy;
            frac = dx * dy >> (AA_FIX + 1);
            frac += dx * (ytop - y1) >> AA_FIX;
            if (xleft & AA_PMN) { frac += dy * (xleft & AA_PMN) >> AA_FIX; frac += (ytop - y1) * (xleft & AA_PMN) >> AA_FIX; }
            AA_pixel_rhs(xleft >> AA_FIX, y1 >> AA_FIX, frac);
            if (xleft <= x2) return;
            xright = xleft; ytop = ybot;
        }
    }
}
...
static void AA_span_lhs(SLONG dydx, SLONG x1, SLONG y1, SLONG x2, SLONG y2) {
    ...
    if (x2 > x1) {
        xleft = x1; ytop = y1;
        while (1) {
            xright = xleft + AA_PIX; xright &= ~AA_PMN; xpixel = xright;
            if (xright > x2) xright = x2;
            dx = xright - xleft; dy = dx * dydx >> AA_FIX; ybot = ytop + dy;
            frac_right = dx * dy >> (AA_FIX + 1); frac_right += dx * (ytop - y1) >> AA_FIX;
            if (xpixel - xright) { frac_right += dy * (xpixel - xright) >> AA_FIX; frac_right += (ytop - y1) * (xpixel - xright) >> AA_FIX; }
            frac_left = dx * dy >> (AA_FIX + 1); frac_left += dx * (y2 - ybot) >> AA_FIX;
            if (xleft & AA_PMN) { frac_left += dy * (xleft & AA_PMN) >> AA_FIX; frac_left += (y2 - ybot) * (xleft & AA_PMN) >> AA_FIX; }
        ...
        xright = x1; ytop = y1;
        while (1) {
            xleft = xright - 1; xleft &= ~AA_PMN; xpixel = xleft + AA_PIX;
            if (xleft < x2) xleft = x2;
            dx = xright - xleft; dy = dx * -dydx >> AA_FIX; ybot = ytop + dy;
            frac_right = dx * dy >> (AA_FIX + 1); frac_right += dx * (y2 - ybot) >> AA_FIX;
            if (xpixel - xright) { frac_right += dy * (xpixel - xright) >> AA_FIX; frac_right += (y2 - ybot) * (xpixel - xright) >> AA_FIX; }
            frac_left = dx * dy >> (AA_FIX + 1); frac_left += dx * (ytop - y1) >> AA_FIX;
...
static void AA_line_rhs(SLONG px1, SLONG py1, SLONG px2, SLONG py2) {
    ...
    dxdy = (dx << AA_FIX) / dy;
    if (dx) { dydx = dy << AA_FIX; dydx /= dx; } else { dydx = 0; }
    x1 = px1; y1 = py1;
    while (1) {
        y2 = y1 + AA_PIX; y2 &= ~AA_PMN;
        if (y2 > py2) y2 = py2;
        dy = y2 - y1;
        if (dy != 0) { x2 = x1 + (dy * dxdy >> AA_FIX); AA_span_rhs(dydx, x1, y1, x2, y2); }
        if (y2 >= py2) return;
        x1 = x2; y1 = y2;
    }
}
...
static void AA_line_lhs(SLONG px1, SLONG py1, SLONG px2, SLONG py2) {
    ...
    while (1) {
        ...
        if (y2 > py2) y2 = py2;
        dy = y2 - y1;
        if (dy != 0) { x2 = x1 + (dy * dxdy >> AA_FIX); AA_span_lhs(dydx, x1, y1, x2, y2); }
        if (y2 >= py2) return;
        y1 = y2; x1 = x2;
    }
}
...
static void AA_draw_do(UBYTE* bitmap, UBYTE x_res, UBYTE y_res, SLONG pitch, SLONG px[3], SLONG py[3]) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (y1 < y2) AA_line_rhs(x1, y1, x2, y2);
        else AA_line_lhs(x2, y2, x1, y1);
        p1 = p2;
    }

    fill = fill_top;
    for (y = miny; y <= maxy; y++) {
        if (y == maxy) fill = fill_bot;
        AA_Span* as = &AA_span[y];
        line = &bitmap[y * pitch];
        x = as->lhs_min;
        while (x <= as->lhs_max) {
            val = line[x]; val += as->pixel[x]; SATURATE(val, 0, 255); line[x] = (UBYTE)val; x += 1;
        }
        while (x < as->rhs_min) {
            val = line[x]; val += fill; SATURATE(val, 0, 255); line[x] = (UBYTE)val; x += 1;
        }
        while (x <= as->rhs_max) {
            val = line[x]; val += as->pixel[x]; SATURATE(val, 0, 255); line[x] = (UBYTE)val; x += 1;
        }
...
void AA_draw(UBYTE* bitmap, UBYTE x_res, UBYTE y_res, SLONG pitch,
    ...
    px[0] = p1x >> (16 - AA_FIX); py[0] = p1y >> (16 - AA_FIX);
    px[1] = p2x >> (16 - AA_FIX); py[1] = p2y >> (16 - AA_FIX);
    px[2] = p3x >> (16 - AA_FIX); py[2] = p3y >> (16 - AA_FIX);
    ...
    p1 = minp; p2 = nextp[p1]; p3 = nextp[p2];

    if (py[0] == py[1] || py[1] == py[2] || py[2] == py[0]) {
        SLONG tx[3], ty[3];
        tx[0] = px[p1]; ty[0] = py[p1];
        tx[1] = px[p2]; ty[1] = py[p2];
        tx[2] = px[p3]; ty[2] = py[p3];
        ...
        if (py[p2] > py[p3]) {
            ...
            SLONG tx[3], ty[3];
            tx[0] = px[p1]; ty[0] = py[p1]; tx[1] = nx; ty[1] = ny; tx[2] = px[p3]; ty[2] = py[p3];
            AA_draw_do(bitmap, x_res, y_res, pitch, tx, ty);
            tx[0] = px[p3]; ty[0] = py[p3]; tx[1] = nx; ty[1] = ny; tx[2] = px[p2]; ty[2] = py[p2];
            AA_draw_do(bitmap, x_res, y_res, pitch, tx, ty);
        } else {
            SLONG dx = px[p3] - px[p1], dy = py[p3] - py[p1];
            SLONG nx = px[p1] + (dx * (py[p2] - py[p1])) / dy, ny = py[p2];
            SLONG tx[3], ty[3];
            tx[0] = px[p1]; ty[0] = py[p1]; tx[1] = px[p2]; ty[1] = py[p2]; tx[2] = nx; ty[2] = ny;
            AA_draw_do(bitmap, x_res, y_res, pitch, tx, ty);
            tx[0] = nx; ty[0] = ny; tx[1] = px[p2]; ty[1] = py[p2]; tx[2] = px[p3]; ty[2] = py[p3];
            AA_draw_do(bitmap, x_res, y_res, pitch, tx, ty);
        }
    }
}
