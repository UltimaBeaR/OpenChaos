...
// Writes the IMP_Mesh header followed by each dynamic array.
...
SLONG IMP_binary_save(CBYTE* fname, IMP_Mesh* im)
{
    ...
    if (fwrite(im, sizeof(IMP_Mesh), 1, handle) != 1)
        goto file_error;
    if (fwrite(im->mat, sizeof(IMP_Mat), im->num_mats, handle) != im->num_mats)
        goto file_error;
    if (fwrite(im->vert, sizeof(IMP_Vert), im->num_verts, handle) != im->num_verts)
        goto file_error;
    if (fwrite(im->tvert, sizeof(IMP_Tvert), im->num_tverts, handle) != im->num_tverts)
        goto file_error;
    if (fwrite(im->face, sizeof(IMP_Face), im->num_faces, handle) != im->num_faces)
        goto file_error;
    if (fwrite(im->svert, sizeof(IMP_Svert), im->num_sverts, handle) != im->num_sverts)
        goto file_error;
    if (fwrite(im->quad, sizeof(IMP_Quad), im->num_quads, handle) != im->num_quads)
        goto file_error;
    if (fwrite(im->edge, sizeof(IMP_Edge), im->num_edges, handle) != im->num_edges)
        goto file_error;
    if (fwrite(im->line, sizeof(IMP_Line), im->num_lines, handle) != im->num_lines)
        goto file_error;
...
IMP_Mesh IMP_binary_load(CBYTE* fname)
{
    ...
    if (fread(&ans, sizeof(IMP_Mesh), 1, handle) != 1)
        goto file_error;
    ...
    if (fread(ans.mat,   sizeof(IMP_Mat),   ans.num_mats,   handle) != ans.num_mats)
        goto file_error;
    if (fread(ans.vert,  sizeof(IMP_Vert),  ans.num_verts,  handle) != ans.num_verts)
        goto file_error;
    if (fread(ans.tvert, sizeof(IMP_Tvert), ans.num_tverts, handle) != ans.num_tverts)
        goto file_error;
    if (fread(ans.face,  sizeof(IMP_Face),  ans.num_faces,  handle) != ans.num_faces)
        goto file_error;
    if (fread(ans.svert, sizeof(IMP_Svert), ans.num_sverts, handle) != ans.num_sverts)
        goto file_error;
    if (fread(ans.quad,  sizeof(IMP_Quad),  ans.num_quads,  handle) != ans.num_quads)
        goto file_error;
    if (fread(ans.edge,  sizeof(IMP_Edge),  ans.num_edges,  handle) != ans.num_edges)
        goto file_error;
    if (fread(ans.line,  sizeof(IMP_Line),  ans.num_lines,  handle) != ans.num_lines)
        goto file_error;
