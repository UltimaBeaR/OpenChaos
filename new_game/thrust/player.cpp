...
SLONG PLAYER_process(SLONG* rollback, SLONG ignore_server_messages)
{
    ...
    *rollback = PLAYER_key_gameturn;
    ...
    if (KEY_on[KEY_Z]) {
        local |= PLAYER_KEY_LEFT;
    }
    if (KEY_on[KEY_X]) {
        local |= PLAYER_KEY_RIGHT;
    }
    if (KEY_on[KEY_V]) {
        local |= PLAYER_KEY_THRUST;
    }
    ...
    for (i = 0; i < PLAYER_MAX_PLAYERS; i++) {
        ...
        if (ss->flag & SHIP_FLAG_USED) {
            ...
            if (gameturn < ss->active) {
                ...
                if (pk->key[gameturn & (PLAYER_NUM_KEYS - 1)] & PLAYER_KEY_LEFT) {
                    ss->flag |= SHIP_FLAG_KEY_LEFT;
                }
                if (pk->key[gameturn & (PLAYER_NUM_KEYS - 1)] & PLAYER_KEY_RIGHT) {
                    ss->flag |= SHIP_FLAG_KEY_RIGHT;
                }
                if (pk->key[gameturn & (PLAYER_NUM_KEYS - 1)] & PLAYER_KEY_THRUST) {
                    ss->flag |= SHIP_FLAG_KEY_THRUST;
                }
                if (pk->key[gameturn & (PLAYER_NUM_KEYS - 1)] & PLAYER_KEY_TRACTOR_BEAM) {
                    ss->flag |= SHIP_FLAG_KEY_TRACTOR_BEAM;
                }
            }
        }
