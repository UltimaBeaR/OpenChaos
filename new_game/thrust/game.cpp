...
GAME_Menu GAME_menu_top = {
    ...
    { { "Find LAN Game",
          &GAME_menu_join,
          GAME_MENU_ACTION_MAKE_LAN_CONNECTION | GAME_MENU_ACTION_ENUMERATE_LAN_SESSIONS },
    ...
    while (1) {
        ...
        if (!error) {
            if (text) {
                ...
                if (KEY_on[KEY_RETURN] || KEY_on[KEY_ENTER]) {
                    KEY_on[KEY_RETURN] = 0;
                    KEY_on[KEY_ENTER] = 0;
                ...
                if (KEY_on[KEY_UP] || KEY_on[KEY_DOWN]) {
                    //
...
#define GAME_MAX_SESSION_CHOICE_BUFFER 8192
        ...
        while (gm->item[i].name) {
            ...
            } else if (gm->item[i].action & GAME_MENU_ACTION_MAKE_CHOICE) {
                FONT_format(FONT_FLAG_JUSTIFY_RIGHT);
