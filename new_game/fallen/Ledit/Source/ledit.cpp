...
void LEDIT_sync_colours()
{
    ...
    case LEDIT_MODE_SET_LAMPOST:
        ...
        if (signed_red < 0 || signed_green < 0 || signed_blue < 0) {
            //
...
#define MAX_COLRECTS 8
    ...
    for (i = 0; i < colrect_upto; i++) {
        ...
        if (rect->top >= colrect[i].bottom || rect->bottom <= colrect[i].top || rect->left >= colrect[i].right || rect->right <= colrect[i].left) {
            //
            // No overlap.
            //
        } else {
...
void LEDIT_process()
{
    ...
    if (WITHIN(mouse.x, clientrect.left, clientrect.right) && WITHIN(mouse.y, clientrect.top, clientrect.bottom)) {
        ...
        if (LEDIT_insides) {
            ...
            if (ei)
                el = &ED_light[ei]; // else TRACE("no ei\n");
            if (el)
                INDOORS_INDEX = calc_inside_for_xyz(el->x, el->y, el->z, &INDOORS_ROOM); // else TRACE("no el\n");
        }
        if (INDOORS_INDEX) {
        ...
        for (i = 1; i < ED_MAX_LIGHTS; i++) {
            ...
            if (el->used) {
                if (LEDIT_mode == LEDIT_MODE_EDIT_LIGHT && LEDIT_edit_light == i) {
                    if (el->red < 0 || el->green < 0 || el->blue < 0) {
    ...
    if (LEDIT_mouse_valid && LEDIT_mode == LEDIT_MODE_EDIT_LIGHT && LEDIT_mouse_over == GI_MOUSE_OVER_LIGHT_BOT) {
    ...
    LPARAM param_l)
{
    ...
    if (message_type == LEDIT_wm_mousewheel || message_type == WM_MOUSEWHEEL) {
        ...
        if (LEDIT_mode == LEDIT_MODE_EDIT_LIGHT && LEDIT_edit_light != NULL) {
            ASSERT(WITHIN(LEDIT_edit_light, 1, ED_MAX_LIGHTS - 1));
            ...
            LEDIT_make_undoable();
        } else if (LEDIT_mode == LEDIT_MODE_PLACE_LIGHT && LEDIT_last_placed != NULL) {
            //
    ...
    case WM_KILLFOCUS:
        ...
        if ((HWND)param_w == LEDIT_handle_red || (HWND)param_w == LEDIT_handle_green || (HWND)param_w == LEDIT_handle_blue || (HWND)param_w == LEDIT_handle_range || (HWND)param_w == LEDIT_handle_bright) {
    ...
    LPARAM param_l)
{
    ...
    case WM_MOUSEMOVE:
        ...
        if (LEDIT_mode == LEDIT_MODE_EDIT_LIGHT && LEDIT_edit_light != NULL && LEDIT_edit_dragging) {
            ASSERT(WITHIN(LEDIT_edit_light, 1, ED_MAX_LIGHTS - 1));
    ...
    LPARAM param_l)
{
    ...
    case WM_VSCROLL:
        ...
        if (LEDIT_mode == LEDIT_MODE_EDIT_LIGHT || LEDIT_mode == LEDIT_MODE_SET_LAMPOST) {
            //
            // Change what we are editing.  Only create an undo
            // packet at the end of the dragging of the trackbar- i.e.
            // a TB_THUMPOSITION message.
