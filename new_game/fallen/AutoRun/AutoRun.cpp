    ...
    int nCmdShow)
{
    ...
    if (szAutorunDir[len - 1] == '\\')
        szAutorunDir[len - 1] = '\0';
...
static void OnMouseMove(HWND hWnd, int x, int y)
{
    ...
    MenuItem* item = pMenu->Item;
    while (item) {
        if ((x >= item->Area.left) && (x <= item->Area.right) && (y >= item->Area.top) && (y <= item->Area.bottom)) {
            over = item;
            break;
        }
        item = item->Next;
...
static void OnLButtonDown(HWND hWnd, int x, int y)
{
    ...
    if (!MacroReplace(doc, document, MAX_PATH))
        return;
    if (!MacroReplace(dir, directory, MAX_PATH))
        return;
...
static bool MacroReplace(TCHAR* cmd, TCHAR* buffer, UINT blen)
{
    while (*cmd) {
        if (*cmd == '$') {
            ...
            if (!strnicmp(cmd, "src", cptr - cmd))
                replace = szAutorunDir;
            else if (!strnicmp(cmd, "dst", cptr - cmd))
                replace = pDirector->AppPath;
