...
void BreakFrame()
{
    if (enabled) {
        ...
        if (!max_break)
            max_break = next_break;
        else
            ASSERT(max_break == next_break);
        ...
        for (size_t ii = 1; ii <= next_break; ii++) {
            ...
            if (dsec < breakpoint[ii].min_sec)
                breakpoint[ii].min_sec = dsec;
            if (dsec > breakpoint[ii].max_sec)
                breakpoint[ii].max_sec = dsec;
        ...
        if (dsec < breakpoint[0].min_sec)
            breakpoint[0].min_sec = dsec;
        if (dsec > breakpoint[0].max_sec)
            breakpoint[0].max_sec = dsec;
