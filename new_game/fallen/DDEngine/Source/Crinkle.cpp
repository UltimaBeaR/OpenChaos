...
CRINKLE_Handle CRINKLE_load(CBYTE* asc_filename)
{
    ...
    for (i = 0; i < cc->num_points; i++) {
        if (cc->point[i].vec1 < minx) {
            minx = cc->point[i].vec1;
        }
        if (cc->point[i].vec2 < miny) {
            miny = cc->point[i].vec2;
        }
        if (cc->point[i].vec3 < minz) {
            minz = cc->point[i].vec3;
        }

        if (cc->point[i].vec1 > maxx) {
            maxx = cc->point[i].vec1;
        }
        if (cc->point[i].vec2 > maxy) {
            maxy = cc->point[i].vec2;
        }
        if (cc->point[i].vec3 > maxz) {
            maxz = cc->point[i].vec3;
        }
    }
