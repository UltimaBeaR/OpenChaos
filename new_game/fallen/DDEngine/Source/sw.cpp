...
SLONG ALWAYS_recip_slow(SLONG x, CBYTE* file, SLONG line)
{
    if (x <= -ALWAYS_HOW_MANY_RECIPS_MINUS || x >= ALWAYS_HOW_MANY_RECIPS) {
        ASSERT(0);
...
SLONG ALWAYS_recippt_slow(SLONG x, CBYTE* file, SLONG line)
{
    if (x < 0 || x >= ALWAYS_HOW_MANY_RECIPPT) {
        ASSERT(0);
        ...
        for (y = 0; y < size; y++) {
            ...
            for (dx = -1; dx <= +1; dx++)
                for (dy = -1; dy <= +1; dy++) {
                    ...
                    if (WITHIN(px, 0, size - 1) && WITHIN(py, 0, size - 1)) {
                        static SLONG mul[3] = {
                            664,
                            260,
                            150,
...
void SW_reload_textures()
{
    ...
    for (i = 0; i < SW_MAX_TEXTURES; i++) {
        ...
        if (flat_page_hack) {
        ...
        } else if (TEXTURE_texture[tt_index].Type == D3DTEXTURE_TYPE_UNUSED) {
            //
            ...
            if (strcmp(TEXTURE_texture[tt_index].texture_name, st->name) == 0) {
                ...
                if (!ti.valid) {
                    ...
                    if (i < 64 * 22) {
                        if (RealDisplayWidth < 640 || RealDisplayHeight < 480) {
                            //
                            // Half the size of the texture!
                            //
    ...
    if (dax < dbx) {
        ...
        while (y < y2) {
            if (y >= 0 && y <= SW_buffer_height - 1) {
                ...
                if (xl + w > SW_buffer_width - 1 + 1) {
                    w = SW_buffer_width - 1 + 1 - xl;
                }
        ...
        while (y < y2) {
            if (y >= SW_render_top && y < SW_render_bot) {
                ...
                if (w > 0) {
                    ...
                    } else if (mode == SW_MODE_ADDITIVE_NOZ) {
...
void SW_copy_to_bb()
{
    ...
    if (the_display.screen) {
        ...
        if (the_display.screen_bbp == 32) {
            ...
            */
        } else if (the_display.screen_bbp == 16) {
            SW_buffer = SW_buffer_memory;
