    ...
    */
    for (i = 2000, pp = params.particles; i; i--, pp++)
        if (pp->life) {
            pos.ofs = pp->pos.ofs;
            if (pp->jx)
                pos.loc.x += FlameRand(pp->jx) - (pp->jx >> 1);
            if (pp->jy)
                pos.loc.y += FlameRand(pp->jy) - (pp->jy >> 1);
            pt = data;
            pt += pos.ofs;
            *pt = (UBYTE)pp->pulse;
            switch (pp->pmode) {
            case 0: // Ramp up
                pp->pulse += pp->prate;
                if ((ULONG)pp->pulse >= pp->pend)
                    pp->pulse = (SWORD)pp->pstart;
                break;
            case 1: // Ramp down
                pp->pulse -= pp->prate;
                if ((ULONG)pp->pulse <= pp->pstart)
                    pp->pulse = (SWORD)pp->pend;
                break;
            ...
            case 2: // cascade
                pp->pos.loc.y += 1 + (i & 3);
                if ((pp->pos.loc.y == 255) || (pp->pos.loc.y < pp->ey))
                    pp->pos.loc.y = pp->ey;
                break;
            case 3: // gravity
                pp->pos.loc.y++;
