...
void AENG_draw_ns()
{
    ...
    for (i = 1; i < NS_MAX_CACHES; i++) {
        ...
        if (nc->used) {
            ...
            if (!WITHIN(nc->map_z, NGAMUT_lo_zmin, NGAMUT_lo_zmax) || !WITHIN(nc->map_x, NGAMUT_lo_gamut[nc->map_z].xmin, NGAMUT_lo_gamut[nc->map_z].xmax)) {
                //
                ...
                if (!(p_thing->Flags & FLAGS_IN_SEWERS)) {
                    ...
                    if (p_thing->DrawType == DT_ROT_MULTI) {
                        if (POLY_sphere_visible(
                            ...
                            if (NS_HI_TYPE(nh) != NS_HI_TYPE_ROCK && NS_HI_TYPE(nh) != NS_HI_TYPE_CURVE && nh->water) {
                                //
