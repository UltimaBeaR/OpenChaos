    ...
    while (queue_start < queue_end) {
        ...
        if (eh->type == ES_TYPE_SEWER || ShiftFlag) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                if (WITHIN(nx, 0, PAP_SIZE_HI - 1) && WITHIN(nx, 0, PAP_SIZE_HI - 1)) {
                    if (ES_hi[nx][nz].type == eh->type && ES_hi[nx][nz].height == match) {
...
void ES_build_sewers()
{
    ...
    for (x = 0; x < PAP_SIZE_HI; x++)
        for (z = 0; z < PAP_SIZE_HI; z++) {
            ...
            switch (eh->type) {
            case ES_TYPE_ROCK:
                nh_type = NS_HI_TYPE_ROCK;
                break;
            case ES_TYPE_SEWER:
                nh_type = NS_HI_TYPE_SEWER;
                break;
            case ES_TYPE_GROUND:
                nh_type = NS_HI_TYPE_STONE;
                break;
            case ES_TYPE_HOLE:
                nh_type = NS_HI_TYPE_STONE;
                break;
    ...
    for (x = 1; x < PAP_SIZE_HI - 1; x++)
        for (z = 1; z < PAP_SIZE_HI - 1; z++) {
            ...
            if (eh[PAP_SIZE_HI * 0 - 0].type == ES_TYPE_ROCK) {
                this_top_count += 1;
                this_top += eh[PAP_SIZE_HI * 0 - 0].height;
            }
            if (eh[PAP_SIZE_HI * 0 - 1].type == ES_TYPE_ROCK) {
                this_top_count += 1;
                this_top += eh[PAP_SIZE_HI * 0 - 1].height;
            }
            if (eh[PAP_SIZE_HI * -1 - 0].type == ES_TYPE_ROCK) {
                this_top_count += 1;
                this_top += eh[PAP_SIZE_HI * -1 - 0].height;
            }
            if (eh[PAP_SIZE_HI * -1 - 1].type == ES_TYPE_ROCK) {
                this_top_count += 1;
                this_top += eh[PAP_SIZE_HI * -1 - 1].height;
            }
        ...
        for (z = 0; z < PAP_SIZE_LO; z++) {
            ...
            if (pl->ColVectHead) {
                ...
                for (f_list = pl->ColVectHead, exit = FALSE; !exit; f_list += 1) {
                    ...
                    if (dfacets[facet].FacetType == STOREY_TYPE_LADDER && dfacets[facet].FacetType == FACET_FLAG_IN_SEWERS) {
                        //
                        // Take out this facet.
                        //
    ...
    for (i = 1; i < next_dfacet; i++) {
        if (dfacets[i].FacetType == STOREY_TYPE_LADDER && dfacets[i].FacetType == FACET_FLAG_IN_SEWERS) {
            //
            // Take out this facet.
            //
...
SLONG ES_save(CBYTE* filename)
{
    ...
    if (fwrite(&ed, sizeof(ES_Header), 1, handle) != 1)
        goto file_error;
    if (fwrite(&ES_loadsave, sizeof(ES_Undo), 1, handle) != 1)
        goto file_error;
...
SLONG ES_load(CBYTE* filename)
{
    ...
    if (fread(&ed, sizeof(ES_Header), 1, handle) != 1)
        goto file_error;
    ...
    if (ed.version != 2 || ed.sizeof_es_undo != sizeof(ES_Undo)) {
        //
        // Incompatible!
        //
    ...
    if (fread(&ES_loadsave, sizeof(ES_Undo), 1, handle) != 1)
        goto file_error;
...
void ES_draw_editor(
    ...
    SLONG prim_yaw)
{
    if (!WITHIN(mouse_x, 0, 640) || !WITHIN(mouse_y, 0, 480)) {
        mouse_x = INFINITY;
        mouse_y = INFINITY;
    }
...
void ES_light_move(SLONG x, SLONG z)
{
    ...
    if (WITHIN(lo_map_x, 0, PAP_SIZE_LO - 1) && WITHIN(lo_map_z, 0, PAP_SIZE_LO - 1)) {
        //
        // Move the light.
        //
...
#define QUEUE_SIZE 512
    ...
    while (queue_start < queue_end) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (WITHIN(nx, 0, PAP_SIZE_HI - 1) && WITHIN(nz, 0, PAP_SIZE_HI - 1)) {
