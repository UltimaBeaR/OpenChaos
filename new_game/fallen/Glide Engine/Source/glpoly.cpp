    ...
    float radius)
{
    ...
    if (view_z + radius <= POLY_ZCLIP_PLANE || view_x - radius >= +view_z || view_x + radius <= -view_z || view_y - radius >= +view_z || view_y + radius <= -view_z) {
        return FALSE;
    } else {
        return TRUE;
    }
...
#define MAX_UV 1024.0F
    ...
    for (i = 0; i < 3; i++) {
        if (pp[i]->X < -MAX_OFFSCREEN || pp[i]->X > +MAX_OFFSCREEN + 480.0F || pp[i]->Y < -MAX_OFFSCREEN || pp[i]->Y > +MAX_OFFSCREEN + 640.0F) {
            return;
        }

        if (pp[i]->u < -MAX_UV || pp[i]->u > +MAX_UV || pp[i]->v < -MAX_UV || pp[i]->v > +MAX_UV) {
