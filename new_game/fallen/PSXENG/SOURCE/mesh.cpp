    ...
    UBYTE fade)
{
    ...
    */
    ...
    for (i = sf; i < ef; i++) {
        ...
        pp[0].World.vx = prim_points[p0].X;
        pp[0].World.vy = prim_points[p0].Y;
        pp[0].World.vz = prim_points[p0].Z;
        pp[1].World.vx = prim_points[p1].X;
        pp[1].World.vy = prim_points[p1].Y;
        pp[1].World.vz = prim_points[p1].Z;
        pp[2].World.vx = prim_points[p2].X;
        pp[2].World.vy = prim_points[p2].Y;
        pp[2].World.vz = prim_points[p2].Z;
        pp[3].World.vx = prim_points[p3].X;
        pp[3].World.vy = prim_points[p3].Y;
        pp[3].World.vz = prim_points[p3].Z;
    ...
    for (i = sf; i < ef; i++) {
        ...
        if ((clip_and & (1 << 31)) == 0)
            if ((draw_flags & POLY_FLAG_DOUBLESIDED) || MF_NormalClip(pp[0].SYSX, pp[1].SYSX, pp[2].SYSX) < 0) {
                if (draw_flags & POLY_FLAG_TEXTURED) {
                    if ((max(pp[0].Word.SX, max(pp[1].Word.SX, pp[2].Word.SX)) >= 0) && (min(pp[0].Word.SX, min(pp[1].Word.SX, pp[2].Word.SX)) < DISPLAYWIDTH) && (max(pp[0].Word.SY, max(pp[1].Word.SY, pp[2].Word.SY)) >= 0) && (min(pp[0].Word.SY, min(pp[1].Word.SY, pp[2].Word.SY)) < 256)) {
                        POLY_FT3* p;
