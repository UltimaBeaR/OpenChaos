        ...
        */
        ...
        while (c0-- > 0) {
            ...
            if ((b0 > 0) && !((pp[0].Flag & pp[1].Flag & pp[2].Flag & pp[3].Flag) & (1 << 31))) {
                if ((MIN4(pp[0].Word.SX, pp[1].Word.SX, pp[2].Word.SX, pp[3].Word.SX) < DISPLAYWIDTH) && (MAX4(pp[0].Word.SX, pp[1].Word.SX, pp[2].Word.SX, pp[3].Word.SX) >= 0) && (MIN4(pp[0].Word.SY, pp[1].Word.SY, pp[2].Word.SY, pp[3].Word.SY) < 256) && (MAX4(pp[0].Word.SY, pp[1].Word.SY, pp[2].Word.SY, pp[3].Word.SY) >= 0)) {
                    POLY_GT4* p;
                    SLONG page;
...
#define QSET_TX_TY(p, x1, y1, x2, y2, x3, y3, x4, y4, c, pal) \
    ((ULONG*)p)[3] = ((x1) << 0) | ((y1) << 8) | (c << 16);   \
    ((ULONG*)p)[6] = ((x2) << 0) | ((y2) << 8) | (pal << 16); \
    ((ULONG*)p)[9] = ((x3) << 0) | ((y3) << 8);               \
    ((ULONG*)p)[12] = ((x4) << 0) | ((y4) << 8);
#ifdef FOUR
...
#define dv2 (p[3].World.vy)
    ...
    for (c0 = 0; c0 < 4; c0++) {
        ...
        case 0:
            ...
            setUV4(f4, q->u0, q->v0, q->u1, q->v1, q->u0 + du1, q->v0 + dv1, q->u1 + du1, q->v1 + dv1);
        ...
        case 3:
            setXY4(f4, p[4].Word.SX, p[4].Word.SY,
                ...
                pp[2].Word.SX, pp[2].Word.SY,
                pp[3].Word.SX, pp[3].Word.SY);
            setUV4(f4, q->u2 - du1, q->v2 - dv1, q->u3 - du2, q->v3 - dv2, q->u2, q->v2, q->u3, q->v3);
...
void FACET_draw_quick(SLONG facet)
{
    ...
    case STOREY_TYPE_NORMAL:
        ...
        //			if(pp[0].Flag&(1<<31)==0)
        if ((MAX4(pp[0].Word.SX, pp[1].Word.SX, pp[2].Word.SX, pp[3].Word.SX) > 0) || (MIN4(pp[0].Word.SX, pp[1].Word.SX, pp[2].Word.SX, pp[3].Word.SX) < DISPLAYWIDTH) || (MAX4(pp[0].Word.SY, pp[1].Word.SY, pp[2].Word.SY, pp[3].Word.SY) > 0) || (MIN4(pp[0].Word.SY, pp[1].Word.SY, pp[2].Word.SY, pp[3].Word.SY) < 256)) {
            //
...
{
    {
        ...
        if (out1.vz < 210) {
            ...
            if (out2.vz < 210) {
                ...
                //
                return (0);
            } else if (out2.vz > 210 && out1.vz < -200) // first point should be at least a block away to bother with this
            {
                //
...
void pre_process_facets(void)
{
    ...
    for (c0 = 1; c0 < next_dfacet; c0++) {
        ...
        */
        ...
        if ((p_facet->FacetType == STOREY_TYPE_FENCE || p_facet->FacetType == STOREY_TYPE_FENCE_BRICK || p_facet->FacetType == STOREY_TYPE_INSIDE || p_facet->FacetType == STOREY_TYPE_INSIDE_DOOR || p_facet->FacetType == STOREY_TYPE_LADDER || p_facet->FacetType == STOREY_TYPE_FENCE_FLAT)
...
void FACET_draw(SLONG facet, UBYTE fade_alpha)
{
    ...
    if ((p_facet->FacetType == STOREY_TYPE_FENCE || p_facet->FacetType == STOREY_TYPE_FENCE_BRICK || p_facet->FacetType == STOREY_TYPE_INSIDE ||
            //		p_facet->FacetType == STOREY_TYPE_INSIDE_DOOR ||
            p_facet->FacetType == STOREY_TYPE_LADDER || p_facet->FacetType == STOREY_TYPE_OUTSIDE_DOOR || p_facet->FacetType == STOREY_TYPE_DOOR || p_facet->FacetType == STOREY_TYPE_FENCE_FLAT)
        && !(p_facet->FacetFlags & FACET_FLAG_2SIDED)) {
        //
...
void DRAW_ladder_rungs(SLONG x1, SLONG z1, SLONG x2, SLONG z2, struct DFacet* p_facet, SLONG dx, SLONG dz, int backwards)
{
    ...
    while (count-- && (clip_flag < 2)) {
        ...
        if ((b0 == 0) || (flag & (1 << 31)) || (MAX4(pp->x0, pp->x1, pp->x2, pp->x3) < 0) || (MIN4(pp->x0, pp->x1, pp->x3, pp->x3) > 511) || (MAX4(pp->y0, pp->y1, pp->y2, pp->y3) < 0) || (MIN4(pp->y0, pp->y1, pp->y2, pp->y3) > 255)) {
        ...
        for (i = sp; i < ep; i++, rooftex++) {
            ...
            if (!(p_f4->DrawFlags & RFACE_FLAG_NODRAW))
                if (p_f4->Y <= max_y) {
                    ...
                    if (!(flag & (1 << 31)) && (MAX4(pp[0].Word.SX, pp[1].Word.SX, pp[2].Word.SX, pp[3].Word.SX) >= 0) && (MAX4(pp[0].Word.SY, pp[1].Word.SY, pp[2].Word.SY, pp[3].Word.SY) >= 0) && (MIN4(pp[0].Word.SX, pp[1].Word.SX, pp[2].Word.SX, pp[3].Word.SX) < DISPLAYWIDTH) && (MIN4(pp[0].Word.SY, pp[1].Word.SY, pp[2].Word.SY, pp[3].Word.SY) < 256)) {
