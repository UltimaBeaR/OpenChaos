...
void BLOOM_draw(SLONG x, SLONG y, SLONG z, SLONG dx, SLONG dy, SLONG dz, SLONG col, UBYTE opts)
{
    ...
    if (opts & BLOOM_BEAM) {
        ...
        if (((pp[0].Flag | pp[1].Flag) & (1 << 31)) == 0)
            if ((max(pp[0].Word.SX, pp[1].Word.SX) > 0) && (min(pp[0].Word.SX, pp[1].Word.SX) < DISPLAYWIDTH) && (max(pp[0].Word.SY, pp[1].Word.SY) > 0) && (min(pp[0].Word.SY, pp[1].Word.SY) < 256))
            ...
            if (pyro->counter < 10) {
                for (d = 0; d < 4; d++) {
                    if ((pyro->Flags & PYRO_FLAGS_WAVE) && (pyro->counter < 6)) {
                        x = SIN((d << 7) + (Random() & 127)) >> 3;
                        z = COS((d << 7) + (Random() & 127)) >> 3;
...
void PYRO_draw_dustwave(Pyro* pyro)
{
    ...
    //	for(sector=0;sector<DUSTWAVE_SECTORS;sector++)
    {
        ...
        shock_sector++;
        if (shock_sector == DUSTWAVE_SECTORS)
            shock_sector = 0;
    }
}
...
void PYRO_draw_armageddon(Pyro* pyro)
{
    ...
    if (!(Random() & 2)) {
        ...
        if (pow_type == POW_TYPE_UNUSED)
            pow_type = POW_TYPE_SPAWN_SPHERE_LARGE;
