            ...
            if (file_handle) {
                ...
                for (c0 = 0; c0 < MAX_EVENTPOINTS; c0++) {
                    ...
                    if (temp_mission->EventPoints[c0].WaypointType == WPT_CREATE_ENEMIES) {
                        ...
                        if (!skill)
                            skill = current_mission->SkillLevels[ai];
...
#define MARK_REFERENCED(w)                                              \
    {                                                                   \
        if (WITHIN((w), 1, MAX_EVENTPOINTS - 1)) {                      \
            temp_mission->EventPoints[w].Flags |= WPT_FLAGS_REFERENCED; \
        }                                                               \
    }
...
BOOL SingleFlagCheck(SLONG test)
{
    ...
    for (i = 0; i < 32; i++) {
        if (test & (1 << i)) {
            if (gotone)
                return FALSE; // more than one of em
            gotone = 1;
        }
    }
    return (BOOL)gotone;
...
BOOL valid_ep(EventPoint* ep)
{
    ...
    if ((WaypointUses[ep->TriggeredBy] & WPU_DEPEND) && !ep->EPRef)
        return FALSE;
    ...
    if ((WaypointUses[ep->TriggeredBy] & WPU_BOOLEAN) && !ep->EPRefBool) {
        ...
        if (ep->TriggeredBy != TT_PERSON_SEEN && ep->TriggeredBy != TT_PERSON_IN_VEHICLE) {
            return FALSE;
        }
    }
    ...
    if (ep->TriggeredBy == TT_PERSON_IN_VEHICLE) {
        ...
        if (current_mission->EventPoints[ep->EPRef].WaypointType != WPT_CREATE_PLAYER && current_mission->EventPoints[ep->EPRef].WaypointType != WPT_CREATE_ENEMIES) {
            return FALSE;
        }
    ...
    if (WaypointUses[ep->TriggeredBy] & WPU_COUNTER) {
        if ((!ep->Radius) || (ep->EPRef < 1) || (ep->EPRef > 10))
            return FALSE; // trigger would go off immediately, or bad counter num
    }
    if ((WaypointUses[ep->TriggeredBy] & WPU_RADTEXT) && !ep->Radius)
        return FALSE;
    ...
    if (ep->TriggeredBy == TT_ITEM_HELD) {
        if (!ep->EPRef)
            return FALSE;
        if (current_mission->EventPoints[ep->EPRef].WaypointType == WPT_CREATE_ITEM)
            return TRUE;
        if (current_mission->EventPoints[ep->EPRef].WaypointType == WPT_CREATE_ENEMIES) {
            if (!current_mission->EventPoints[ep->EPRef].Data[8])
                return SingleFlagCheck(current_mission->EventPoints[ep->EPRef].Data[9]);
            if (!current_mission->EventPoints[ep->EPRef].Data[9])
                return SingleFlagCheck(current_mission->EventPoints[ep->EPRef].Data[8]);
            return FALSE;
        }
        return FALSE;
    }
    ...
    if (ep->TriggeredBy == TT_PERSON_SEEN) {
        ...
        if ((type != WPT_CREATE_ENEMIES) && (type != WPT_CREATE_PLAYER))
            return FALSE;
    ...
    if (ep->TriggeredBy == TT_THING_RADIUS_DIR || ep->TriggeredBy == TT_MOVE_RADIUS_DIR) {
        //
        ...
        if (type != WPT_CREATE_ENEMIES && type != WPT_CREATE_PLAYER && type != WPT_CREATE_VEHICLE) {
            return FALSE;
        }
    }

    if ((ep->TriggeredBy == TT_CONVERSATION_OVER) && (current_mission->EventPoints[ep->EPRef].WaypointType != WPT_CONVERSATION))
        return FALSE;
    ...
    case WPT_ADJUST_ENEMY:
        ...
        if (!ep->Data[6])
            return FALSE;
        if (current_mission->EventPoints[ep->Data[6]].WaypointType != WPT_CREATE_ENEMIES && current_mission->EventPoints[ep->Data[6]].WaypointType != WPT_CREATE_PLAYER) {
            return FALSE;
        }
        return TRUE;
    ...
    case WPT_TRANSFER_PLAYER:
        ...
        if (current_mission->EventPoints[ep->Data[0]].WaypointType != WPT_CREATE_ENEMIES && current_mission->EventPoints[ep->Data[0]].WaypointType != WPT_CREATE_PLAYER) {
            return FALSE;
        }
    ...
    case WPT_MOVE_THING:
        ...
        if (ep->Data[0]) {
            if (current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_VEHICLE || current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_PLAYER || current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_ENEMIES || current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_ITEM || current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_CREATURE) {
                return TRUE;
            }
        }
    ...
    case WPT_MAKE_PERSON_PEE:
        ...
        if (ep->Data[0]) {
            if (current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_PLAYER || current_mission->EventPoints[ep->Data[0]].WaypointType == WPT_CREATE_ENEMIES) {
                return TRUE;
            }
        }
            ...
            if (ep->WaypointType == WPT_MESSAGE) {
                if (ep->Data[0]) {
                    ...
                    if (ep->Data[2] == 0xffff) {
                        ...
                        //
                    } else if (ep->Data[2] != NULL) {
                        //
                        // This is a message from someone... so it can't be a street name.
                        //
                    } else if (is_street_name((CBYTE*)ep->Data[0])) {
                        CBYTE mess[512];
