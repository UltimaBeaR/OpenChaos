...
SLONG OpenProperties(EventPoint* ep)
{
    ...
    case WPT_SHOUT:
        if (ep)
            ep->Data[0] = (SLONG)InputBox("Shout Code", "Enter code to be 'shouted':", (CBYTE*)ep->Data[0]);
        break;
    ...
    LPARAM lParam)
{
            ...
            if (hilited_ep) {
                ...
                if (new_event) {
                    ...
                    */
                    ...
                    if (new_event->WaypointType > WPT_SIMPLE)
                        OpenProperties(new_event);
    ...
    case WM_MOUSEMOVE:
        ...
        if (dragging_ep && selected_ep) {
            ...
            if (!(selected_ep->Flags & WPT_FLAGS_INSIDE))
                selected_ep->Y = mouse_world_y + height;
    ...
    case WM_COMMAND:
        ...
        case ID_EVENTPOINTROOT_PROPERTIES:
            if (hilited_ep) {
                ...
                OpenProperties(hilited_ep);
                if (selected_ep == remember_me)
                    WaypointCaption(selected_ep);
            }
            break;
...
void process_view_wind(void)
{
    ...
    if (map_valid) {
        ...
        if (current_mission && (edit_mode != 2)) {
            ...
            while (current_ep) {
                ...
                if (dist < (20 << 8)) {
                    ...
                    //	Draw it.
                    colour = (button_colours[ep_ptr->Colour][0] << 16) | (button_colours[ep_ptr->Colour][1] << 8) | (button_colours[ep_ptr->Colour][2]);
                    ...
                    if (WaypointUses[ep_ptr->TriggeredBy] & WPU_DEPEND) {
                        ...
                        if ((!lines_mode)
                            ...
                            if (ep_ptr->EPRef) {
                                SLONG colour2 = (button_colours[targ->Colour][0] << 16) | (button_colours[targ->Colour][1] << 8) | (button_colours[targ->Colour][2]);
                                if (targ->Used) {
                                    POLY_frame_init(FALSE, FALSE);
                                    AENG_world_line(
                    ...
                    // direction
                    if (ep_ptr->WaypointType == WPT_CREATE_PLAYER || ep_ptr->WaypointType == WPT_CREATE_ENEMIES || ep_ptr->WaypointType == WPT_CREATE_VEHICLE || ep_ptr->WaypointType == WPT_CREATE_CREATURE || ep_ptr->WaypointType == WPT_CREATE_CAMERA || ep_ptr->WaypointType == WPT_CAMERA_WAYPOINT || ep_ptr->WaypointType == WPT_CREATE_ITEM || ep_ptr->WaypointType == WPT_SIMPLE || ep_ptr->WaypointType == WPT_GOTHERE_DOTHIS || ep_ptr->WaypointType == WPT_CREATE_TRAP || ep_ptr->TriggeredBy == TT_THING_RADIUS_DIR || ep_ptr->TriggeredBy == TT_MOVE_RADIUS_DIR || ep_ptr->WaypointType == WPT_MOVE_THING) {
                        SLONG dir, dirx, dirz;
        ...
        if (mouse_valid && !hilited_ep) {
            ...
            colour = (button_colours[waypoint_colour][0] << 16) | (button_colours[waypoint_colour][1] << 8) | (button_colours[waypoint_colour][2]);
