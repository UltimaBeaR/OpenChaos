...
void CUTSCENE_del_packet(CSChannel* chan, int pack)
{
    ...
    if ((pack < 0) || (pack >= chan->packetcount))
        return;

    buff = chan->packets + pack;
    if ((buff->type == PT_TEXT) && (buff->pos.X))
        free((CBYTE*)buff->pos.X);
...
void CUTSCENE_write_channel(FILE* file_handle, CSChannel* chan, int version)
{
    ...
    for (packnum = 0; packnum < chan->packetcount; packnum++, pack++)
        CUTSCENE_write_packet(file_handle, pack, version);
}
...
void CUTSCENE_write(FILE* file_handle, CSData* cutscene)
{
    ...
    for (channum = 0; channum < cutscene->channelcount; channum++, chan++)
        CUTSCENE_write_channel(file_handle, chan, cutscene->version);
}
...
void CUTSCENE_read_channel(FILE* file_handle, CSData* cutscene)
{
    ...
    chan->packets = 0;
    packct = chan->packetcount;
    chan->packetcount = 0;
    for (packnum = 0; packnum < packct; packnum++)
        CUTSCENE_read_packet(file_handle, chan, cutscene->version);
}
...
void CUTSCENE_read(FILE* file_handle, CSData** cutsceneref)
{
    ...
    *cutsceneref = cutscene;
    ...
    for (channum = 0; channum < chanct; channum++)
        CUTSCENE_read_channel(file_handle, cutscene);
}
...
Thing* CUTSCENE_item_from_point(CSData* cutscene, SLONG x, SLONG y, SLONG z)
{
    ...
    for (chanctr = 0; chanctr < cutscene->channelcount; chanctr++) {
        if (chan->type == CT_CHAR) {
            if ((abs(x - (edit->thing->WorldPos.X >> 8)) < 50) && (abs(z - (edit->thing->WorldPos.Z >> 8)) < 50))
                return edit->thing;
        }
        chan++;
        edit++;
...
void LERPCamera(CSChannel* chan, int cell)
{
    ...
    if (!CUTSCENE_find_surrounding_packets(chan, cell, &left, &right))
        return;
    ...
    case TBCB_DBLCLK:
        ...
        if ((image == IM_SCENE_SPEAKER) || (image == IM_SCENE_PERSON) || (image == IM_SCENE_CAMERA) || (image == IM_SCENE_BUBBLE)) {
            ...
            case IM_SCENE_PERSON:
                ...
                if (who > -1)
                    echan->thing = CUTSCENE_create_person(who, x, y, z);
                break;
            case IM_SCENE_CAMERA:
                chan->type = CT_CAM;
                break;
            case IM_SCENE_SPEAKER:
                chan->type = CT_WAVE;
                break;
            case IM_SCENE_BUBBLE:
                chan->type = CT_TEXT;
                break;
            }
        }
...
void Update3DStuff(CSChannel* chan, CSEditChannel* edit, int cell)
{
    ...
    if (!pkt) {
        if (chan->type == CT_CHAR)
            LERPAnim(chan, edit->thing, cell);
        if ((chan->type == CT_CAM) && !(GetAsyncKeyState(VK_CONTROL) & (1 << 15)))
            LERPCamera(chan, cell);
        return;
    }
    switch (pkt->type) {
    case PT_ANIM:
        ...
        if (edit->thing) {
            ...
            if (pkt->flags & 1)
                LERPAnim(chan, edit->thing, cell); // should handle the reversing
        }
        break;
    ...
    case PT_WAVE:
        if (CUTSCENE_playback)
            MFX_play_xyz(MUSIC_REF + (chan - cutscene->channels), pkt->index - 2048, 0, pkt->pos.X << 8, pkt->pos.Y << 8, pkt->pos.Z << 8);
        break;
...
BOOL timelineCB(TimeLine* tb, int reason, int index, int subline, int cell)
{
    ...
    case TLCB_SELECT:
        ...
        case PT_CAM:
            ...
            if (pkt->flags & PF_SECURICAM)
                pedit->Update(1, "security camera");
            pedit->Add("Movement", interpolate_strings, PROPTYPE_MULTI);
            pedit->Add("Rotation", interpolate_strings, PROPTYPE_MULTI);
...
BOOL CALLBACK cuts_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MEASUREITEM:
        if (timeline && (wParam == IDC_LIST2))
            timeline->Measure(lParam);
        break;
    case WM_DRAWITEM:
        if (timeline && (wParam == IDC_LIST2))
            timeline->Draw(lParam);
        if (ruler && (wParam == IDC_BUTTON1))
            ruler->Draw(lParam);
        break;
    ...
    case WM_COMMAND:
        ...
        case ID_CEDIT_REWIND:
            if ((i = timeline->GetReadHead()) > 0)
                timeline->SetReadHead(i - 1);
            return true;

        case ID_CEDIT_FFWD:
            if ((i = timeline->GetReadHead()) < 2000)
                timeline->SetReadHead(i + 1);
            return true;
        ...
        if ((HWND)wParam == browser->hWnd) {
            if (target == timeline->hWnd) {
                ...
                if ((chan >= 0) && (chan < cutscene->channelcount)) {
                    ...
                    if ((browser->drag_item.iImage == IM_SCENE_WAVE) && (chn->type != CT_WAVE))
                        break;
                    if ((browser->drag_item.iImage == IM_SCENE_ACTION) && (chn->type != CT_CHAR))
                        break;
                    if ((browser->drag_item.iImage == IM_SCENE_ANIM) && (chn->type != CT_CHAR))
                        break;
...
void do_cutscene_setup(EventPoint* the_ep)
{
    ...
    while (1) {
        ...
        if (!block_keyboard_messages) {
            BOOL ok = 1;
            if (!CUTSCENE_need_keyboard)
                ok = !TranslateAccelerator(dlg, CEDIT_accel, &msg);
            if (ok) {
