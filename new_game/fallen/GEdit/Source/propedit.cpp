...
void PropertyEditor::Update(UWORD index, CBYTE* value)
{
    ...
    ListView_SetItem(hWnd, &item);
    if (callback)
        callback(this, PECB_UPDATE, index, value);
}
...
BOOL PropertyEditor::Process(HWND parent, WPARAM wParam, LPARAM lParam)
{
    ...
    case LVN_ENDLABELEDIT:
        if (callback)
            callback(this, PECB_EDITMODE, 0, 0);
    ...
    case NM_CLICK: {
        ...
        if (item.iItem > -1) {
            ...
            case PROPTYPE_INT:
                ...
                if (callback)
                    callback(this, PECB_EDITMODE, 1, 0);
            ...
            case PROPTYPE_BUTTON:
                ...
                SetWindowLong(hWnd, GWL_STYLE, flirble);
                if (callback)
                    callback(this, PECB_BUTTON, item.iItem, 0);
                break;
            case PROPTYPE_MULTI:
...
HTREEITEM TreeBrowser::GetItemFromOffset(HTREEITEM hItem, int ofs)
{
    ...
    if (ofs > 0) {
        ...
        while (ofs++)
            hItem = TreeView_GetPrevVisible(hWnd, hItem);
    }
    return hItem;
}
...
BOOL TreeBrowser::Process(HWND parent, WPARAM wParam, LPARAM lParam)
{
    ...
    case TVN_BEGINDRAG:
        if (drag) {
            if (callback) {
                ...
                if (!callback(this, TBCB_DRAG, nm->itemNew.lParam, nm->itemNew.hItem, msg))
                    return FALSE;
            }
    ...
    case NM_DBLCLK:
        ...
        if (callback)
            callback(this, TBCB_DBLCLK, selection, TreeView_GetSelection(hWnd), msg);
        break;
    }
    return FALSE;
}
...
BOOL TimeLine::Process(HWND parent, WPARAM wParam, LPARAM lParam)
{
    ...
    case LBN_SELCHANGE:
        ...
        if (callback)
            callback(this, TLCB_SELECT, SendMessage(hWnd, LB_GETCURSEL, 0, 0), 1, read_head);
        return 0;
    }
    return 1;
}
...
void TimeLine::Draw(LPARAM lParam)
{
    ...
    if (draw_read_head >= 0) {
        ...
        brs = CreateSolidBrush(rgb - rgbmod);
        rc.left = draw_read_head;
        rc.right = draw_read_head + 16;
        FillRect(dis->hDC, &rc, brs);
        DeleteObject(brs);
    }
...
void TimeLine::MarkEntry(UWORD index, UWORD start, UWORD length, UBYTE which)
{
    ...
    SendMessage(hWnd, LB_GETTEXT, index, (LPARAM)&entry);
    if ((start > 0) && (entry->marks[start - 1]))
        entry->marks[start - 1] |= 64;
    for (c0 = start; c0 < (start + length); c0++) {
        v = which | (entry->marks[c0] & 7);
        if (c0 == start)
            v |= 128;
        if (c0 == start + length - 1)
            v |= 64;
        if (which == 0xff)
            v = 0; // blank it out
        entry->marks[c0] = v;
    }
    if ((start + length < 2000) && (entry->marks[start + length]))
        entry->marks[start + length] |= 128;
}
...
void TimeLine::SetReadHead(int newpos)
{
    if (read_head != newpos) {
        ...
        InvalidateRect(hWnd, NULL, false);
        if (ruler)
            InvalidateRect(ruler->hWnd, NULL, false);
    }
    if (callback)
        callback(this, TLCB_SELECT, SendMessage(hWnd, LB_GETCURSEL, 0, 0), 1, newpos);
}
...
BOOL TimeLineRuler::Process(HWND parent, WPARAM wParam, LPARAM lParam)
{
    ...
    case BN_CLICKED:
        ...
        ScreenToClient(hWnd, &pt);
        if (owner)
            owner->SetReadHead(owner->GetCellFromX(pt.x - 100));
