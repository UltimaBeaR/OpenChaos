...
int GetReply(DWORD dwMsgType)
{
    ...
    while (g_wReply == -1) {
        ...
        while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            if (msg.message == WM_QUIT || msg.message == WM_CLOSE || msg.message == WM_SYSCOMMAND || msg.message == WM_DESTROY) {
...
//-----------------------------------------------------------------------------
DWORD WINAPI
DirectXSetupCallbackFunction(DWORD dwReason, DWORD dwMsgType, LPSTR szMessage, LPSTR szName, void* pInfo)
{
...
BOOL DirectXInstall(HWND hWnd)
{
    ...
    retc = DirectXSetup(hWnd, szSource, DSETUP_DIRECTX); // | DSETUP_TESTINSTALL);
