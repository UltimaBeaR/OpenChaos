...
#define SET_TEXTURE_COORDS                      \
    texture_x = TextureX;                       \
    if (texture_x > (256 - (1 << TextureZoom))) \
        texture_x = (256 - (1 << TextureZoom)); \
    texture_y = TextureY;                       \
    if (texture_y > (256 - (1 << TextureZoom))) \
        texture_y = (256 - (1 << TextureZoom));
...
void draw_selected_face_textures(SLONG tx, SLONG ty, SLONG zoom)
{
    ...
    for (c0 = 1; c0 < next_face_selected; c0++) {
        ...
        } else if (face_selected_list[c0] < 0) {
