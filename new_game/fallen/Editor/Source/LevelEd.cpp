...
void LevelEditor::TextureFace(SWORD face, PaintTab* texture_mode)
{
    ...
    if (texture_mode->GetPaintMode() == PALETTE_PAINT) {
        if (face < 0) {
            if (
                !(prim_faces3[-face].DrawFlags & POLY_FLAG_TEXTURED) && prim_faces3[-face].Col2 == texture_mode->GetCurrentColour())
                return;
        } else {
            if (
                !(prim_faces4[face].DrawFlags & POLY_FLAG_TEXTURED) && prim_faces4[face].Col2 == texture_mode->GetCurrentColour())
                return;
        }
    } else {
        ...
        if (face < 0) {
            if (
                prim_faces3[-face].DrawFlags & POLY_FLAG_TEXTURED && prim_faces3[-face].TexturePage == (SBYTE)texture_mode->GetTexturePage() && prim_faces3[-face].UV[0][0] == current_texture->U[0] && prim_faces3[-face].UV[0][1] == current_texture->V[0] && prim_faces3[-face].UV[1][0] == current_texture->U[1] && prim_faces3[-face].UV[1][1] == current_texture->V[1] && prim_faces3[-face].UV[2][0] == current_texture->U[2] && prim_faces3[-face].UV[2][1] == current_texture->V[2])
                return;
        } else {

            if (
                prim_faces4[face].DrawFlags & POLY_FLAG_TEXTURED && prim_faces4[face].TexturePage == (UBYTE)texture_mode->GetTexturePage() && prim_faces4[face].UV[0][0] == current_texture->U[0] && prim_faces4[face].UV[0][1] == current_texture->V[0] && prim_faces4[face].UV[1][0] == current_texture->U[1] && prim_faces4[face].UV[1][1] == current_texture->V[1] && prim_faces4[face].UV[2][0] == current_texture->U[2] && prim_faces4[face].UV[2][1] == current_texture->V[2] && prim_faces4[face].UV[3][0] == current_texture->U[3] && prim_faces4[face].UV[3][1] == current_texture->V[3])
                return;
        }
    }
    ...
    if (texture_mode->GetPaintMode() == PALETTE_PAINT) {
        ...
        if (face < 0) {
            ...
            if (texture_mode->GetTexturePage() < 0) {
                ...
                if (prim_faces4[face].ThingIndex < 0) { // face is part of a buildtab building, so set walls texture info
                    UBYTE type = 0;
                    if (PaintMode->GetPaintMode() == STYLE_PAINT) {
                        type = PaintMode->CurrentStyleEdit;
