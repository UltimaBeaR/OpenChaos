...
void bring_module_to_front(EditorModule* the_module)
{
    ...
    if (the_module->GetLastModuleLink()) { // Not at start of list.
    ...
    while (SHELL_ACTIVE && (editor_status & EDITOR_NORMAL)) {
        ...
        if (Keys[KB_PPOINT]) {
            ...
            if (!ShiftFlag) {
                ...
                if (Keys[KB_P2]) {
                    Keys[KB_P2] = 0;
                    editor_texture_set = 2;
                }
                if (Keys[KB_P3]) {
                    Keys[KB_P3] = 0;
                    editor_texture_set = 3;
                }
                if (Keys[KB_P4]) {
                    Keys[KB_P4] = 0;
                    editor_texture_set = 4;
                }
                if (Keys[KB_P5]) {
                    Keys[KB_P5] = 0;
                    editor_texture_set = 5;
                }
                if (Keys[KB_P6]) {
                    Keys[KB_P6] = 0;
                    editor_texture_set = 6;
                }
                if (Keys[KB_P7]) {
                    Keys[KB_P7] = 0;
                    editor_texture_set = 7;
                }
                if (Keys[KB_P8]) {
                    Keys[KB_P8] = 0;
                    editor_texture_set = 8;
                }
                if (Keys[KB_P9]) {
                    Keys[KB_P9] = 0;
                    editor_texture_set = 9;
                }
                if (Keys[KB_P0]) {
                    Keys[KB_P0] = 0;
                    editor_texture_set = 10;
                }
            } else {
                if (Keys[KB_P1]) {
                    Keys[KB_P1] = 0;
                    editor_texture_set = 11;
                }
                if (Keys[KB_P2]) {
                    Keys[KB_P2] = 0;
                    editor_texture_set = 12;
                }
                if (Keys[KB_P3]) {
                    Keys[KB_P3] = 0;
                    editor_texture_set = 13;
                }
                if (Keys[KB_P4]) {
                    Keys[KB_P4] = 0;
                    editor_texture_set = 14;
                }
                if (Keys[KB_P5]) {
                    Keys[KB_P5] = 0;
                    editor_texture_set = 15;
                }
                if (Keys[KB_P6]) {
                    Keys[KB_P6] = 0;
                    editor_texture_set = 16;
                }
                if (Keys[KB_P7]) {
                    Keys[KB_P7] = 0;
                    editor_texture_set = 17;
                }
                if (Keys[KB_P8]) {
                    Keys[KB_P8] = 0;
                    editor_texture_set = 18;
                }
                if (Keys[KB_P9]) {
                    Keys[KB_P9] = 0;
                    editor_texture_set = 19;
                }
                if (Keys[KB_P0]) {
                    Keys[KB_P0] = 0;
                    editor_texture_set = 20;
                }
...
#define MAX_TICKS 16
                ...
                if (Keys[KB_P0]) {
                    Keys[KB_P0] = 0;
                    tick_key[tick_upto] = 0;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P1]) {
                    Keys[KB_P1] = 0;
                    tick_key[tick_upto] = 1;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P2]) {
                    Keys[KB_P2] = 0;
                    tick_key[tick_upto] = 2;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P3]) {
                    Keys[KB_P3] = 0;
                    tick_key[tick_upto] = 3;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P4]) {
                    Keys[KB_P4] = 0;
                    tick_key[tick_upto] = 4;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P5]) {
                    Keys[KB_P5] = 0;
                    tick_key[tick_upto] = 5;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P6]) {
                    Keys[KB_P6] = 0;
                    tick_key[tick_upto] = 6;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P7]) {
                    Keys[KB_P7] = 0;
                    tick_key[tick_upto] = 7;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P8]) {
                    Keys[KB_P8] = 0;
                    tick_key[tick_upto] = 8;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                if (Keys[KB_P9]) {
                    Keys[KB_P9] = 0;
                    tick_key[tick_upto] = 9;
                    tick_time[tick_upto] = GetTickCount();
                    tick_upto++;
                }
                ...
                if (tick_key[(tick_upto - 1) & (MAX_TICKS - 1)] == tick_key[(tick_upto - 2) & (MAX_TICKS - 1)] && tick_key[(tick_upto - 1) & (MAX_TICKS - 1)] == tick_key[(tick_upto - 3) & (MAX_TICKS - 1)] && tick_key[(tick_upto - 1) & (MAX_TICKS - 1)] == tick_key[(tick_upto - 4) & (MAX_TICKS - 1)] && tick_key[(tick_upto - 1) & (MAX_TICKS - 1)] == tick_key[(tick_upto - 5) & (MAX_TICKS - 1)]) {
                    ...
                    if (t1 > 0 && t2 > 0 && t3 > 0 && t4 > 0) {
                        ...
                        if (WITHIN(same1, 200, 320) && WITHIN(same2, 200, 320)) {
                            quaver = t1 + t2 >> 1;
                            crotchet = t3 + t4 >> 1;
