...
void HMTAB_save_primgrids(CBYTE* fname)
{
    ...
    for (i = 1; i < next_prim_object; i++) {
        ...
        if (hp->defined) {
            if (hp->x_res != 2 || hp->y_res != 2 || hp->z_res != 2 || hp->x_point[0] != 0 || hp->y_point[0] != 0 || hp->z_point[0] != 0 || hp->x_point[1] != 0x10000 || hp->y_point[1] != 0x10000 || hp->z_point[1] != 0x10000) {
...
void HmTab::HandleTab(MFPoint* current_point)
{
    ...
    if (Keys[KB_TAB]) {
        ...
        switch (VD_cam_view) {
        case VD_CAM_VIEW_X:
            VD_cam_view = VD_CAM_VIEW_Y;
            break;
        case VD_CAM_VIEW_Y:
            VD_cam_view = VD_CAM_VIEW_Z;
            break;
        case VD_CAM_VIEW_Z:
            VD_cam_view = VD_CAM_VIEW_X;
            break;
...
#define HMTAB_DGRAV (0.1F)
    ...
    case CTRL_SAVE:
        HMTAB_save_primgrids("data\\primgrid.dat");
        break;
