...
void BuildTab::DrawTabContent(void)
{
    ...
    if (WITHIN(EditBuilding, 1, MAX_BUILDINGS - 1)) {
        ...
        case BUILDING_TYPE_HOUSE:
            SetControlState(CTRL_BUILDING_TYPE_HOUSE, CTRL_SELECTED);
            break;
        case BUILDING_TYPE_WAREHOUSE:
            SetControlState(CTRL_BUILDING_TYPE_WAREHOUSE, CTRL_SELECTED);
            break;
        case BUILDING_TYPE_OFFICE:
            SetControlState(CTRL_BUILDING_TYPE_OFFICE, CTRL_SELECTED);
            break;
        case BUILDING_TYPE_APARTEMENT:
            SetControlState(CTRL_BUILDING_TYPE_APARTEMENT, CTRL_SELECTED);
            break;
        case BUILDING_TYPE_CRATE_IN:
            SetControlState(CTRL_BUILDING_TYPE_CRATE_IN, CTRL_SELECTED);
            break;
        case BUILDING_TYPE_CRATE_OUT:
            SetControlState(CTRL_BUILDING_TYPE_CRATE_OUT, CTRL_SELECTED);
            break;
...
void BuildTab::DrawModuleContent(SLONG x, SLONG y, SLONG w, SLONG h)
{
    ...
    if (EditBuilding && EditStorey) {
        if (inside_building != EditBuilding || inside_storey != EditStorey) {
            //
        ...
        if (building_list[building].BuildingFlags & 1) {
            ...
            while (storey_index) {
                ...
                //			if(storey_index==337 || storey_index==24)
                {
                    ...
                    if (storey_index == inside_storey && building == inside_building) {
                    //
...
//									storey_list[storey_list[index].Roof].DY-=BLOCK_SIZE*4;
    ...
    case CTRL_NEXT_STOREY:
        ...
        if (WITHIN(EditBuilding, 1, MAX_BUILDINGS - 1) && WITHIN(EditStorey, 1, MAX_STOREYS - 1)) {
            SLONG i_storey;
