...
void draw_element(UWORD prim, SLONG x, SLONG y, SLONG z, struct KeyFrameElement* anim_info)
{
    ...
    for (c0 = p_obj->StartFace3; c0 < p_obj->EndFace3; c0++) {
        ...
        if (p_f3->Points[0] < p_obj->StartPoint || p_f3->Points[1] < p_obj->StartPoint || p_f3->Points[2] < p_obj->StartPoint) {
            ERROR_MSG(0, " point before start point");
            goto exit;
        }
        if (p_f3->Points[0] > p_obj->EndPoint || p_f3->Points[1] > p_obj->EndPoint || p_f3->Points[2] > p_obj->EndPoint) {
