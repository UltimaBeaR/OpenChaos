...
void do_quad_clip_list(SWORD face, SLONG p0, SLONG p1, SLONG p2, SLONG p3)
{
    ...
    } else if (global_res[p0].X > edit_info.SelectRect.GetLeft() && global_res[p0].X < edit_info.SelectRect.GetRight() && global_res[p0].Y > edit_info.SelectRect.GetTop() && global_res[p0].Y < edit_info.SelectRect.GetBottom())
        if (global_res[p1].X > edit_info.SelectRect.GetLeft() && global_res[p1].X < edit_info.SelectRect.GetRight() && global_res[p1].Y > edit_info.SelectRect.GetTop() && global_res[p1].Y < edit_info.SelectRect.GetBottom())
            if (global_res[p2].X > edit_info.SelectRect.GetLeft() && global_res[p2].X < edit_info.SelectRect.GetRight() && global_res[p2].Y > edit_info.SelectRect.GetTop() && global_res[p2].Y < edit_info.SelectRect.GetBottom())
                if (global_res[p3].X > edit_info.SelectRect.GetLeft() && global_res[p3].X < edit_info.SelectRect.GetRight() && global_res[p3].Y > edit_info.SelectRect.GetTop() && global_res[p3].Y < edit_info.SelectRect.GetBottom())
...
UWORD find_pair(UWORD start_face, UWORD end_face, UWORD half_face, UWORD tolerance)
{
    ...
    dx = prim_object_points[p_half_face->Points[0]].X - prim_object_points[p_half_face->Points[1]].X;

    dy = prim_object_points[p_half_face->Points[0]].Y - prim_object_points[p_half_face->Points[1]].Y;

    dz = prim_object_points[p_half_face->Points[0]].Z - prim_object_points[p_half_face->Points[1]].Z;

    len1 = dx * dx + dy * dy + dz * dz;
    if (abs(dy) < 5)
        len1 >>= 1;

    dx = prim_object_points[p_half_face->Points[1]].X - prim_object_points[p_half_face->Points[2]].X;
    dy = prim_object_points[p_half_face->Points[1]].Y - prim_object_points[p_half_face->Points[2]].Y;
    dz = prim_object_points[p_half_face->Points[1]].Z - prim_object_points[p_half_face->Points[2]].Z;
    ...
    dx = prim_object_points[p_half_face->Points[2]].X - prim_object_points[p_half_face->Points[0]].X;
    dy = prim_object_points[p_half_face->Points[2]].Y - prim_object_points[p_half_face->Points[0]].Y;
    dz = prim_object_points[p_half_face->Points[2]].Z - prim_object_points[p_half_face->Points[0]].Z;
        ...
        for (current_face = start_face; current_face <= end_face; current_face++) {
            ...
            if ((current_face != half_face) && failed_match == 0) {
                ...
                if (search_face->Points[0] == p_half_face->Points[0]) {
                    corners[0] = 0;
                    count++;
                } else if (search_face->Points[1] == p_half_face->Points[0]) {
                    corners[1] = 0;
                    count++;
                } else if (search_face->Points[2] == p_half_face->Points[0]) {
                    corners[2] = 0;
                    count++;
                }
                ...
                if (search_face->Points[0] == p_half_face->Points[1]) {
                    corners[0] = 1;
                    count++;
                } else if (search_face->Points[1] == p_half_face->Points[1]) {
                    corners[1] = 1;
                    count++;
                } else if (search_face->Points[2] == p_half_face->Points[1]) {
                    corners[2] = 1;
                    count++;
                }
                ...
                if (search_face->Points[0] == p_half_face->Points[2]) {
                    corners[0] = 2;
                    count++;
                } else if (search_face->Points[1] == p_half_face->Points[2]) {
                    corners[1] = 2;
                    count++;
                } else if (search_face->Points[2] == p_half_face->Points[2]) {
                    corners[2] = 2;
                    count++;
                }
                if (count == 1) {
                ...
                if (count == 2) { // have found a poly with two shared points
                    ...
                    dx = prim_object_points[p_half_face->Points[p1]].X - prim_object_points[p_half_face->Points[p2]].X;
                    dy = prim_object_points[p_half_face->Points[p1]].Y - prim_object_points[p_half_face->Points[p2]].Y;
                    dz = prim_object_points[p_half_face->Points[p1]].Z - prim_object_points[p_half_face->Points[p2]].Z;
...
void make_object_quad(UWORD prim)
{
    ...
    for (i = po->StartFace3; i < po->EndFace3; i++) {
        ...
        if (!(f3_a->FaceFlags & FACE_FLAG_EDGE_VISIBLE_A)) {
            combine = TRUE;
            combine_edge = 0;
        }
        if (!(f3_a->FaceFlags & FACE_FLAG_EDGE_VISIBLE_B)) {
            combine = TRUE;
            combine_edge = 1;
        }
        if (!(f3_a->FaceFlags & FACE_FLAG_EDGE_VISIBLE_C)) {
            combine = TRUE;
            combine_edge = 2;
        }
            ...
            for (j = po->StartFace3; j < po->EndFace3; j++) {
                ...
                for (k = 0; k < 3; k++) {
                    if (!(f3_b->FaceFlags & (FACE_FLAG_EDGE_VISIBLE << k))) {
                        ...
                        if (p1 == combine_p2 && p2 == combine_p1) {
    ...
    for (i = po->StartFace3; i < po->EndFace3; i++) {
        ...
        f3_a->FaceFlags &= ~(FACE_FLAG_EDGE_VISIBLE_A | FACE_FLAG_EDGE_VISIBLE_B | FACE_FLAG_EDGE_VISIBLE_C);
    }
}
...
#ifdef MATRIX_CALC
rot X 1 0 0 0 cx sx 0 - sx cx

        rot Y rotXY cy 0
    - sy cy 0 - sy 0 1 0 sxsy cx sxcy sy 0 cy cxsy - sx cxcy

        rot Z rotXYZ cz sz 0 czcy cysz
    - sy
    - sz cz 0 sxsycz - cxsz sxsysz + cxcz sxcy 0 0 1 cxsycz + sxsz cxsysz - sxcz cxcy
    ...
    - sxcz = H
            cxcy = I
                ...
                czcy = A
                    cysz = B
    - sy = C sxcz * (-C) - cxsz = D sx * (-C)sz + cxcz = E
                        sxcy = F
                                   cx(-C) cz
    + sxsz = G
                            cx(-C) sz
...
SLONG read_sex(CBYTE* fname, SLONG scale /* Ignored */, SLONG offset)
{
    ...
    while (fgets(line, MAX_LINE_LEN, handle)) {
        ...
        if (match == 10) {
            ...
            if (edge_a) {
                prim_faces3[next_prim_face3].FaceFlags |= FACE_FLAG_EDGE_VISIBLE_B;
            }
            if (edge_b) {
                prim_faces3[next_prim_face3].FaceFlags |= FACE_FLAG_EDGE_VISIBLE_A;
            }
            if (edge_c) {
                prim_faces3[next_prim_face3].FaceFlags |= FACE_FLAG_EDGE_VISIBLE_C;
            }
...
SBYTE read_asc(CBYTE* fname, SLONG scale, ULONG offset)
{
    ...
    if (fname_len >= 5) {
        if (toupper(fname[fname_len - 3]) == 'S' && toupper(fname[fname_len - 2]) == 'E' && toupper(fname[fname_len - 1]) == 'X') {
            //
            ...
            } else if (!strcmp(ts, "Vertices:")) // 3ds max  3dx export
            {
                ...
                if (strcmp(ts, "list:")) {
                    ...
                    */
                    ...
                    if (prim_points[next_prim_point].X < minx) {
                        minx = prim_points[next_prim_point].X;
                    }
                    if (prim_points[next_prim_point].Z < minz) {
                        minz = prim_points[next_prim_point].Z;
                    }

                    if (prim_points[next_prim_point].X > maxx) {
                        maxx = prim_points[next_prim_point].X;
                    }
                    if (prim_points[next_prim_point].Z > maxz) {
                        maxz = prim_points[next_prim_point].Z;
                    }
    ...
    if (handle) {
        do {
            ...
            } else if (!strcmp(ts, "object")) // 3ds max is "object:" rather than "named object:"
            {
                ...
                fscanf(handle, "%s", ts);
                if (!strcmp(ts, "list:")) { // start of material list for coming faces
...
SLONG find_and_apply_to_quad(struct PrimPoint* pp, struct PrimFace4* face4, SLONG gx, SLONG gy, SLONG gz)
{
    ...
    while (index) {
        ...
        for (c0 = sf; c0 < ef; c0++) {
            if (CLOSE(prim_points[prim_faces4[c0].Points[0]].X, pp[0].X - gx) && CLOSE(prim_points[prim_faces4[c0].Points[0]].Y, pp[0].Y - gy) && CLOSE(prim_points[prim_faces4[c0].Points[0]].Z, pp[0].Z - gz) && CLOSE(prim_points[prim_faces4[c0].Points[1]].X, pp[1].X - gx) && CLOSE(prim_points[prim_faces4[c0].Points[1]].Y, pp[1].Y - gy) && CLOSE(prim_points[prim_faces4[c0].Points[1]].Z, pp[1].Z - gz) && CLOSE(prim_points[prim_faces4[c0].Points[2]].X, pp[2].X - gx) && CLOSE(prim_points[prim_faces4[c0].Points[2]].Y, pp[2].Y - gy) && CLOSE(prim_points[prim_faces4[c0].Points[2]].Z, pp[2].Z - gz)) {
...
SLONG find_and_apply_to_tri(struct PrimPoint* pp, struct PrimFace3* face3, SLONG gx, SLONG gy, SLONG gz)
{
    ...
    while (index) {
        ...
        for (c0 = sf; c0 < ef; c0++) {
            if (CLOSE(prim_points[prim_faces3[c0].Points[0]].X, pp[0].X - gx) && CLOSE(prim_points[prim_faces3[c0].Points[0]].Y, pp[0].Y - gy) && CLOSE(prim_points[prim_faces3[c0].Points[0]].Z, pp[0].Z - gz) && CLOSE(prim_points[prim_faces3[c0].Points[1]].X, pp[1].X - gx) && CLOSE(prim_points[prim_faces3[c0].Points[1]].Y, pp[1].Y - gy) && CLOSE(prim_points[prim_faces3[c0].Points[1]].Z, pp[1].Z - gz) && CLOSE(prim_points[prim_faces3[c0].Points[2]].X, pp[2].X - gx) && CLOSE(prim_points[prim_faces3[c0].Points[2]].Y, pp[2].Y - gy) && CLOSE(prim_points[prim_faces3[c0].Points[2]].Z, pp[2].Z - gz)) {
...
SLONG save_prim_object(SLONG prim)
{
    ...
    for (i = po->StartFace4; i < po->EndFace4; i++) {
        if ((prim_faces4[i].DrawFlags & POLY_FLAG_WALKABLE) || (prim_faces4[i].FaceFlags & FACE_FLAG_WALKABLE)) {
    ...
    if (fwrite(&prim_objects[prim], sizeof(PrimObject), 1, handle) != 1)
        goto file_error;
    if (fwrite(&prim_points[po->StartPoint], sizeof(PrimPoint), num_points, handle) != num_points)
        goto file_error;
    if (fwrite(&prim_faces3[po->StartFace3], sizeof(PrimFace3), num_faces3, handle) != num_faces3)
        goto file_error;
    if (fwrite(&prim_faces4[po->StartFace4], sizeof(PrimFace4), num_faces4, handle) != num_faces4)
        goto file_error;
