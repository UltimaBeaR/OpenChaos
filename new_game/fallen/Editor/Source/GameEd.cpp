...
void GameEditor::HandleContentClick(UBYTE flags, MFPoint* clicked_point)
{
    ...
    case LEFT_CLICK:
        ...
        case ED_ITEM_MAP_BLOCK:
            if (!SelectMode) {
                if (
                    ThingMode->GetCurrentClass() && ThingMode->GetCurrentGenus()) {
    ...
    if (p_facet->EndFace4)
        for (c0 = p_facet->StartFace4; c0 < p_facet->EndFace4; c0++) {
            ...
            if ((!(flag_and & EF_CLIPFLAGS)) && ((flag_or & EF_BEHIND_YOU) == 0)) {
                az = (global_res[p0].Z + global_res[p1].Z + global_res[p2].Z + global_res[p3].Z) / 4;
                ...
                while (bucket) {
                    ...
                    case BT_QUAD:
                        ...
                        if (the_quad->DrawFlags & POLY_FLAG_DOUBLESIDED) {
                            if (
                                (Z_NORMAL(the_quad->P[0], the_quad->P[1])) > 0 && (Z_NORMAL(the_quad->P[1], the_quad->P[3])) > 0 && (Z_NORMAL(the_quad->P[3], the_quad->P[2])) > 0 && (Z_NORMAL(the_quad->P[2], the_quad->P[0])) > 0) {
                    ...
                    case BT_TRI:
                        ...
                        if (the_tri->DrawFlags & POLY_FLAG_DOUBLESIDED) {
                            if (
                                (Z_NORMAL(the_tri->P[0], the_tri->P[1])) > 0 && (Z_NORMAL(the_tri->P[1], the_tri->P[2])) > 0 && (Z_NORMAL(the_tri->P[2], the_tri->P[0])) > 0) {
                    ...
                    case BT_MAP_THING:
                        ...
                        if (
                            mouse_point.X >= the_map_thing->X - (THING_BOX_SIZE >> 1) && mouse_point.X < the_map_thing->X + (THING_BOX_SIZE >> 1) && mouse_point.Y >= the_map_thing->Y - (THING_BOX_SIZE >> 1) && mouse_point.Y < the_map_thing->Y + (THING_BOX_SIZE >> 1)) {
                    ...
                    case BT_WAYPOINT:
                        ...
                        if (
                            mouse_point.X >= the_waypoint->X - (WP_BOX_SIZE >> 1) && mouse_point.X < the_waypoint->X + (WP_BOX_SIZE >> 1) && mouse_point.Y >= the_waypoint->Y - (WP_BOX_SIZE >> 1) && mouse_point.Y < the_waypoint->Y + (WP_BOX_SIZE >> 1)) {
                    ...
                    case BT_SPHERE_AREA:
                        ...
                        if (
                            the_sphere->ShowSizeHook && mouse_point.X >= offset_x - (SH_BOX_SIZE >> 1) && mouse_point.X < offset_x + (SH_BOX_SIZE >> 1) && mouse_point.Y >= offset_y - (SH_BOX_SIZE >> 1) && mouse_point.Y < offset_y + (SH_BOX_SIZE >> 1)) {
        ...
        bucket = p->BucketPtr;
        {
            ...
            while (bucket) {
                ...
                case BT_QUAD:
                    the_quad = (BucketQuad*)bucket;
                    if (
                        the_quad->EditRef.ItemType == SelectedItem.ItemType && the_quad->EditRef.ItemRef == SelectedItem.ItemRef && FlashState) {
                        poly_info.DrawFlags = the_quad->DrawFlags & (POLY_FLAG_DOUBLESIDED);
                        poly_info.Col = ACTIVE_COL;
                    } else if (
                        the_quad->EditRef.ItemType == HilitedItem.ItemType && the_quad->EditRef.ItemRef == HilitedItem.ItemRef && SelectMode == 0) {
                        poly_info.DrawFlags = the_quad->DrawFlags & (POLY_FLAG_DOUBLESIDED);
                ...
                case BT_TRI:
                    the_tri = (BucketTri*)bucket;
                    if (
                        the_tri->EditRef.ItemType == SelectedItem.ItemType && the_tri->EditRef.ItemRef == SelectedItem.ItemRef && FlashState) {
                        poly_info.DrawFlags = the_tri->DrawFlags & (POLY_FLAG_DOUBLESIDED);
                        poly_info.Col = ACTIVE_COL;
                    } else if (
                        the_tri->EditRef.ItemType == HilitedItem.ItemType && the_tri->EditRef.ItemRef == HilitedItem.ItemRef && SelectMode == 0) {
                        poly_info.DrawFlags = the_tri->DrawFlags & (POLY_FLAG_DOUBLESIDED);
                        poly_info.Col = HILITE_COL;
                    } else {
                ...
                case BT_MAP_THING:
                    ...
                    if (
                        the_map_thing->EditRef.ItemType == HilitedItem.ItemType && the_map_thing->EditRef.ItemRef == HilitedItem.ItemRef && (SelectMode == 0 || (SelectMode >= SELECT_COND_TAB_THING && SelectMode <= SELECT_THING_TAB_SWITCH) || (SelectMode >= SELECT_COM_TAB_THING && SelectMode <= SELECT_COM_TAB_SWITCH))) {
                        draw_colour = HILITE_COL;
                        hilited_thing = the_map_thing;
                    } else if (
                        the_map_thing->EditRef.ItemType == SelectedItem.ItemType && the_map_thing->EditRef.ItemRef == SelectedItem.ItemRef) {
                        if (FlashState)
                ...
                case BT_WAYPOINT:
                    ...
                    if (
                        the_waypoint->EditRef.ItemType == HilitedItem.ItemType && the_waypoint->EditRef.ItemRef == HilitedItem.ItemRef && (SelectMode == 0 || SelectMode <= SELECT_PREV_WAYPOINT || SelectMode == SELECT_COM_TAB_WAYPOINT)) {
                        draw_colour = HILITE_COL;
                        hilited_waypoint = the_waypoint;
                    } else if (
                        the_waypoint->EditRef.ItemType == SelectedItem.ItemType && the_waypoint->EditRef.ItemRef == SelectedItem.ItemRef) {
                        if (FlashState)
                            draw_colour = ACTIVE_COL;
                ...
                case BT_SPHERE_AREA:
                    ...
                    if (
                        the_sphere->EditRef.ItemType == HilitedItem.ItemType && the_sphere->EditRef.ItemRef == HilitedItem.ItemRef && SelectMode == 0) {
                        draw_colour = HILITE_COL;
