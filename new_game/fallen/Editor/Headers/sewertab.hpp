...
public:
    ...
    inline void SetViewToEngine(void)
    {
        ViewX = (engine.X >> 8) & ELE_AND;
        ViewY = (engine.Y >> 8) & ELE_AND;
        ViewZ = (engine.Z >> 8) & ELE_AND;
    }
    inline void SetEngineToView(void)
    {
        engine.X = ViewX << 8;
        engine.Y = ViewY << 8;
        engine.Z = ViewZ << 8;
    }
    inline void SetView(SLONG x, SLONG z)
    {
        ViewX = x;
        ViewZ = z;
    }
