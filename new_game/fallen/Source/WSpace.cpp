...
void handle_ws_context(POINT* click_point)
{
    ...
    if (ws_tree && click_point) {
        if (get_element_at_point(click_point, &the_element)) {
            ...
            if (the_element->ElementType == ET_LMAP)
                return;
    ...
    if (ws_tree && click_point) {
        if (get_element_at_point(click_point, &the_element)) {
            ...
            case ET_MAP:
                ...
                if (next)
                    TreeView_SelectItem(ws_tree, next);
...
void handle_ws_select(WSElement* the_element)
{
    ...
    case ET_MISSION:
        if (
            current_map && MAP_NUMBER(current_map) == TO_MISSION(the_element->MissionRef)->MapIndex) {
            TVITEM item;
            // clear old bold
        ...
        if (file_handle) {
            ...
            for (c0 = 0; c0 < count; c0++) {
                ...
                if (mission_pool[c0].Flags & MISSION_FLAG_USED) {
                    ...
                    if (!mission_pool[c0].BoredomRate)
                        mission_pool[c0].BoredomRate = 4;
...
HTREEITEM ws_root_waypoint(CBYTE* msg, SLONG type, LPARAM param)
{
    ...
    tv_is.item.iImage = tv_is.item.iSelectedImage = type;
...
void ws_add_waypoint(EventPoint* ep)
{
    ...
    if (code == -1)
        return;
    if (!parent)
        parent = ws_root_waypoint(msg, 0, code);
    ...
    tv_is.item.iImage = tv_is.item.iSelectedImage = ws_image_from_type(ep);
