...
void AZ_create_lines(void)
{
    ...
    for (x = 0; x < MAP_WIDTH; x++) {
        ...
        for (z = 0; z < MAP_HEIGHT; z++) {
            if ((PAP_2HI(x, z).Flags & PAP_FLAG_SINK_POINT) && !(PAP_2HI(x, z).Flags & PAP_FLAG_NOUPPER)) {
                //
    ...
    for (i = 1; i < next_dfacet; i++) {
        ...
        fence = df->FacetType == STOREY_TYPE_FENCE || df->FacetType == STOREY_TYPE_FENCE_BRICK || df->FacetType == STOREY_TYPE_FENCE_FLAT;
