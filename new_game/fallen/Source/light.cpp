...
void LIGHT_map_move(UBYTE l_index, GameCoord newpos)
{
    ...
    if (map_x_old == map_x_new && map_z_old == map_z_new) {
        //
...
void LIGHT_pos_set(LIGHT_Index l_index, GameCoord newpos)
{
    ...
    if (IMPLIES(ll->type == LIGHT_TYPE_BROKEN, ll->counter & 0x80) && IMPLIES(ll->type == LIGHT_TYPE_PULSE, ll->counter > (ll->param >> 1))) {
        //
        // The light is on.
        //
...
void LIGHT_prim(THING_INDEX t_index)
{
    ...
    if (LIGHT_context_t_index == t_index && LIGHT_context_gameturn == GAME_TURN) {
        //
