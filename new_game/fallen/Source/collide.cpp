...
ULONG similar_col_vect(UWORD index)
{
    ...
    for (c0 = 1; c0 < next_col_vect; c0++) {
        if (col_vects[c0].X[0] == col_vects[index].X[0] && col_vects[c0].Y[0] == col_vects[index].Y[0] && col_vects[c0].X[1] == col_vects[index].X[1] && col_vects[c0].Y[1] == col_vects[index].Y[1])
            return (1);
        /*
                ...
                if (collision_storey(p_vect->PrimType)) {
                    ...
                    if (WITHIN(midy, y_bot, y_top)) {
                        ...
                        if (flag_and) {
                            ...
                            if ((WITHIN(rx1, minx, maxx) && WITHIN(rz1, minz, maxz)) || (WITHIN(rx2, minx, maxx) && WITHIN(rz2, minz, maxz))) {
                                //
                                // Definitely collided.
                                //
