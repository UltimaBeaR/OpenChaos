    ...
    SPARK_Point* sp)
{
    ...
    if (WITHIN(mx, 0, MAP_WIDTH - 1) && WITHIN(mz, 0, MAP_HEIGHT - 1)) {
        ...
        while (v_list) {
            ...
            if (p_vect->X[0] == p_vect->X[1]) {
                clamp |= SPARK_FLAG_CLAMP_X;
            }
            if (p_vect->Z[0] == p_vect->Z[1]) {
                clamp |= SPARK_FLAG_CLAMP_Z;
            }

            y_top = p_vect->Y[0] + BLOCK_SIZE * p_vect->PrimExtra;
            y_bot = p_vect->Y[0];

            if (p_vect->PrimType == STOREY_TYPE_NORMAL_FOUNDATION || p_vect->PrimType == STOREY_TYPE_FENCE_FLAT) {
                y_bot = -INFINITY;
            }
            ...
            for (i = 0; i < 3; i++) {
                ...
                if (WITHIN(y, y_bot, y_top)) {
                    if (clamp & SPARK_FLAG_CLAMP_X) {
                        x = p_vect->X[0];
                    } else {
                        x = mid_x + (rand() % radius) - (radius >> 1);
                    }
                    if (clamp & SPARK_FLAG_CLAMP_Z) {
                        z = p_vect->Z[0];
                    } else {
                        z = mid_z + (rand() % radius) - (radius >> 1);
                    }
        ...
        for (mz = z1; mz <= z2; mz++) {
            ...
            */
            ...
            while (v_list) {
                ...
                if (p_vect->PrimType == STOREY_TYPE_WALL || p_vect->PrimType == STOREY_TYPE_NORMAL || p_vect->PrimType == STOREY_TYPE_NORMAL_FOUNDATION || p_vect->PrimType == STOREY_TYPE_FENCE || p_vect->PrimType == STOREY_TYPE_FENCE_FLAT) {
                    //
