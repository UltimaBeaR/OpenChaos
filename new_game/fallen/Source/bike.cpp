    ...
    SLONG* sphere_z)
{
    ...
    if (WITHIN(map_x, 0, PAP_SIZE_HI - 1) && WITHIN(map_z, 0, PAP_SIZE_HI - 1)) {
        ...
        if (map_x > 0) {
            if (map_z > 0) {
                if (MAVHEIGHT(map_x - 1, map_z - 1) >= maxup) {
                    if ((sx & 0xffff) < (BIKE_WHEEL_COL << 8) && (sz & 0xffff) < (BIKE_WHEEL_COL << 8)) {
                        sx &= ~0xffff;
                        sz &= ~0xffff;
            ...
            if (map_z < PAP_SIZE_HI - 1) {
                if (MAVHEIGHT(map_x - 1, map_z + 1) >= maxup) {
                    if ((sx & 0xffff) < (BIKE_WHEEL_COL << 8) && (sz & 0xffff) > 0x10000 - (BIKE_WHEEL_COL << 8)) {
                        sx &= ~0xffff;
                        sz &= ~0xffff;
        ...
        if (map_x < PAP_SIZE_HI - 1) {
            if (map_z > 0) {
                if (MAVHEIGHT(map_x + 1, map_z - 1) >= maxup) {
                    if ((sx & 0xffff) > 0x10000 - (BIKE_WHEEL_COL << 8) && (sz & 0xffff) < (BIKE_WHEEL_COL << 8)) {
                        sx &= ~0xffff;
                        sz &= ~0xffff;
            ...
            if (map_z < PAP_SIZE_HI - 1) {
                if (MAVHEIGHT(map_x + 1, map_z + 1) >= maxup) {
                    if ((sx & 0xffff) > 0x10000 - (BIKE_WHEEL_COL << 8) && (sz & 0xffff) > 0x10000 - (BIKE_WHEEL_COL << 8)) {
                        sx &= ~0xffff;
                        sz &= ~0xffff;
...
void BIKE_process_normal(Thing* p_bike)
{
    ...
    if (bb->mode == BIKE_MODE_PARKED || bb->mode == BIKE_MODE_MOUNTING || bb->mode == BIKE_MODE_DISMOUNTING) {
        go_again = 0;
    } else {
...
//	TRACE("bike d's %d / %d \n",bb->back_dx,bb->back_dz);
    ...
    if (bb->SlideTimer) {
        ...
        if (abs(bb->back_dx) < 8 * 2) {
            bb->back_dx = 0;
        }
        if (abs(bb->back_dz) < 8 * 2) {
            bb->back_dz = 0;
        }
    } else if ((bb->flag & BIKE_FLAG_ONGROUND_BACK)) // && bb->SlideTimer==0)
    {
        bb->back_dx -= bb->back_dx / (8 * 2);
        bb->back_dz -= bb->back_dz / (8 * 2);
    ...
    if ((bb->flag & BIKE_FLAG_ONGROUND_BACK) && !(bb->flag & BIKE_FLAG_ONGROUND_FRONT)) {
