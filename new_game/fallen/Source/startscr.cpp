        ...
        if (widget->tag) {
            ...
            if (widget->tag < 100) {
                ...
                for (wig = form->children; wig; wig = nxt) {
                    nxt = wig->next;
                    if ((wig->tag > 99) && (wig->tag < 999) && wig->methods->Free)
...
BOOL brief_dlg(Form* form, Widget* widget, SLONG message)
{
    ...
    if (!widget) {
        ...
        if (message == WFN_PAINT) {
            t = form->age * 6;
            if (t > 255)
                MENUFONT_Draw(10, 120, 256, mission_name, 0xffFFFFFF, 0);
            if (t < 512) {
                c = (t > 255) ? 511 - t : t;
...
SLONG LoadMissionList(Form* form, CBYTE* script, SLONG district, UBYTE firstonly)
{
    ...
    while (1) {
        ...
        if ((text[0] == '/') && (text[1] == '/')) {
            ...
            if (firstonly && (mdata->ParentID != 1))
                mdata->District = -9; // heh... force disabled
...
SLONG MainWidgetLoop(void)
{
    ...
    if (!form) {
        ...
        case 10:
            ...
            if (txt)
                widget->methods->Data(widget, WTM_ADDBLOCK, 0, (SLONG)txt);
            widget->methods->Char(widget, 13);
