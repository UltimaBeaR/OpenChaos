    ...
    SBYTE blue)
{
    ...
    for (i = 0; i < ns->number; i++) {
        ...
        if ((nsl->x << 2) + map_x == (x & ~3) && (nsl->z << 2) + map_z == (z & ~3) && nsl->y == y && nsl->red == (red >> 1) && nsl->green == (green >> 1) && nsl->blue == (blue >> 1) && nsl->radius == radius) {
            //
...
#define NIGHT_DARKEN_GROUND 16
                                    ...
                                    if (red <= NIGHT_DARKEN_GROUND) {
                                        red = 0;
                                    } else {
                                        red -= NIGHT_DARKEN_GROUND;
                                    }
                                    if (green <= NIGHT_DARKEN_GROUND) {
                                        green = 0;
                                    } else {
                                        green -= NIGHT_DARKEN_GROUND;
                                    }
                                    if (blue <= NIGHT_DARKEN_GROUND) {
                                        blue = 0;
                                    } else {
                                        blue -= NIGHT_DARKEN_GROUND;
                                    }
...
SLONG NIGHT_load_ed_file(CBYTE* name)
{
    ...
    if (FileRead(handle, &sizeof_ed_light, sizeof(SLONG)) != sizeof(SLONG))
        goto file_error;
    if (FileRead(handle, &ed_max_lights, sizeof(SLONG)) != sizeof(SLONG))
        goto file_error;
    if (FileRead(handle, &sizeof_night_colour, sizeof(SLONG)) != sizeof(SLONG))
        goto file_error;
    ...
    for (i = 0; i < ed_max_lights; i++) {
        if (FileRead(handle, &el, sizeof(ED_Light)) != sizeof(ED_Light))
            goto file_error;
    ...
    if (FileRead(handle, &ed_light_free, sizeof(SLONG)) != sizeof(SLONG))
        goto file_error;
    ...
    if (FileRead(handle, &flag, sizeof(ULONG)) != sizeof(ULONG))
        goto file_error;
    if (FileRead(handle, &amb_d3d_colour, sizeof(ULONG)) != sizeof(ULONG))
        goto file_error;
    if (FileRead(handle, &amb_d3d_specular, sizeof(ULONG)) != sizeof(ULONG))
        goto file_error;
    if (FileRead(handle, &amb_red, sizeof(ULONG)) != sizeof(ULONG))
        goto file_error;
    if (FileRead(handle, &amb_green, sizeof(ULONG)) != sizeof(ULONG))
        goto file_error;
    if (FileRead(handle, &amb_blue, sizeof(ULONG)) != sizeof(ULONG))
        goto file_error;
    if (FileRead(handle, &lampost_red, sizeof(SBYTE)) != sizeof(SBYTE))
        goto file_error;
    if (FileRead(handle, &lampost_green, sizeof(SBYTE)) != sizeof(SBYTE))
        goto file_error;
    if (FileRead(handle, &lampost_blue, sizeof(SBYTE)) != sizeof(SBYTE))
        goto file_error;
    if (FileRead(handle, &padding, sizeof(UBYTE)) != sizeof(UBYTE))
        goto file_error;
    if (FileRead(handle, &lampost_radius, sizeof(SLONG)) != sizeof(SLONG))
        goto file_error;
    if (FileRead(handle, &sky_colour, sizeof(NIGHT_Colour)) != sizeof(NIGHT_Colour))
        goto file_error;
