...
BOOL ChooseD3DDevice(ULONG flags)
{
    ...
    if (GotD3D2) {
        ...
        if (dd_result == DD_OK) {
            ...
            case 1:
                DeviceBitDepth = DDBD_1;
                break;
            case 2:
                DeviceBitDepth = DDBD_2;
                break;
            case 4:
                DeviceBitDepth = DDBD_4;
                break;
            case 8:
                DeviceBitDepth = DDBD_8;
                break;
            case 16:
                DeviceBitDepth = DDBD_16;
                break;
            case 24:
                DeviceBitDepth = DDBD_24;
                break;
            case 32:
                DeviceBitDepth = DDBD_32;
                break;
            default:
                DeviceBitDepth = 0;
            }
            ...
            if (dd_result == D3D_OK && DeviceFound) {
                ...
                dd_sd.dwFlags = DDSD_WIDTH | DDSD_HEIGHT | DDSD_CAPS | DDSD_ZBUFFERBITDEPTH;
                ...
                if (d3d_DeviceDesc.dwDeviceZBufferBitDepth & DDBD_32) {
                    dd_sd.dwZBufferBitDepth = 32;
                    LogText("32 bit zbuffer\n");
                }
                if (d3d_DeviceDesc.dwDeviceZBufferBitDepth & DDBD_24) {
                    dd_sd.dwZBufferBitDepth = 24;
                    LogText("24 bit zbuffer\n");
                }
                if (d3d_DeviceDesc.dwDeviceZBufferBitDepth & DDBD_16) {
                    dd_sd.dwZBufferBitDepth = 16;
                    LogText("16 bit zbuffer\n");
                }
                if (d3d_DeviceDesc.dwDeviceZBufferBitDepth & DDBD_8) {
                    dd_sd.dwZBufferBitDepth = 8;
                    LogText("8 bit zbuffer\n");
                }
                /*
    ...
    LPVOID lpUserArg)
{
    ...
    {
        if (
            (RequestFlags & D3D_GOURAUD) && !(lp_d3d_device_desc->dpcTriCaps.dwShadeCaps & D3DPSHADECAPS_COLORGOURAUDMONO))
            return D3DENUMRET_OK; // No Ramp gouraud shading.
    } else // RGB device.
    {
        if (
            (RequestFlags & D3D_GOURAUD) && !(lp_d3d_device_desc->dpcTriCaps.dwShadeCaps & D3DPSHADECAPS_COLORGOURAUDRGB))
