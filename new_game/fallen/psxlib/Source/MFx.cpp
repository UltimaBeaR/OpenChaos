...
void MFX_init(void)
{
    ...
    s_attr.mask = (SPU_VOICE_VOLL | SPU_VOICE_VOLR | SPU_VOICE_VOLMODEL | SPU_VOICE_VOLMODER | SPU_VOICE_PITCH | SPU_VOICE_ADSR_AMODE | SPU_VOICE_ADSR_SMODE | SPU_VOICE_ADSR_RMODE | SPU_VOICE_ADSR_AR | SPU_VOICE_ADSR_DR | SPU_VOICE_ADSR_SR | SPU_VOICE_ADSR_RR | SPU_VOICE_ADSR_SL);
...
AudioOffset* MFX_Find_Speech(SLONG waypoint, SLONG conv, SLONG conv_off)
{
    ...
    for (i = 0; (i < MFX_Speech_files) && (found == -1); i++) {
        if ((Audio_data[i].waypoint == waypoint) && (Audio_data[i].conversation == conv) && (Audio_data[i].conv_off == conv_off))
            found = i;
    }
    if (found != -1)
