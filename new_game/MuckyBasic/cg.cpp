        ...
        if (!ST_find(pn->label)) {
            ...
            if (ST_found_value <= CG_generating_line) {
                ...
                case PARSE_NODE_TYPE_GOTO:
                    ...
                    break;
                case PARSE_NODE_TYPE_GOSUB:
                    CG_forward_goto[CG_forward_goto_upto].type = CG_FORWARD_GOTO_TYPE_GOSUB;
                    break;
...
SLONG CG_do(CBYTE* fname, SLONG output)
{
    ...
    if (output & CG_OUTPUT_EXECUTABLE) {
        ...
        if (fwrite(&mh, sizeof(mh), 1, handle) != 1)
            goto file_error;
        ...
        if (fwrite(CG_instruction, sizeof(SLONG), CG_instruction_upto, handle) != CG_instruction_upto)
            goto file_error;
        ...
        if (fwrite(CG_data_table, sizeof(UBYTE), CG_data_table_upto, handle) != CG_data_table_upto)
            goto file_error;
...
#define CG_WRITE_MAGIC()                                   \
    {                                                      \
        if (fwrite(&magic, sizeof(SLONG), 1, handle) != 1) \
            goto file_error;                               \
    }
        ...
        if (fwrite(&lh, sizeof(lh), 1, handle) != 1)
            goto file_error;
...

        {
            ...
            if (fwrite(lg, sizeof(LINK_Global), CG_global_upto, handle) != CG_global_upto)
                goto file_error;
            CG_WRITE_MAGIC();
        ...
        for (i = 1; i < CG_func_upto; i++) {
            ...
            if (fwrite(&lf, sizeof(lf), 1, handle) != 1)
                goto file_error;
        }
        ...
        if (fwrite(CG_line, sizeof(CG_Line), CG_line_upto, handle) != CG_line_upto)
            goto file_error;
        CG_WRITE_MAGIC();
        if (fwrite(CG_jump, sizeof(CG_Jump), CG_jump_upto, handle) != CG_jump_upto)
            goto file_error;
        CG_WRITE_MAGIC();
...

        {
            ...
            if (fwrite(ld, sizeof(LINK_Field), CG_field_id_upto, handle) != CG_field_id_upto)
                goto file_error;
            CG_WRITE_MAGIC();
        ...
        if (fwrite(CG_globalref, sizeof(CG_Globalref), CG_globalref_upto, handle) != CG_globalref_upto)
            goto file_error;
        CG_WRITE_MAGIC();
        if (fwrite(CG_undefref, sizeof(CG_Undefref), CG_undefref_upto, handle) != CG_undefref_upto)
            goto file_error;
        CG_WRITE_MAGIC();
        if (fwrite(CG_fieldref, sizeof(CG_Fieldref), CG_fieldref_upto, handle) != CG_fieldref_upto)
            goto file_error;
        CG_WRITE_MAGIC();
        if (fwrite(CG_datatableref, sizeof(CG_Datatableref), CG_datatableref_upto, handle) != CG_datatableref_upto)
            goto file_error;
        CG_WRITE_MAGIC();
        ...
        if (fwrite(&CG_debug_data_upto, sizeof(SLONG), 1, handle) != 1)
            goto file_error;
        if (fwrite(CG_debug_data, sizeof(CBYTE), CG_debug_data_upto, handle) != CG_debug_data_upto)
            goto file_error;
