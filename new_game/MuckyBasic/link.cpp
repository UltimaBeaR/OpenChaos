...
#define LINK_MAGIC_CHECK()                                \
    {                                                     \
        if (fread(&magic, sizeof(SLONG), 1, handle) != 1) \
            goto file_error;                              \
        ASSERT(magic == 12345678);                        \
    }
    ...
    for (i = 0; i < num_object_files; i++) {
        ...
        if (fread(&lh, sizeof(LINK_Header), 1, handle) != 1)
            goto file_error;
        ...
        if (fread(LINK_instruction + LINK_instruction_upto, sizeof(SLONG), lh.num_instructions, handle) != lh.num_instructions)
            goto file_error;
        ...
        if (fread(LINK_table_data + LINK_table_data_upto, sizeof(CBYTE), lh.data_table_length_in_bytes, handle) != lh.data_table_length_in_bytes)
            goto file_error;
        ...
        if (fread(&lf->num_debug_data, sizeof(SLONG), 1, handle) != 1)
            goto file_error;
        ...
        if (fread(LINK_debug_data + LINK_debug_data_upto, sizeof(CBYTE), lf->num_debug_data, handle) != lf->num_debug_data)
            goto file_error;
    ...
    if (fwrite(LINK_instruction, sizeof(SLONG), LINK_instruction_upto, handle) != LINK_instruction_upto)
        goto file_error;
    ...
    if (fwrite(LINK_table_data, sizeof(UBYTE), LINK_table_data_upto, handle) != LINK_table_data_upto)
        goto file_error;
