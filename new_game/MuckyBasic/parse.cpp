...
PARSE_Node* PARSE_copy_tree(PARSE_Node* tree)
{
    ...
    *ans = *tree;
    ...
    if (ans->child1) {
        ans->child1 = PARSE_copy_tree(ans->child1);
    }
    if (ans->child2) {
        ans->child2 = PARSE_copy_tree(ans->child2);
    }
    if (ans->child3) {
        ans->child3 = PARSE_copy_tree(ans->child3);
    }
...
SLONG PARSE_trees_the_same(PARSE_Node* tree1, PARSE_Node* tree2)
{
    ...
    switch (tree1->type) {
    case PARSE_NODE_TYPE_SLUMBER:
        if (tree1->slumber != tree2->slumber)
            return FALSE;
        break;

    case PARSE_NODE_TYPE_FLUMBER:
        if (tree1->flumber != tree2->flumber)
            return FALSE;
        break;

    case PARSE_NODE_TYPE_STRING:
        if (strcmp(tree1->string, tree2->string) != 0)
            return FALSE;
        break;
    ...
    case PARSE_NODE_TYPE_LOCAL:
    case PARSE_NODE_TYPE_EXPORT:
        if (strcmp(tree1->variable, tree2->variable) != 0)
            return FALSE;
        break;
    ...
    case PARSE_NODE_TYPE_LABEL:
    case PARSE_NODE_TYPE_GOSUB:
        if (strcmp(tree1->label, tree2->label) != 0)
            return FALSE;
        break;

    case PARSE_NODE_TYPE_BOOLEAN:
        if (tree1->boolean != tree2->boolean)
            return FALSE;
        break;
    ...
    if (tree1->child1 && !tree2->child1)
        return FALSE;
    if (!tree1->child1 && tree2->child1)
        return FALSE;

    if (tree1->child2 && !tree2->child2)
        return FALSE;
    if (!tree1->child2 && tree2->child2)
        return FALSE;

    if (tree1->child3 && !tree2->child3)
        return FALSE;
    if (!tree1->child3 && tree2->child3)
        return FALSE;
    ...
    if (tree1->child1 && !PARSE_trees_the_same(tree1->child1, tree2->child1))
        return FALSE;
    if (tree1->child2 && !PARSE_trees_the_same(tree1->child2, tree2->child2))
        return FALSE;
    if (tree1->child3 && !PARSE_trees_the_same(tree1->child3, tree2->child3))
        return FALSE;
...
PARSE_Node* PARSE_primary()
{
    ...
    case LEX_TOKEN_TYPE_BUFFER:
        ...
        switch (lt.type) {
        case LEX_TOKEN_TYPE_TEXTURE:
            ans->type = PARSE_NODE_TYPE_TEXTURE;
            break;
        case LEX_TOKEN_TYPE_BUFFER:
            ans->type = PARSE_NODE_TYPE_BUFFER;
            break;
        ...
        if (lt.type != LEX_TOKEN_TYPE_OPEN) {
            ...
            switch (ans->type) {
            case PARSE_NODE_TYPE_TEXTURE:
                PARSE_throw("No open bracket after the keyword TEXTURE");
                break;
            case PARSE_NODE_TYPE_BUFFER:
                PARSE_throw("No open bracket after the keyword BUFFER");
                break;
        ...
        if (lt.type == LEX_TOKEN_TYPE_CLOSE) {
            ...
            if (lt.type != LEX_TOKEN_TYPE_CLOSE) {
                ...
                switch (ans->type) {
                case PARSE_NODE_TYPE_TEXTURE:
                    PARSE_throw("No close bracket after the keyword TEXTURE");
                    break;
                case PARSE_NODE_TYPE_BUFFER:
                    PARSE_throw("No close bracket after the keyword BUFFER");
                    break;
    ...
    case LEX_TOKEN_TYPE_LEN:
        ...
        switch (lt.type) {
        case LEX_TOKEN_TYPE_SQRT:
            ans->type = PARSE_NODE_TYPE_SQRT;
            break;
        case LEX_TOKEN_TYPE_ABS:
            ans->type = PARSE_NODE_TYPE_ABS;
            break;
        case LEX_TOKEN_TYPE_SIN:
            ans->type = PARSE_NODE_TYPE_SIN;
            break;
        case LEX_TOKEN_TYPE_COS:
            ans->type = PARSE_NODE_TYPE_COS;
            break;
        case LEX_TOKEN_TYPE_TAN:
            ans->type = PARSE_NODE_TYPE_TAN;
            break;
        case LEX_TOKEN_TYPE_ASIN:
            ans->type = PARSE_NODE_TYPE_ASIN;
            break;
        case LEX_TOKEN_TYPE_ACOS:
            ans->type = PARSE_NODE_TYPE_ACOS;
            break;
        case LEX_TOKEN_TYPE_ATAN:
            ans->type = PARSE_NODE_TYPE_ATAN;
            break;
        case LEX_TOKEN_TYPE_LEN:
            ans->type = PARSE_NODE_TYPE_LEN;
            break;
        ...
        if (lt.type != LEX_TOKEN_TYPE_OPEN) {
            ...
            switch (ans->type) {
            case PARSE_NODE_TYPE_SQRT:
                PARSE_throw("Missing open bracket after SQRT");
                break;
            case PARSE_NODE_TYPE_ABS:
                PARSE_throw("Missing open bracket after ABS");
                break;
            case PARSE_NODE_TYPE_SIN:
                PARSE_throw("Missing open bracket after SIN");
                break;
            case PARSE_NODE_TYPE_COS:
                PARSE_throw("Missing open bracket after COS");
                break;
            case PARSE_NODE_TYPE_TAN:
                PARSE_throw("Missing open bracket after TAN");
                break;
            case PARSE_NODE_TYPE_ASIN:
                PARSE_throw("Missing open bracket after ASIN");
                break;
            case PARSE_NODE_TYPE_ACOS:
                PARSE_throw("Missing open bracket after ACOS");
                break;
            case PARSE_NODE_TYPE_ATAN:
                PARSE_throw("Missing open bracket after ATAN");
                break;
            case PARSE_NODE_TYPE_LEN:
                PARSE_throw("Missing open bracket after LEN");
                break;
        ...
        if (lt.type != LEX_TOKEN_TYPE_CLOSE) {
            ...
            switch (ans->type) {
            case PARSE_NODE_TYPE_SQRT:
                PARSE_throw("Missing close bracket after SQRT");
                break;
            case PARSE_NODE_TYPE_ABS:
                PARSE_throw("Missing close bracket after ABS");
                break;
            case PARSE_NODE_TYPE_SIN:
                PARSE_throw("Missing close bracket after SIN");
                break;
            case PARSE_NODE_TYPE_COS:
                PARSE_throw("Missing close bracket after COS");
                break;
            case PARSE_NODE_TYPE_TAN:
                PARSE_throw("Missing close bracket after TAN");
                break;
            case PARSE_NODE_TYPE_ASIN:
                PARSE_throw("Missing close bracket after ASIN");
                break;
            case PARSE_NODE_TYPE_ACOS:
                PARSE_throw("Missing close bracket after ACOS");
                break;
            case PARSE_NODE_TYPE_ATAN:
                PARSE_throw("Missing close bracket after ATAN");
                break;
            case PARSE_NODE_TYPE_LEN:
                PARSE_throw("Missing close bracket after LEN");
                break;
    ...
    case LEX_TOKEN_TYPE_RIGHT:
        ...
        switch (lt.type) {
        case LEX_TOKEN_TYPE_LEFT:
            ans->type = PARSE_NODE_TYPE_LEFT;
            break;
        case LEX_TOKEN_TYPE_RIGHT:
            ans->type = PARSE_NODE_TYPE_RIGHT;
            break;
        ...
        if (lt.type != LEX_TOKEN_TYPE_OPEN) {
            ...
            switch (ans->type) {
            case PARSE_NODE_TYPE_LEFT:
                PARSE_throw("Missing open bracket after LEFT");
                break;
            case PARSE_NODE_TYPE_RIGHT:
                PARSE_throw("Missing open bracket after RIGHT");
                break;
        ...
        if (lt.type == LEX_TOKEN_TYPE_CLOSE) {
            ...
            switch (ans->type) {
            case PARSE_NODE_TYPE_LEFT:
                PARSE_throw("Expected a comma or a close bracket after the first argument to LEFT");
                break;
            case PARSE_NODE_TYPE_RIGHT:
                PARSE_throw("Expected a comma or a close bracket after the first argument to RIGHT");
                break;
        ...
        if (lt.type != LEX_TOKEN_TYPE_CLOSE) {
            ...
            switch (ans->type) {
            case PARSE_NODE_TYPE_LEFT:
                PARSE_throw("Expected a close bracket after LEFT");
                break;
            case PARSE_NODE_TYPE_RIGHT:
                PARSE_throw("Expected a close bracket after RIGHT");
                break;
...
PARSE_Node* PARSE_p5exp()
{
    ...
    while (1) {
        ...
        case LEX_TOKEN_TYPE_CPROD:
            ...
            switch (lt.type) {
            case LEX_TOKEN_TYPE_TIMES:
                ans->type = PARSE_NODE_TYPE_TIMES;
                break;
            case LEX_TOKEN_TYPE_DIVIDE:
                ans->type = PARSE_NODE_TYPE_DIVIDE;
                break;
            case LEX_TOKEN_TYPE_MOD:
                ans->type = PARSE_NODE_TYPE_MOD;
                break;
...
PARSE_Node* PARSE_p4exp()
{
    ...
    while (1) {
        ...
        case LEX_TOKEN_TYPE_MINUS:
            ...
            switch (lt.type) {
            case LEX_TOKEN_TYPE_PLUS:
                ans->type = PARSE_NODE_TYPE_PLUS;
                break;
            case LEX_TOKEN_TYPE_MINUS:
                ans->type = PARSE_NODE_TYPE_MINUS;
                break;
...
PARSE_Node* PARSE_p3exp()
{
    ...
    while (1) {
        ...
        case LEX_TOKEN_TYPE_GTEQ:
            ...
            switch (lt.type) {
            case LEX_TOKEN_TYPE_LT:
                ans->type = PARSE_NODE_TYPE_LT;
                break;
            case LEX_TOKEN_TYPE_GT:
                ans->type = PARSE_NODE_TYPE_GT;
                break;
            case LEX_TOKEN_TYPE_LTEQ:
                ans->type = PARSE_NODE_TYPE_LTEQ;
                break;
            case LEX_TOKEN_TYPE_GTEQ:
                ans->type = PARSE_NODE_TYPE_GTEQ;
                break;
...
PARSE_Node* PARSE_p2exp()
{
    ...
    while (1) {
        ...
        case LEX_TOKEN_TYPE_NOTEQUAL:
            ...
            switch (lt.type) {
            case LEX_TOKEN_TYPE_EQUALS:
                ans->type = PARSE_NODE_TYPE_EQUALS;
                break;
            case LEX_TOKEN_TYPE_NOTEQUAL:
                ans->type = PARSE_NODE_TYPE_NOTEQUAL;
                break;
...
PARSE_Node* PARSE_expression()
{
    ...
    while (1) {
        ...
        case LEX_TOKEN_TYPE_XOR:
            ...
            switch (lt.type) {
            case LEX_TOKEN_TYPE_OR:
                ans->type = PARSE_NODE_TYPE_OR;
                break;
            case LEX_TOKEN_TYPE_XOR:
                ans->type = PARSE_NODE_TYPE_XOR;
                break;
...
PARSE_Node* PARSE_statement()
{
    ...
    case LEX_TOKEN_TYPE_PRINT:
        ...
        if (LEX_get().type == LEX_TOKEN_TYPE_NEWLINE || LEX_get().type == LEX_TOKEN_TYPE_COLON) {
    ...
    case LEX_TOKEN_TYPE_GOSUB:
        ...
        switch (lt.type) {
        case LEX_TOKEN_TYPE_GOTO:
            ans->type = PARSE_NODE_TYPE_GOTO;
            break;
        case LEX_TOKEN_TYPE_GOSUB:
            ans->type = PARSE_NODE_TYPE_GOSUB;
            break;
    ...
    case LEX_TOKEN_TYPE_RETURN:
        ...
        if (lt.type == LEX_TOKEN_TYPE_ELSE || lt.type == LEX_TOKEN_TYPE_ENDIF || lt.type == LEX_TOKEN_TYPE_COLON || lt.type == LEX_TOKEN_TYPE_NEWLINE) {
            //
