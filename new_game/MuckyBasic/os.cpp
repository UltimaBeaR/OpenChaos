    ...
    ULONG* dest_num_bytes)
{
    ...
    case ACMERR_NOTPOSSIBLE:
        TRACE("The requested operation cannot be performed.\n");
        break;
    case MMSYSERR_INVALFLAG:
        TRACE("At least one flag is invalid. 			  \n");
        break;
    case MMSYSERR_INVALHANDLE:
        TRACE("The specified handle is invalid. 			  \n");
        break;
    case MMSYSERR_INVALPARAM:
        TRACE("At least one parameter is invalid. 		  \n");
        break;
    case MMSYSERR_NOMEM:
        TRACE("The system is unable to allocate resources. \n");
        break;
    }
...
OS_Texture* OS_texture_create(CBYTE* fname, SLONG invert)
{
    ...
    if (!ti.valid || OS_bit_count(ti.width) != 1 || OS_bit_count(ti.height) != 1) {
        if (!ti.valid) {
...
OS_Texture* OS_texture_create(SLONG width, SLONG height, SLONG format)
{
    ...
    if (OS_bit_count(width) > 1 || OS_bit_count(height) > 1) {
        return NULL;
    }
...

    {
        ...
        if (width > dh.dwMaxTextureWidth || height > dh.dwMaxTextureHeight) {
            return NULL;
        }
    }
    ...
    LPARAM param_l)
{
    ...
    case WM_KEYUP:
        ...
        if (KEY_on[KEY_LSHIFT] || KEY_on[KEY_RSHIFT])
            KEY_shift |= KEY_SHIFT;
        if (KEY_on[KEY_LCONTROL] || KEY_on[KEY_RCONTROL])
            KEY_shift |= KEY_CONTROL;
        if (KEY_on[KEY_LALT] || KEY_on[KEY_RALT])
            KEY_shift |= KEY_ALT;
...
void OS_clip_helper_interpolate(void* new_point, void* point1, void* point2, float along)
{
    ...
    if (otn->X < 0.0F) {
        otn->clip |= OS_CLIP_LEFT;
    } else if (otn->X > OS_screen_width) {
        otn->clip |= OS_CLIP_RIGHT;
    }

    if (otn->Y < 0.0F) {
        otn->clip |= OS_CLIP_TOP;
    } else if (otn->Y > OS_screen_height) {
        otn->clip |= OS_CLIP_BOTTOM;
    }
    ...
    if (clip_and & OS_CLIP_TRANSFORMED) {
        ...
        if (clip_or & OS_CLIP_TRANSFORMED) {
            ...
            if (clip_or & OS_CLIP_FAR) {
                ...
                clip_and = OS_trans[ov1->trans].clip & OS_trans[ov2->trans].clip & OS_trans[ov3->trans].clip;
