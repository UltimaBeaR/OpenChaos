...
## Текстовая кодировка игры

**Выяснено в ходе Этапа 2 (2026-03-16)** при конвертации исходников в UTF-8.

Игровой рендерер шрифта (`font2d.cpp`, `Font.cpp`) использует байт из строки напрямую как индекс в таблицу глифов — системная кодовая страница Windows игнорируется.

**Кодировка строк в рантайме — Windows-1252 (= Latin-1 для байт ≥ 0xA0).**

Подтверждение: в таблице пунктуации шрифта константа `FONT_PUNCT_POUND` стоит на позиции `0xA3`, что в Windows-1252 = `£` (знак фунта стерлингов). Совпадение с Latin-1 в этом диапазоне (0xA0–0xFF идентичны в обеих кодировках).

**Следствия для портирования:**
- Строковые литералы в исходниках с non-ASCII символами → хранить как `\xNN` с оригинальным байтом Windows-1252
- Локализационные `.txt` файлы (`lang_*.txt`, `citsez_*.txt`) читаются как сырые байты через `fgets` → **не конвертировать в UTF-8**, оставить в Windows-1252
- В новом движке: если текстовый рендерер будет поддерживать Unicode — потребуется таблица перекодировки Windows-1252 → Unicode для загрузки старых `.txt` файлов

**Кодировка редакторских исходников — CP850.**

Файлы из `Editor/` (Edit.cpp, FileReq.cpp и др.) были написаны в DOS-редакторе с кодовой страницей CP850, а не Windows-1252. Обнаружено при конвертации в UTF-8: байт `0xA7` в Edit.cpp в контексте "faces along the 0° axis" и "Angle: %f°" — в Windows-1252 это `§`, в CP850 это `º` (визуально близко к знаку градуса, что соответствует смыслу). Редакторские файлы всё равно удаляются в конце Этапа 2 — практического значения не имеет.

---

## ⚠️ Переименованные макросы (конфликты со стандартной библиотекой)

**Подробности:** `new_game_devlog/stage4_bug_define_collisions.md` + `stage4_bug_define_collisions__audit_results.md`.

Оригинальная кодовая база определяет макросы с именами, совпадающими со стандартной
библиотекой C/C++. В новой кодовой базе следующие макросы **переименованы с префиксом `UC_`**:

| Оригинальное имя | Новое имя | Значение | Причина переименования |
|-------------------|-----------|----------|----------------------|
| `TRUE` | `UC_TRUE` | `1` | Конфликт с `<windef.h>` — значения совпадают, но переименовано для явного разделения |
| `FALSE` | `UC_FALSE` | `0` | Конфликт с `<windef.h>` — аналогично |
| `INFINITY` | `UC_INFINITY` | `0x7fffffff` | **Подтверждённый баг.** Стандартный `INFINITY` из `<math.h>` = `((float)(1e+300))`, наш = целочисленный INT_MAX. Конфликт вызывал UB в Release (баг с кнопкой C). |

**При возвращении вырезанного кода из `original_game/`** (например BIKE, другие фичи
...
в конкретном месте.** Таблица аудита покрывает только `.cpp` файлы которые были в кодовой
базе на момент аудита — `.h` файлы и вырезанные файлы туда не попали. Проверять нужно
все файлы (и `.h` и `.cpp`) по месту.

**Как проверить** (трюк с препроцессором):
1. Временно заменить `#define TRUE 1` → `#define TRUE UCREDEFINED_TRUE` в хедере
   (и аналогично для `FALSE`, `INFINITY`)
2. Прогнать `clang++ -E` на файле с возвращаемым кодом
3. Grep'нуть `UCREDEFINED_` — если нашлось, макрос резолвился через наше переопределение
   и нужно менять на `UC_*`
4. Откатить временные изменения в хедере

Подробности методики — `new_game_devlog/stage4_bug_define_collisions.md` +
`stage4_bug_define_collisions__audit_results.md`.

**Приоритет:**
- **`INFINITY` — критично.** Без замены `SLONG x = INFINITY` станет UB (float +inf →
  signed int), что ломает логику в Release (подробности — Баг 4 в девлоге).
- **`TRUE` / `FALSE` — не критично.** Значения совпадают с SDK (`1` и `0`), поведение
  не сломается. Но для консистентности лучше заменить.

В будущем при переименовании других сущностей — дополнять эту таблицу.
