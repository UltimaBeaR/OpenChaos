...
### Альтернативный подход: D3D API shim

Вместо полной абстракции + рефакторинга можно создать **заглушку под D3D API вызовы** —
тонкий слой, который перехватывает D3D6/DDraw вызовы и эмулирует их через OpenGL.

**Плюсы:**
- Значительно меньше работы: не нужно рефакторить весь код, отделяя D3D от логики
- Код остаётся максимально близким к оригиналу → проще сравнивать поведение
- Быстрый путь к кросс-платформе

**Минусы:**
- Привязывает архитектуру к устаревшему D3D6 API навсегда
- **Не позволяет улучшать графику дальше** — застреваем в пределах D3D6 feature set
- По сути повторяет то, что делает dgVoodoo/WineD3D, но хуже (они отлажены годами)

**Вывод:** не рекомендуется как основной путь, но может быть полезен как
промежуточный шаг для быстрого получения рабочей OpenGL-версии.
