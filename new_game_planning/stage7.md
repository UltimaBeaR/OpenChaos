...
- tga.h перенесён в d3d_texture.cpp (private include).
- `CreateFonts` — из public метода D3DTexture стала file-static функцией (вызывается только из Reload_TGA).
...
**Осталось:**
- `d3d_texture.cpp` и `display.cpp` — `#include "assets/formats/tga.h"` в .cpp файлах.
  Бэкенд всё ещё сам загружает TGA (Reload_TGA, OpenTGAClump/CloseTGAClump).
  Нужно вынести TGA loading — бэкенд получает пиксели через callback или ge_* API.
