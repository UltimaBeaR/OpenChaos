...
**Фаза 6 — Framebuffer access:** 🔧 (реализовано, частично проверено — см. список ниже)
...
- [ ] `ge_capture_backbuffer_to_texture` — glCopyTexSubImage2D (flame feedback loop в меню). **Реализовано, не проверено** — проверить: главное меню, огненный эффект
- [ ] `ge_plot_pixel` / `ge_get_pixel` / `ge_plot_formatted_pixel` — прямой доступ к locked буферу. **Частично проверено:** звёзды (plot_pixel) ✅. **Не проверено:** шрифт FONT_draw (plot_pixel), скриншоты (get_pixel, клавиша S в debug), ge_plot_formatted_pixel (glow звёзд)
- [ ] `ge_blit_back_buffer` — SwapBuffers (эквивалент flip). **Реализовано, не проверено** — вызывается через AENG_blit()
