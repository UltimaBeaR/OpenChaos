...
### Шаг 4 — OpenGL реализация 🟡 ОСНОВНОЕ СДЕЛАНО
...
- Crash handler: `host.cpp`, пишет `OpenChaos.crash_log.txt` с RVA и стеком
...
**Фаза 6 — Framebuffer access:** 🔧 (реализовано, частично проверено — чеклист: [stage7_opengl_verification.md](stage7_opengl_verification.md))
**Фаза 7 — Vertex buffer pool:** ✅ (сделано в Фазе 2 + расширено в Фазе 3)
**Фаза 8 — Text rendering:** ❌ УДАЛЕНА (TrueType — мёртвый код, `DrawTextTT` нигде не вызывался, весь текст через FONT2D)
**Фаза 9 — Outro backend:** 🔧 (минимальная реализация работает, dual-texture не поддержан)
...
- [x] `ge_lock_screen` / `ge_unlock_screen` — glReadPixels → CPU буфер → wibble → fullscreen quad writeback. **Проверено:** лужи (wibble) ✅, звёзды (ge_plot_pixel) ✅
- [x] `ge_plot_pixel` / `ge_get_pixel` / `ge_plot_formatted_pixel` — прямой доступ к locked буферу. **Проверено:** звёзды (plot_pixel) ✅, скриншоты (get_pixel) ✅. **Не проверено:** шрифт FONT_draw (plot_pixel), ge_plot_formatted_pixel (glow звёзд)
- [x] `ge_blit_back_buffer` — SwapBuffers (эквивалент flip). Оба пути (blit и flip) делают SwapBuffers — экран обновляется каждый кадр.
- [ ] `ge_restore_screen_surface` — восстановление offscreen поверхности после device-lost (возможно no-op для GL)

Фаза 9 — Outro (план реализации):

Outro — отдельная мини-рендер система с собственными текстурами, render state, и **двумя texcoord** на вершину (bump mapping). Vertex формат: 40 байт (vs 32 в game backend). Нужен отдельный шейдер.

Фаза 9.1 — Текстуры:
- [ ] `oge_texture_create` — BGRA пиксели → GL текстура, linked list дедуп-кеш, поддержка invert
- [ ] `oge_texture_create_blank` — пустая GL текстура заданного размера
- [ ] `oge_texture_size` — вернуть сохранённый размер

Фаза 9.2 — Шейдер + рисование:
- [ ] Новый vertex shader (`outro_vert.glsl`) — как `tl_vert.glsl`, но с **двумя texcoord** выходами
- [ ] Новый fragment shader (`outro_frag.glsl`) — dual-texture multiply (`OS_DRAW_TEX_MUL`), additive, alpha blend, decal, no-texture. Без fog/color key/specular.
- [ ] VAO для 40-byte vertex (position vec3, rhw, color u8x4, specular u8x4, texcoord0 vec2, texcoord1 vec2)
- [ ] `oge_bind_texture` — привязка GL текстуры к stage 0 или 1
- [ ] `oge_draw_indexed` — upload в VBO/EBO, draw

Фаза 9.3 — Render state:
- [ ] `oge_change_renderstate` — OS_DRAW_* флаги → GL blend/depth/cull
- [ ] `oge_undo_renderstate_changes` — сброс к дефолтам

Не нужно реализовывать:
- `oge_shutdown` — outro говорит "DO NO CLEANUP!", никогда не вызывается
- `oge_texture_lock/unlock` — мёртвый код (никто не вызывает)
- `oge_texture_finished_creating` — no-op и в D3D
- `oge_init_renderstates` / `oge_calculate_pipeline` — внутренние, в GL не нужны (multitexture всегда есть)
...
**Чеклист проверки OpenGL:** → [stage7_opengl_verification.md](stage7_opengl_verification.md)
