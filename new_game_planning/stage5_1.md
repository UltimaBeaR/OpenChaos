...
### Weapon-specific DualSense feedback (дробаш, автомат)

**Проблема:** сейчас adaptive triggers + вибрация реализованы только для пистолета (B3).
Для дробаша и автомата — ничего: ни adaptive triggers, ни вибрации, ни haptics.
...
- Adaptive trigger R2: вибрация курка при стрельбе (trigger vibration / choppy resistance) —
  курок должно "колбасить" пока зажат, как в современных шутерах на DualSense.
  Dualsense-Multiplatform поддерживает `TriggerEffectType::Vibration` с frequency + amplitude
- Вибрация: серия коротких импульсов синхронно с очередью (не один удар как у дробаша)
- Возможно audio-to-haptic (B7): конвертация звука очереди в haptic feedback через miniaudio —
  даёт более натуральное ощущение чем программные импульсы
- Ощущение: тряска и отдача непрерывной очереди

**Пистолет (текущее состояние — ок):**
- Adaptive trigger R2: weapon click (короткое сопротивление + щелчок) — уже реализовано (B3)
- Вибрация: средний одиночный импульс — уже реализовано (A5)

**Референс:** современные шутеры на PS5 (Returnal, Ratchet & Clank, COD MW2) —
у каждого оружия свой профиль триггера и haptics.
