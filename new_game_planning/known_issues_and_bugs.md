...
| Проблема | Описание | Статус |
|----------|----------|--------|
| CRT heap corruption (debug build) | Debug Assertion Failed в `debug_heap.cpp:904-908`: `_CrtIsValidHeapPointer(block)` и `is_block_type_valid(header->_block_use)`. Появляется периодически во время геймплея. Давняя проблема — double free или невалидный указатель в legacy коде. При нажатии "Пропустить" приводит к крашу: `PolyPage::PointAlloc` (polypage.cpp:113) → `ge_vb_alloc` (core.cpp:1548) падает на мусорном указателе (corrupted heap data). **Вероятно связано с крашем Urban Shakedown** — heap corruption портит VB data, NVIDIA драйвер крашится на мусорных данных. Стандартный CRT assert (не наш ASSERT) — текст идёт в диалог, не в crash_log.txt. Добавлен `_set_abort_behavior(0, _WRITE_ABORT_MSG)` + `_set_invalid_parameter_handler` для перехвата CRT ошибок. | Исследовать |
...
| Крэш на синематике Urban Shakedown | **Стабильно воспроизводится:** запуск → Urban Shakedown → краш примерно в конце синематика. Только OpenGL бэкенд (D3D6 — ок). **Детальное расследование (сессия 2026-04-01):** (1) Краш в `glBufferData` при копировании наших данных в драйверный буфер (адрес `...237B` стабилен). (2) Наши данные **валидны**: page touch проходит, canary за VB не повреждён, индексы в пределах, вершины zero-init (calloc), NaN/0xCDCD проверки чисты. (3) CRT обнаружил **heap corruption: write past end of 65536-byte block** — но это **не наш VB** (canary цел). (4) `ge_vb_expand` имел use-after-free (`free(buf->data)`) — **исправлено** (leak вместо free). (5) Heap corruption — в **legacy game коде** (не в VB pool, не в GL backend). Портит heap, `glBufferData` крашится при внутренней аллокации. **Что сделано:** per-slot VBO/EBO/VAO (как D3D6), calloc вместо malloc, canary проверки, CRT report hook, max-index upload. **Следующие шаги:** искать heap overflow в legacy коде (65536-byte блок), возможно Address Sanitizer (если поддерживается clang на Windows). | Расследуется |
