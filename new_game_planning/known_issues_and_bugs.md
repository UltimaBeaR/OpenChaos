...
| ~~OpenGL: шрифты меню (olyfont2) с ghost RGB фоном~~ | ~~Текстура `olyfont2.tga` имела ненулевой RGB (до 255) в пикселях с alpha=0. При additive blending (POLY_PAGE_NEWFONT_INVERSE) ghost RGB добавлялся к фону → яркие прямоугольники за буквами.~~ **Исправлено:** обнуление RGB для alpha=0 пикселей в Font2 текстурах после edge bleeding (`backend_opengl/game/core.cpp`). Баг оригинальных текстур — на PS1 его нет, в D3D6 менее заметен из-за A4R4G4B4 квантования. | ✅ Исправлено |
| OpenGL: HUD иконка (fist и др.) с ghost RGB фоном | `PCdisplay01.tga` — фон (alpha=0) содержит ghost RGB (30532 пикселя, maxRGB=238/255/221). При additive blend виден ореол вокруг иконки. Нельзя обнулить RGB по alpha — иконка кулака сама рисуется через additive blend с alpha=0 (RGB = данные иконки). Нужен другой подход: пороговое обнуление, shader premultiply flag, или GL_RGBA4 для этой текстуры. В D3D6 ореол тоже есть, но менее заметен. На PS1 ореола нет. | Исследовать |
