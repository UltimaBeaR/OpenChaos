...
| OpenGL: нет анимации перехода между экранами меню | В D3D версии экраны меню (главное → выбор миссий → брифинг) переключаются с плавной slide-анимацией. В OpenGL задник появляется резко. Причина: `ge_blit_surface_to_backbuffer` — стаб, `FRONTEND_show_xition()` использует его для отрисовки промежуточных кадров анимации со смещением. | Реализовать |
