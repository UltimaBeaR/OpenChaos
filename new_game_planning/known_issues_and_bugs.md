...
| OpenGL: нет пара из люков (steam) | На Urban Shakedown во вступительном синематике при переключении со сцены Дарси на вторую сцену — виден люк, из которого должен идти пар, но пар не отображается. Скорее всего проблема OpenGL рендерера — эффект пара не рисуется или не доходит до pipeline. | Исследовать |
