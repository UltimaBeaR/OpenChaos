...
| ~~OpenGL: нет пара из люков (steam)~~ | ~~На Urban Shakedown пар из решёток люков не отображался.~~ **Исправлено (workaround):** fog шейдер интерпретировал `specular.a = 0` (дефолт для спрайтов) как "CPU fog = 100%" → полностью заменял цвет пара на fog color → с additive blend невидимый. Workaround: `SetFogEnabled(false)` для `POLY_PAGE_STEAM`. Корневая проблема — баг fog шейдера, см. ниже. | ✅ Исправлено (workaround) |
| OpenGL: fog шейдер ломает additive спрайты с specular.a=0 | Корневая проблема невидимого пара. Fog шейдер (`common_frag.glsl`) использует `specular.a` для переключения CPU fog vs table fog: если `< 0.99` — считает CPU fog. Спрайты из `SPRITE_draw_tex` имеют `specular=0` → fog_factor=0 → цвет заменяется на fog_color. В D3D6 table fog не имел этого эффекта — fog не убивал additive спрайты. Нужно починить логику fog в шейдере чтобы правильно обрабатывать случай "specular.a=0 при отсутствии CPU fog". Может затронуть другие additive pages (BANG, DRIP, DROPLET, FINALGLOW, WINMAP, ENVMAP, ADDITIVE, TEXT). Для STEAM — workaround через `SetFogEnabled(false)`. | Исследовать |
