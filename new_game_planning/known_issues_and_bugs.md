...
| OpenGL: шрифты меню с «размазанным» фоном | Шрифтовая текстура `olyfont2.tga` — 32-bit с полным альфа-градиентом (256 уровней). D3D6 хранит в A4R4G4B4 (4 бита/канал) → мелкие значения квантуются до 0. OpenGL RGBA8 сохраняет все значения → видна полупрозрачная «каша» вокруг букв. Попытка GL_RGBA4 не помогла — требуется дальнейшее исследование. | Исследовать |
| OpenGL: HUD иконки (fist и др.) с артефактами фона | Иконки используют additive blend (One,One) — чёрный фон = невидимый. Но билинейная фильтрация на границе создаёт тёмную кайму. В D3D6 16-бит текстуры квантуют эти значения до 0. | Исследовать |
