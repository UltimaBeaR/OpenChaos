...
| Z-buffer/порядок отрисовки лежаков у бассейна | Estate of Emergency — у пустого бассейна с аптечкой лежаки (lounge chairs) имеют проблемы с z-buffer: (1) листья на заднем плане рисуются сквозь лежаки, (2) два рядом стоящих лежака — один всегда рисуется поверх другого независимо от позиции камеры. Похоже лежаки не пишут в z-buffer или имеют неправильный порядок отрисовки. Возможно связано с общей проблемой порядка отрисовки объектов/листьев. Нужно проверить как выглядит в релизной PC-версии. | Исследовать (сверить с оригиналом) |
