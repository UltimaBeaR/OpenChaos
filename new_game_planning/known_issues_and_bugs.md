...
| Лужи пропадают на краю камеры | Лужи итерируются по gamut-ячейкам (`PUDDLE_get_start(z, xmin, xmax)`). Когда камера поворачивается, ячейка с лужей выпадает из gamut и лужа не рисуется, даже если ещё видна на экране. Присутствует в оригинале. **Причина:** gamut рассчитан для пола/стен, а лужи — плоские квады которые видны под углом дальше чем gamut покрывает. **Костыльный фикс:** заменить `for (z = NGAMUT_zmin; z <= NGAMUT_zmax; z++) { PUDDLE_get_start(z, NGAMUT_gamut[z].xmin, NGAMUT_gamut[z].xmax); }` на `for (z = 0; z <= 127; z++) { PUDDLE_get_start(z, 0, 127); }` в aeng.cpp (~строка 4876). Работает но итерирует все ячейки карты — лишняя работа. **Правильный фикс:** расширить gamut для луж на несколько ячеек за пределы видимого frustum'а. | Костыль (релизный баг) |
...
| Чёрные дырки на ближнем клиппинге | Целые полигоны (пол, стены, машины, персонажи) пропадают на периферии зрения. Баг оригинального движка. **Масштабное исследование (2026-04-06):** отключены 21 система CPU-reject — дырки остались. Установлено: проблема в CPU near-clip коде, зависит от `POLY_ZCLIP_PLANE`. Увеличение порога → больше дырок. Одно из подтверждённых мест: `POLY_perspective()` (poly.cpp). Подробности → [`black_holes_investigation.md`](../new_game_devlog/black_holes_investigation.md). | Исследуется |
