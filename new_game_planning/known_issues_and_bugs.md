...
| Outro: 3D модель не отображается (x64) | После перехода на OpenGL (этап 7) 3D-изображение в outro работало (без bump mapping — `OS_DRAW_TEX_MUL` dual-texture не реализован). После перехода на x64 (этап 8) 3D модель пропала полностью. **Причина установлена:** `IMP_binary_load` делает `fread(&ans, sizeof(IMP_Mesh), ...)` — структура содержит 10 указателей (4 байта на x86 → 8 на x64), sizeof не совпадает с файлом на диске → все поля после указателей читаются из неправильного смещения. Аналогично `IMP_Mat` (4 указателя) — `fread(ans.mat, sizeof(IMP_Mat), ...)` читает массив с неправильным stride. Аналог уже исправленного бага `CRINKLE_read_bin`. **Фикс:** читать поля вручную с фиксированными on-disk размерами, пропуская указатели (как сделано в crinkle). | Причина найдена |
