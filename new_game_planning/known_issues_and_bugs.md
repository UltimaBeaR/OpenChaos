...
| x64: пропадающие стены и ящики вблизи | После перехода на x64 некоторые куски стен и грани ящиков не рисуются при приближении, но видны издалека. Аудит rendering pipeline (facet.cpp, polypage.cpp, figure.cpp, fastprim.cpp, aeng.cpp) НЕ выявил 64-бит багов — все касты, sizeof, culling чисты. Подозрение на corrupted данные или memory issue. Не путать с "Чёрные дырки на ближнем клиппинге" (pre-existing баг). | Исследовать |
...
| Крэш на синематике Urban Shakedown | Крашит во время вступительного синематика миссии Urban Shakedown. Не сразу — какое-то время показывает, потом падает. **Стабильно воспроизводится:** запустить игру → загрузить сохранку → зайти в Urban Shakedown → краш на синематике. Crash log не собран — нужно воспроизвести с новым билдом (с датой в crash_log.txt) и символизировать. | Исследовать |
