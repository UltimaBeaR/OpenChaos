...
| OpenGL: сильные тормоза при огне в кадре | На уровне Urban Shakedown (и вероятно других с огнём) — когда огонь в кадре, FPS резко падает. Если отвернуть камеру от огня — тормоза пропадают. В D3D версии не тормозит. Огонь виден и в стартовом синематике и в самой игре — тормозит в обоих случаях. Подозрение: CPU-transform (`ge_draw_multi_matrix` / `ge_draw_indexed_primitive_lit`) вместо GPU-transform в шейдере — большое количество огневых частиц/ribbon'ов перегружает CPU path. В D3D всё считалось аппаратно через fixed-function pipeline. Также возможно: per-draw `glBufferData` (orphan + fill каждый draw call) при сотнях мелких draw calls огня — overhead на driver-side sync. Решение: вернуть GPU-transform (lit vertex shader) или батчить мелкие draw calls. | Исследовать |
...
| Крэш на синематике Urban Shakedown | Крашит во время вступительного синематика миссии Urban Shakedown. **Стабильно воспроизводится:** запуск → Urban Shakedown → краш в момент произнесения "district" (финал синематика). Проявления нестабильны: то ACCESS_VIOLATION в NVIDIA OpenGL драйвере (адрес ...237B стабильно), то CRT debug assertion (heap corruption), то процесс тихо завершается (TerminateProcess от драйвера — crash handler не срабатывает). Наблюдение: если проскипать диалог и перезагрузить миссию — при повторном просмотре не крашит (буферы уже переиспользованы, нет uninit данных). **Частичный фикс:** `ge_draw_indexed_primitive_vb` теперь загружает только реально используемые вершины (по max index), а не весь `1<<logsize` буфер с malloc-мусором. Не помогло полностью — краш сохраняется. **Гипотезы:** (1) CRT heap corruption (double free / use-after-free) в legacy коде портит VB data или GL state, (2) проблема специфична для OpenGL бэкенда (на D3D6 не воспроизводится). Тормоза огня в кадре (см. отдельный баг) могут быть связаны — перегрузка CPU path при множестве draw calls. | Расследуется |
