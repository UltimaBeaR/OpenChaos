...
| OpenGL: alpha sort отключён — возможны артефакты прозрачности | Alpha sort (`DrawSinglePoly` per-polygon) отключён для OpenGL бэкенда: 350+ отдельных draw calls × 0.2ms = 70-85ms на macOS GL→Metal. Без alpha sort прозрачные полигоны (лужи, отражения, стёкла) могут рисоваться в неправильном порядке. **Правильный фикс:** batched alpha sort — собрать sorted polys в один VBO/IBO, один draw call на текстурную страницу. Подробности → [`stage8_log.md`](../new_game_devlog/stage8_log.md) (2026-04-02). | Workaround (batched sort TODO) |
