...
| Автоскролл в меню паузы без геймпада | Если геймпад НЕ подключён, пункты меню паузы автоматически переключаются (как будто зажата стрелка вверх/вниз). С подключённым геймпадом проблемы нет. Вероятно неинициализированные оси геймпада возвращают ненулевые значения. Появилось после этапа 5.1 (SDL3 gamepad). | Исследовать |
