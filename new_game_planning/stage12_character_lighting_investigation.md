...
## 13. Финальный результат имплементации (2026-04-12)

### Что реально потребовалось помимо плана

План сработал в целом, но по пути выяснилось два критичных нюанса которые не были очевидны из KB-анализа:

**1. Неправильный масштаб нормалей в исходном плане.** Расследование предполагало что в figure.cpp `prim_normal * (1/256)` даёт tiny-magnitude vector (~1/256). На самом деле (по комментарию в [building.cpp:6485](../new_game/src/buildings/building.cpp#L6485)) `prim_normal[]` хранится нормированным **на длину 256**, т.е. умножение на 1/256 даёт **unit vector**. Соответственно:
- Исправленная формула: `cos_nl = dot_raw / 251.0f` (а не `× 256/251` как было в плане)
- Первая версия кода с неправильной формулой давала бинарное освещение: любой lit-вертекс → idx=63, любой shadow → idx=64, и множитель контраста не влиял ни на что. Это было поймано диагностикой (захардкодили idx=0 и idx=63 для проверки что таблица вообще имеет range).

**2. Классический Lambert сделал персонажей слишком тёмными.** После фикса масштаба чистый Lambert дал корректное направленное освещение, но визуально персонажи стали заметно темнее окружающего мира — окружение (здания, машины) идёт через другой lighting path и остаётся ярким, а персонажи с теневым cutoff'ом на idx=64 "проваливаются" в сцену.

Решение — **half-Lambert** (Valve/HL2 техника): `wrap = cos × 0.5 + 0.5` маппит весь диапазон косинуса [-1, 1] на [0, 1], все вертексы получают индекс в лит-рампе 0..63 без резкого cutoff'а:
- Передняя сторона (cos=1) → idx=63 (fully lit)
- Терминатор (cos=0) → idx=32 (половинная яркость)
- Задняя сторона (cos=-1) → idx=0 (ambient)

Это стандарт для character shading со времён HL2 (2004), де-факто используется в Source Engine, Unity/Unreal character shaders, мобильных играх. Документация: https://developer.valvesoftware.com/wiki/Half_Lambert

### Итоговый код

[polypage.cpp:646-730](../new_game/src/engine/graphics/pipeline/polypage.cpp#L646-L730) — функция `ge_draw_multi_matrix`, ветка `unlit`:
...
float dot_raw = nx * lx + ny * ly + nz * lz;
float cos_nl = dot_raw * (1.0f / 251.0f);    // undo fNormScale = 251
...
Полный комментарий в коде содержит всю контекстную информацию (background, data scales, byte 12 handling, half-Lambert обоснование, contrast tuning, перенос на GPU).

### Визуальный результат vs релиз

- **Наша версия (half-Lambert, mult=64):** плавный градиент по всей модели, нет резких обрывов, персонажи визуально интегрируются в сцену, естественный вид.
- **Релизная PC / PieroZ:** byte-quantized освещение 1999 года — резкие переходы, пересвеченная лит-сторона, "пластиковые" персонажи. Окружение при этом освещено нормально, поэтому персонажи выглядят оторванными от сцены.

Пользователь сравнил обе версии side-by-side и предпочёл нашу. **Осознанно отходим от канона релиза** — в одном ряду с AA, mipmap'ами и texture filtering, которые тоже улучшают визуал относительно 1999 года. Float-точность современного железа позволяет делать лучше чем byte-math того времени.

### Тюнинг множителя контраста

Пробовали значения `64, 80, 128, 192`:
- **64** — текущее, чистый half-Lambert, наиболее натуральный вид
- **80..128** — чуть больше контраста, ближе к byte-quantized look
- **192** — почти бинарное on/off, близко к пластиковому виду релиза

Выбрано **64** как самое гармоничное с окружением. Это параметр "по вкусу", не функциональный баг.

### Незакрытое

- **"Слишком ярко ночью на Auto Destruct"** — второй симптом того же бага. По гипотезе должен был исчезнуть вместе с основным фиксом (раньше отсутствие directional lighting давало персонажу общий ambient со всех сторон, а теперь — направленный свет от point lights ложится корректно). Ожидает визуальной проверки на сцене — если не исчез, расследовать отдельно.
- **Общая яркость ("пересатурация" лит-стороны в релизе)** — не баг у нас, это то самое отличие от релиза которое мы осознанно оставили. Если когда-нибудь захочется точнее повторить релиз, искать в `BuildMMLightingTable` (`×2` boost "PC people are lit x2"), `NIGHT_LIGHT_MULTIPLIER`, или modulate2x GPU blend mode. Не блокер для 1.0.
