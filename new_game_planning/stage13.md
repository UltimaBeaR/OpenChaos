...
- **Полный перенос функционала крестовины (D-pad) на левый стик** — сейчас ряд действий работает только через крестовину и не срабатывает со стика. Конкретные проблемы:
  ...
  - **⚠️ См. также:** [known_issues_and_bugs.md "Геймпад: на левом стике нет перекатов в стороны…"](known_issues_and_bugs.md) — приоритет повышен до 1.0, плюс предложена альтернативная схема (L2 как tactical-модификатор: L2+стик в сторону = перекат, L2+назад = задний ход, L2+назад+✕ = прыжок назад, авто-перекаты убрать). При реализации учитывать оба подхода.
