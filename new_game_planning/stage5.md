...
### Листья мерцают группами при повороте камеры ✅

`POLY_set_local_rotation_none()` в aeng.cpp был пустым макросом `{}` (оригинал: `#ifndef TARGET_DC`,
на Dreamcast — реальная функция, на PC — пустышка). Внутри `AENG_draw_dirt()` банки и гильзы
рендерятся через `MESH_draw_poly()` → `POLY_set_local_rotation()` → `SetTransform(WORLD, ...)`.
Листья рисуются батчами через `DrawIndexedPrimitive(D3DFVF_LVERTEX)` — D3D применяет World
transform к координатам. Без сброса transform листья рисовались относительно последней банки
в батче, а не в мировых координатах → мерцали группами при повороте камеры.
Фикс: удалён пустой макрос, используется реальная `POLY_set_local_rotation_none()` из poly.cpp.

### Бампмеппинг (crinkle) мигает на ящиках и дверях ✅

`facet.cpp:1224` — условие `(GAME_TURN & 0x20)` включало crinkle только каждые 32 такта из 64.
Оптимизация для железа 1999 года, на современном — просто мигание. Убрано условие.
