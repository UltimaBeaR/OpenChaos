...
- DualSense: LED-панель, вибрация, adaptive triggers — полностью отлажены, без глюков. Доработки до 1.0:
  - Audio-to-haptic (B7): конвертация игровых звуков в тактильную обратную связь (стрельба, взрывы, двигатель). API: `AudioHapticUpdate()` через miniaudio. Подробности → [`stage13_dualsense.md`](stage13_dualsense.md). **⚠️ 2026-04-15:** true audio-haptic через Sony API в 1.0 не войдёт — расследование показало что оба пути GamepadCore нерабочие и требуют 2+ недель. Вместо этого для 1.0 делаем упрощённый envelope-based подход (огибающая реального WAV → рамбл-моторы). См. [`new_game_devlog/dualsense_audio_haptic_investigation.md`](../new_game_devlog/dualsense_audio_haptic_investigation.md)
