...
**Текущее состояние (2026-04-27, после батча 34, сессия 16):**
- Discards count: 4534 (старт) → ... → **1821** (батч 34, регенерация) → **134 dead symbols в 38 файлах** (dc_map.py)
- Регенерация перед батчем 34: 1821 discards, 134 dead symbols в 38 файлах
...
2. **После батча 34 (1821 discards, 134 dead symbols) — оставшиеся реальные кандидаты:**
   - **НЕ удалять:** save_globals (save.cpp всегда компилируется), index_lookup (live source caller в aeng.cpp), PUDDLE_create (live caller в process_controls), FIGURE_draw_hierarchical_prim_recurse_individual_cull (inlined, source ref есть), set_person_in_building_through_roof (after ASSERT(0), source ref есть), camera_*/set_person_* stubs (live source callers в person.cpp)
   - Большинство оставшихся symbols — system noise (STL RTTI, XMM constants, _Avx2WmemEnabledWeakValue, wmemcpy, c_DwmMax*)
   - Реальных deletable кандидатов осталось мало — список фактически исчерпан
   - Следующий шаг: регенерировать и проверить актуальный список
