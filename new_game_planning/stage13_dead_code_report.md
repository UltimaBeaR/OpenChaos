...
**Текущее состояние (2026-04-27, после батча 35, сессия 17):**
- Discards count: 4534 (старт) → ... → **1821** (батч 34) → **1800** (батч 35) — **80 dead symbols в 19 файлах** (dc_map.py)
...
⚠️ **gc-sections dead-code pass ЗАВЕРШЁН** (батч 35, 2026-04-27). Все 80 оставшихся символов нельзя удалять — system noise, source-level callers, inlining artifacts. Следующий шаг — финальные проходы:

1. **Финальный проход: пустые файлы** — найти и удалить файлы, ставшие фактически пустыми после чистки (см. секцию "Финальный проход — удаление пустых файлов" в stage13_dead_code_cleanup.md)
2. **Финальный проход: лишние #include** — удалить unused includes (clangd помечает, делать после пустых файлов)
