...
**Симптом:** в retail PC при старте новой игры дефолтно выбрана **беговая** миссия (с препятствиями), не миссия с полицейской машиной. В нашей текущей версии — дефолт на машине.

**Проверено 2026-04-29:** на PS1 первой идёт именно **миссия с машиной** — наше поведение совпадает с PS1. Расхождение только с PC retail. Решение зависит от того какую версию считать эталоном — PS1 или PC retail.

**Где проверять:**
- **UI-35** `FRONTEND_MissionHierarchy` comparator direction — почти наверняка причина. Pre-release: `if (order < best_score)` → наименьший order. `new/`: `if (order > best_score)` → наибольший order. Инверсия выбора.
- PC retail даёт беговую миссию первой. PS1 даёт машинку первой. Наш вариант = PS1.
