...
**⚠️ KB не помещается в контекстное окно целиком!** Суммарно ~40+ файлов, каждый 100-350 строк.
Читать **только нужные подсистемы по одной**. Если задача затрагивает 5+ подсистем — использовать компактную версию: `original_game_knowledge_base_dense/DENSE_SUMMARY.md`.
Массовое чтение KB файлов забивает контекст и приводит к компакции (потере данных).
