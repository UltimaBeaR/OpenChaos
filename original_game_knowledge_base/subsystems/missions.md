# Система миссий — Urban Chaos (EWAY runtime)

**Связанный файл:** [missions_implementation.md](missions_implementation.md) — WPT→EWAY трансляция, CRIME_RATE, Win/Lose, binary .ucm формат
...
> MuckyBasic — отдельная изолированная система, не интегрированная с игрой. Вне скоупа анализа, см. [analysis_scope.md](../analysis_scope.md).
>
> Реальная логика миссий — непрерывный опрос (polling) `EWAY_process()` каждый кадр.

Файлы: `fallen/Source/eway.cpp` (~8509 строк), `fallen/Headers/eway.h`, `fallen/Headers/Mission.h`
...
## 4. Условия триггеров (EWAY_COND_*) — 42 типа (0–41)
...
## 5. Действия вэйпойнтов (WaypointType) — 57 WPT-типов (→ 52 EWAY_DO_* после трансляции)
...
## 7. Детали реализации (из анализа eway.cpp)
...
## 8. Setup-структуры (начальный расстановка объектов)
...
**Секции 10-13** (WPT→EWAY трансляция, CRIME_RATE, Win/Lose, binary .ucm формат) → [missions_implementation.md](missions_implementation.md)
