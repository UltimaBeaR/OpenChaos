...
Полный Win/Lose flow (GS_LEVEL_WON → FRONTEND_level_won, DarciDeadCivWarnings, cutscenes) → см. [game_states.md](game_states.md) раздел 5.

### Статистика миссии (Person.cpp):
```c
stat_killed_thug, stat_killed_innocent      // убийства guilty/non-guilty
stat_arrested_thug, stat_arrested_innocent  // аресты guilty/non-guilty
stat_count_bonus                            // бонусные предметы
stat_game_time                              // мс от start до set_stats()
```
Инициализация в `init_stats()` при загрузке уровня.
