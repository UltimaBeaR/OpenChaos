...
#define GRAVITY (-(128 * 10 * 256) / (80*80))   // = -51 юнит/тик² (integer division)
```
...
**Важно:** движение и физика умножаются на `TICK_RATIO >> TICK_SHIFT` —
это даёт frame-rate independence. Схема включает 4-кадровое сглаживание и clamp 0.5x–2.0x.
...
**Важно:** WMOVE — активная геймплейная фича (стоять на крыше машины, на платформах).
