...
|---------|-------|-----------|---------|
| **MAV** | `mav.cpp`, `mav.h` | Основная навигация NPC | A* на 128×128 сетке |
...
- **Внутри здания (INSIDE2)** → `INSIDE2_mav_*()`
...
- A* ищет только 32 шага вперёд, затем возвращает лучший частичный путь — NPC вызывает MAV_do() повторно
