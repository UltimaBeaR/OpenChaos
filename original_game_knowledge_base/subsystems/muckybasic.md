...
**Ключевая проблема при замене:**
- Исходников скриптов НЕТ — только скомпилированные `.ucm`
- Варианты:
  1. Написать дизассемблер для `.ucm` → восстановить логику скриптов
  2. Реализовать совместимую VM, которая читает `.ucm` напрямую
  3. Переписать скрипты вручную на новом языке, анализируя поведение игры
