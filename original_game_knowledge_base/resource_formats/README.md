...
| save_type | Значение |
|-----------|----------|
| < 18 | PSX-формат — другой layout объектов уровня |
| >= 18 | PC-формат — LoadGameThing struct |
| > 23 | Поле размера OB-данных (4 байта) перед PAP_Hi |
| >= 20 | Поле texture_set (4 байта) после объектов |
| >= 25 | psx_textures_xy данные (PSX only, не переносить) |
| >= 26 | Текстуры крыши: 128×128 × 2 байта (PSX only) |
...
[PAP_Hi данные] — высокое разрешение (128×128)
  struct PAP_Hi {                    // 6 байт каждый (UWORD+UWORD+SBYTE+SBYTE)
...
[объекты уровня] — позиции и параметры всех объектов (save_type >= 18)
...
#### .MOJ файлы — скомпилированные 3D модели (multi-prim)

**Функция загрузки:** `io.cpp::load_a_multi_prim()`

Внутренний формат движка для хранения иерархических объектов (автомобили, персонажи, окружение). Содержит Prim-структуры с вершинами и полигонами.

**Ключевые структуры:**
```c
struct PrimPoint {         // вершина — 6 байт (SWORD, не SLONG!)
    SWORD X, Y, Z;         // 2 байта каждый — координаты в локальном пространстве
};
...
struct PrimFace4 {         // четырёхугольный полигон
    ...
    UWORD flags;           // флаги (double-sided, transparent и т.д.)
};
```

**Важно:** PrimPoint использует `SWORD` (2-байтный знаковый int), а НЕ `SLONG` (4-байтный). Это важно для корректного парсинга формата.
...
**Структура KeyFrame** (из Editor/Headers/Anim.h — детальнее см. [characters.md](../subsystems/characters.md)):
```c
...
struct GameKeyFrame {
    ...
    GameFightCol *Fight;     // Коллизии для боя (у кадров удара)
};
```

Анимации — **vertex morphing**, не скелетная анимация. Встроены в `.IMP` файлы примов.
...
| 5 | 3D модели (.IMP, .MOJ) | Средняя | Критична |
| 6 | Звук (.WAV) | Низкая | Важна |
| 7 | Скрипты (.ucm) | Очень высокая | Критична для миссий |
| 8 | Анимации (встроены в .MOJ/.IMP) | Средняя | Критична |
