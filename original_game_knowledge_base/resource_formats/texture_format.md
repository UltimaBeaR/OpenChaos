...
**Рекомендация:** НЕ парсить .txc напрямую (проблема с size_t). Вместо этого:
- Читать оригинальные TGA файлы из `Textures/` напрямую (они есть в репозитории)
- OpenGL: `glGenTextures` + `stb_image` для загрузки TGA
- .txc — runtime optimization оригинала, не нужен в новой игре
...
Структуры TXTY/DXTXTY → в новой игре: UV координаты в float (Tx, Ty / atlas_size).
...
|-----------|---------|------------|
| .TGA файлы | 1:1 — переиспользовать | Стандартный формат, читается stb_image |
| .TXC архивы | НЕ нужен — загружать TGA | size_t платформозависим; TGA файлы есть в Debug/Textures/ |
