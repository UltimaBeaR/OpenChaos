...
## Ограничение размера файла — разбивка на части

Инструмент Read не может загрузить файл размером **≥49KB** за один вызов. Больше — получаешь только превью (2KB), остальное сохраняется во временный файл и в контекст не попадает.

**После сборки проверить размер:**
```bash
wc -c original_game_knowledge_base_dense/DENSE_SUMMARY.md
```

**Если файл ≥49000 байт** — разбить на части по ~25-30KB каждая (по границам секций `##`), столько частей сколько нужно. Части нумеруются: `DENSE_SUMMARY_2.md`, `DENSE_SUMMARY_3.md` и т.д. (первая часть остаётся `DENSE_SUMMARY.md`).

**Связь между частями — через пометку в конце каждого файла (кроме последнего):**
...
Аналогично в конце `DENSE_SUMMARY_2.md` если есть `DENSE_SUMMARY_3.md`, и т.д.

**Правила разбивки:**
- Резать только по границам секций `##` — никогда не в середине секции
- Каждая часть должна быть <49000 байт
- Внешние ссылки (CLAUDE.md, KB_INDEX.md) не менять — они всегда указывают на `DENSE_SUMMARY.md`; о наличии продолжений читающий узнаёт из самого файла
