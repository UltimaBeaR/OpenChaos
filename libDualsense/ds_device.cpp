...
// (Report ID 0x31 + 1-byte sub-ID + 47-byte payload + 25 reserved + 4-byte
// CRC32).
...
bool device_open_first(Device *out) {
  ...
  *out = Device{};
  ...
  // (correct interface picked on first try) this is a handful of lines.
  {
    ...
    for (auto *cur = devs; cur; cur = cur->next) {
      if (cur->product_id != DUALSENSE_PID &&
        ...
        continue;
      std::fprintf(stderr,
                   "[libDualsense] HID candidate #%d: path=%s iface=%d "
                   "usage_page=0x%04X usage=0x%04X\n",
                   idx++, cur->path ? cur->path : "<null>",
                   cur->interface_number, (unsigned)cur->usage_page,
                   (unsigned)cur->usage);
    }
  }
  ...
  if (match) {
    std::fprintf(stderr, "[libDualsense] opening by usage filter: %s\n",
  ...
  if (!dev) {
    ...
    if (match) {
      std::fprintf(stderr, "[libDualsense] opening by VID/PID fallback: %s\n",
                   match->path ? match->path : "<null>");
      dev = SDL_hid_open_path(match->path);
    }
  }
  ...
  out->connection =
      (match->interface_number == -1) ? Connection::Bluetooth : Connection::Usb;
...
int device_read_latest(Device *dev, std::uint8_t *buf,
  ...
  if (!dev || !dev->handle || !buf || buf_capacity == 0)
    return -1;
...
int device_get_feature_report(Device *dev, std::uint8_t report_id,
  ...
  if (!dev || !dev->handle || !buf || buf_capacity < 2)
    return -1;
  auto *sd = static_cast<SDL_hid_device *>(dev->handle);
...
void device_shutdown(Device *dev) {
  ...
  for (int i = 0; i < SHUTDOWN_TONE_DISABLE_RETRIES; ++i) {
    std::uint8_t rx[64] = {};
    std::size_t rx_n = 0;
    test_command(dev, TestDevice::Audio, ACTION_WAVEOUT_CTRL, disable_ctrl,
                 sizeof(disable_ctrl), rx, sizeof(rx), &rx_n);
...
bool device_send_init_packet(Device *dev) {
  ...
  if (device_write(dev, buf, sizeof(buf)) <= 0)
    return false;
