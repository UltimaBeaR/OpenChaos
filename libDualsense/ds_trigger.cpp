...
bool trigger_feedback(std::uint8_t out[10], std::uint8_t position,
  ...
  if (strength == 0)
    return trigger_off(out);

  const std::uint8_t force_value =
      static_cast<std::uint8_t>((strength - 1) & 0x07);
...
bool trigger_vibration(std::uint8_t out[10], std::uint8_t position,
  ...
  const std::uint8_t strength_value =
      static_cast<std::uint8_t>((amplitude - 1) & 0x07);
...
bool trigger_bow(std::uint8_t out[10], std::uint8_t start, std::uint8_t end,
  ...
  if (end == 0 || strength == 0 || snap == 0)
    return trigger_off(out);

  const std::uint16_t start_and_stop =
      static_cast<std::uint16_t>((1u << start) | (1u << end));
  const std::uint32_t force_pair = static_cast<std::uint32_t>(
      (((strength - 1) & 0x07) << 0) | (((snap - 1) & 0x07) << 3));
...
bool trigger_machine(std::uint8_t out[10], std::uint8_t start, std::uint8_t end,
  ...
  if (start > 8 || end > 9 || end <= start || amplitude_a > 7 ||
      amplitude_b > 7) {
    zero_slot(out);
    return false;
  }
  ...
  const std::uint32_t strength_pair = static_cast<std::uint32_t>(
      ((amplitude_a & 0x07) << 0) | ((amplitude_b & 0x07) << 3));
...
bool trigger_galloping(std::uint8_t out[10], std::uint8_t start,
  ...
  if (start > 8 || end > 9 || start >= end || second_foot > 7 ||
      first_foot > 6 || first_foot >= second_foot) {
    zero_slot(out);
    return false;
  }
  if (frequency == 0)
  ...
  const std::uint32_t time_and_ratio = static_cast<std::uint32_t>(
      ((second_foot & 0x07) << 0) | ((first_foot & 0x07) << 3));
...
bool trigger_limited_weapon(std::uint8_t out[10], std::uint8_t start_raw,
  ...
  if (start_raw < 0x10 || end_raw < start_raw || (end_raw - start_raw) > 100 ||
      strength > 10) {
    zero_slot(out);
    return false;
  }
