...
void parse_input_report(const std::uint8_t *r, InputState *out) {
  ...
  //
  // Layout per daidr reference (InputInfo.vue, BatteryLevel / ChargeStatus
  // enums):
  ...
  const std::uint8_t effective_lvl = (charge_status == 2) ? 10u
                                     : (batt_lvl_raw <= 10u)
                                         ? batt_lvl_raw
                                         : 10u /* UNKNOWN cap */;
  if (batt_lvl_raw <= 11u || charge_status == 2) {
    ...
    if (batt_lvl_raw <= 10u || charge_status == 2) {
      out->battery_level_percent =
          static_cast<std::uint8_t>(effective_lvl * 10u);
    }
  ...
  // Byte 0: bit 7 = contact (0 = finger DOWN, 1 = lifted), bits 0..6 = touch
  // ID. Bytes 1..3 pack X (12 bits, low) and Y (12 bits, high).
  {
  ...
  // Offsets from daidr/dualsense-tester
  // (router/DualSense/_utils/offset.util.ts).
  ...
  out->motion_timestamp = static_cast<std::uint32_t>(r[0x1B]) |
...
int device_read_input(Device *dev, InputState *out) {
  ...
  if (n == 0) {
    ...
    if (dev->connection == Connection::Bluetooth && dev->last_input_ms != 0) {
