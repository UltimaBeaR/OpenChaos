...
constexpr std::uint8_t ReleaseLedsDefault =
    0x08; // bit 3 — MUST stay 0 when setting LED/lightbar
...
void build_output_report(const OutputState &state, std::uint8_t *buf,
                         bool bluetooth) {
  if (!buf)
    return;
  ...
  std::uint8_t validFlag0 = ValidFlag0::TriggerRight | ValidFlag0::TriggerLeft;
  ...
  if (state.audio_volumes_enabled) {
    ...
    validFlag0 |= ValidFlag0::HeadphoneVolume | ValidFlag0::SpeakerVolume |
                  ValidFlag0::AudioControl;
  }

  const std::uint8_t validFlag1 =
      ValidFlag1::MicMute | ValidFlag1::LightbarRGB | ValidFlag1::PlayerLedMask;

  const std::uint8_t validFlag2 =
      ValidFlag2::PlayerLedBrightness | ValidFlag2::LightbarSetup;
  ...
  if (state.audio_volumes_enabled) {
    ...
    p[7] =
        (state.audio_route == OutputState::AudioRoute::Speaker) ? 0x30 : 0x00;
  }
