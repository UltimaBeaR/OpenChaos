...
//        0 = IDLE
//        1 = RUNNING
//        2 = COMPLETE      — data is final, read `out_capacity` bytes from
//        offset 4 3 = COMPLETE_2    — 56-byte chunk, keep polling for more
//        chunks 255 = TIMEOUT
...
// firmware update.
static constexpr std::size_t FEATURE_80_WIRE_SIZE =
    64; // reportId + 63 data bytes
...
//   const idealLength = featureReportMeta.items[0].reportCount;   // = 63 for
//   0x80 const newData = new Uint8Array(idealLength);                   //
//   zero-init newData.set([deviceId, actionId, ...params]);                  //
//   at offset 0 if (bluetooth) fillFeatureReportChecksum(reportId, newData); //
//   last 4 bytes device.sendFeatureReport(reportId, newData); // prepends
//   reportId
//
...
// both.
static bool send_test_request(Device *dev, std::uint8_t device_id,
  ...
  if (bt) {
    ...
    buf[FEATURE_80_WIRE_SIZE - 4] =
        static_cast<std::uint8_t>((crc >> 0) & 0xFF);
    buf[FEATURE_80_WIRE_SIZE - 3] =
        static_cast<std::uint8_t>((crc >> 8) & 0xFF);
    buf[FEATURE_80_WIRE_SIZE - 2] =
        static_cast<std::uint8_t>((crc >> 16) & 0xFF);
    buf[FEATURE_80_WIRE_SIZE - 1] =
...
static std::uint8_t poll_test_response(Device *dev, std::uint8_t device_id,
                                       std::uint8_t action_id,
                                       std::uint8_t *out_buf,
                                       std::size_t out_capacity,
                                       std::size_t *out_n) {
  if (out_capacity < FEATURE_81_WIRE_SIZE)
    return TS_TIMEOUT;

  for (int tries = 0; tries < POLL_MAX_TRIES; ++tries) {
    const int n = device_get_feature_report(dev, 0x81, out_buf, out_capacity);
    if (n >= static_cast<int>(RESPONSE_HEADER_SIZE) && out_buf[0] == 0x81 &&
        out_buf[1] == device_id && out_buf[2] == action_id) {
...
TestResult test_command(Device *dev, TestDevice device_id,
  ...
  if (!dev || !dev->handle || !out_data || !out_len)
  ...
  *out_len = 0;
  ...
  for (;;) {
    const std::uint8_t status =
        poll_test_response(dev, did, action_id, rx, sizeof(rx), &rx_n);
    if (status == TS_TIMEOUT)
      return TestResult::PollFailed;

    // Copy payload from this chunk.
    const std::size_t payload =
        (rx_n > RESPONSE_HEADER_SIZE) ? rx_n - RESPONSE_HEADER_SIZE : 0u;
...
bool test_command_set(Device *dev, TestDevice device_id, std::uint8_t action_id,
  ...
  if (!dev || !dev->handle)
    return false;
  return send_test_request(dev, static_cast<std::uint8_t>(device_id), action_id,
                           params, params_len);
}
...
bool get_mcu_unique_id(Device *dev, std::uint64_t *out) {
  ...
  *out = 0;
  std::uint8_t buf[16] = {};
  std::size_t n = 0;
  if (test_command(dev, TestDevice::System, TestAction::GetMcuUniqueId, nullptr,
                   0, buf, sizeof(buf), &n) != TestResult::Ok)
    return false;
...
bool get_bd_mac_address(Device *dev, std::uint8_t out[6]) {
  ...
  std::uint8_t buf[16] = {};
  std::size_t n = 0;
  if (test_command(dev, TestDevice::Bluetooth, TestAction::ReadBdAddr, nullptr,
                   0, buf, sizeof(buf), &n) != TestResult::Ok)
    return false;
  if (n < 6)
    return false;
...
bool get_pcba_id(Device *dev, std::uint64_t *out) {
  ...
  *out = 0;
  std::uint8_t buf[8] = {};
  std::size_t n = 0;
  if (test_command(dev, TestDevice::System, TestAction::ReadPcbaId, nullptr, 0,
                   buf, sizeof(buf), &n) != TestResult::Ok)
    return false;
  if (n < 6)
    return false;
  std::uint64_t v = 0;
...
static std::size_t get_fixed_length_data(Device *dev, TestDevice dev_id,
                                         std::uint8_t action_id,
                                         std::uint8_t *out,
                                         std::size_t expected_len) {
  std::size_t n = 0;
  if (test_command(dev, dev_id, action_id, nullptr, 0, out, expected_len, &n) !=
      TestResult::Ok)
    return 0;
  return n;
}
...
bool get_battery_voltage(Device *dev, BatteryVoltageRaw *out) {
  ...
  *out = BatteryVoltageRaw{};
  std::uint8_t buf[8] = {};
  std::size_t n = 0;
  if (test_command(dev, TestDevice::Power, TestAction::BatteryVoltage, nullptr,
                   0, buf, sizeof(buf), &n) != TestResult::Ok)
    return false;
  if (n == 0 || n > sizeof(out->data))
    return false;
...
static bool get_single_byte_state(Device *dev, TestDevice did,
  ...
  *out = 0;
  std::uint8_t buf[4] = {};
  std::size_t n = 0;
  if (test_command(dev, did, action_id, nullptr, 0, buf, sizeof(buf), &n) !=
      TestResult::Ok)
    return false;
  if (n < 1)
    return false;
