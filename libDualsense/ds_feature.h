...
struct FirmwareInfo {
  ...
  std::uint32_t mainFwVersion; // main application firmware (major<<24 |
                               // minor<<16 | patch)
...
struct SensorCalibration {
  ...
  std::int16_t
      gyro_pitch_plus; // +calibration point (raw units at +speed_ref deg/s)
  std::int16_t
      gyro_pitch_minus; // -calibration point (raw units at -speed_ref deg/s)
