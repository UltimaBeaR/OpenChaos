...
static const std::uint32_t s_crc32_table[256] = {
...
static constexpr std::uint32_t s_crc32_seed = 0xeada2d49u;

std::uint32_t crc32_compute(const std::uint8_t* buffer, std::size_t len)
{
    std::uint32_t result = s_crc32_seed;
    for (std::size_t i = 0; i < len; ++i) {
        const std::uint8_t idx = static_cast<std::uint8_t>(result) ^ buffer[i];
        result = s_crc32_table[idx] ^ (result >> 8);
    }
    return result;
}
