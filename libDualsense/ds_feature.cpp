...
bool get_firmware_info(Device *dev, FirmwareInfo *out) {
  ...
  *out = FirmwareInfo{};
  ...
  std::uint8_t buf[64] = {};
  if (device_get_feature_report(dev, 0x20, buf, sizeof(buf)) <= 0)
    return false;
...
bool get_sensor_calibration(Device *dev, SensorCalibration *out) {
  ...
  *out = SensorCalibration{};
  ...
  std::uint8_t buf[64] = {};
  if (device_get_feature_report(dev, 0x05, buf, sizeof(buf)) <= 0)
    return false;
...
bool get_bt_patch_version(Device *dev, std::uint32_t *out) {
  ...
  *out = 0;
  ...
  std::uint8_t buf[64] = {};
  if (device_get_feature_report(dev, 0x22, buf, sizeof(buf)) <= 0)
    return false;
