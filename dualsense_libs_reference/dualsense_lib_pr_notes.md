...
от автора самой спеки". Packing-код Weapon **байт-в-байт идентичен**:

**duaLib::Weapon**:
```cpp
uint16_t startAndStopZones = (1 << startPosition) | (1 << endPosition);
forces[dst + 0] = Weapon;
forces[dst + 1] = (startAndStopZones >> 0) & 0xff;
forces[dst + 2] = (startAndStopZones >> 8) & 0xff;
forces[dst + 3] = strength - 1;
```

**Наш OPENCHAOS-PATCH** в GamepadTrigger.h::Weapon25:
```cpp
const std::uint16_t startAndStopZones =
    static_cast<std::uint16_t>((1u << startPosition) | (1u << endPosition));
Compose[0] = static_cast<std::uint8_t>(startAndStopZones & 0xff);
Compose[1] = static_cast<std::uint8_t>((startAndStopZones >> 8) & 0xff);
Compose[2] = static_cast<std::uint8_t>(strength > 0 ? (strength - 1) : 0);
```

**Эквивалентно до последнего бита**. Различие только в синтаксисе C++
cast-ов и explicit strength-check вместо `return Off()`.

#### Что duaLib даёт на будущее

MIT-лицензированные реализации **всех** эффектов которых нет в
rafaelvaloto либе:
- `Feedback (0x21)` — правильный linear resistance (нужен для тормоза
  в машине вместо сломанного `Resistance`/0x01).
- `Vibration (0x26)` — правильная вибрация триггера (нужна для AK47
  вместо сломанного `MachineGun26`).
- `Bow (0x22)`, `Galloping (0x23)`, `Machine (0x27)` — правильные
  варианты всех сломанных функций.
- `MultiplePositionFeedback`, `SlopeFeedback`, `MultiplePositionVibration` —
  продвинутые варианты feedback/vibration с per-zone контролем.
- `Simple_Feedback (0x01)`, `Simple_Weapon (0x02)`,
  `Simple_Vibration (0x06)`, `Limited_Feedback (0x11)`,
  `Limited_Weapon (0x12)` — все raw-packing варианты.
- `Apple::*` и `ReWASD::*` wrappers — normalized-parameter версии
  и presets.

#### Следствие для PR #2 в апстрим

Изначальный план PR #2 — "исправить Weapon25 packing". После находки
duaLib план можно **расширить радикально**: предложить автору
rafaelvaloto/Dualsense-Multiplatform **полностью заменить
`GamepadTrigger.h`** на адаптированный triggerFactory.cpp из duaLib.
Это:
- Чинит **все** сломанные эффекты за один коммит (Bow22, Weapon25,
  MachineGun26, Machine27, Galloping23-partial).
- Добавляет ~15 отсутствующих реализаций.
- MIT лицензия совместима (rafaelvaloto либа тоже MIT или похожая
  permissive — проверить).
- Авторство сохраняется через copyright notice.
- Автор спеки (Nielk1) и автор реф-имплементации — одно лицо, нет
  risk of incompatibility между "спека говорит X, код делает Y".

Альтернатива — оставить PR #2 узким (только Weapon25 fix) и отдельно
предложить автору "consider adopting Nielk1's duaLib wholesale". На
решение автора либы.

### 6.4 Независимое подтверждение спеки из DualSenseX/DSX
...
### 6.5 Эмпирическое подтверждение (A/B тест)
...
### 6.8 Общий аудит ВСЕХ эффектов в либе — Weapon25 не единственный сломанный
...
### 6.9 Попутно — Limited_Weapon (0x12) и другие не реализованы
...
недостающих реализаций (см. таблицу в п. 6.8). Пока в OpenChaos
